...
1. **Script đồng bộ pubkey** (đọc `%USERPROFILE%\.ssh\id_rsa_gcp.pub`, nối vào metadata, **không** xóa key cũ):  
   `.\scripts\gcloud_staging_ssh_key_sync.ps1`  
...
| Mục                                                            | Đường dẫn / lệnh                                       |
| -------------------------------------------------------------- | ------------------------------------------------------ |
| Bật VM + set project                                           | `.\scripts\gcloud_staging_start.ps1`                   |
| Đồng bộ pubkey `hoangphat5393` + `id_rsa_gcp.pub` vào metadata | `.\scripts\gcloud_staging_ssh_key_sync.ps1`            |
