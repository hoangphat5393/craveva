...
### 3.1 Pricing Resolution Order (Revised based on Miao Lin SRS)

The system follows a 2-stage calculation process to comply with customer requirements (Volume Discount is applied _after_ determining the base unit price).

#### Stage 1: Determine Applicable Unit Price (Xác định đơn giá áp dụng)

```
1. Check for Company Customer Product Pricing (Client-Specific Product Price)   <-- Highest Priority
   ↓ (if not found)
2. Check for Company Customer Pricing (Corporate/Contract Blanket Discount)     <-- B2B Contract Price
   ↓ (if not found)
3. Check for Pricing Tier (Company-specific B2B Tier)                           <-- Company Tier Price
   ↓ (if not found)
4. Check for Pricing Tier (Platform-wide B2B Tier)                              <-- Platform Tier Price
   ↓ (if not found)
5. Use Public Pricing / Base Product Price                                      <-- Base Price
```

#### Stage 2: Apply Volume Discount (Áp dụng chiết khấu số lượng)

```
6. Check for Volume Discount Rules (Final Adjustment)
   - Input: Unit Price determined from Stage 1 + Purchase Quantity
   - Output: Final Price after Volume Discount
```
