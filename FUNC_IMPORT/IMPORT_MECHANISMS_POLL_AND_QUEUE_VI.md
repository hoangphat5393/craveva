...
**Sales History** là luồng **stream** riêng — **ưu thế khi Excel nhiều sheet** cần xử lý xuyên suốt (xem mục **1ter**). Job **`ImportSalesHistoryStreamJob`** không **throw** khi chỉ có lỗi từng dòng (client/SKU không tìm thấy, …): lỗi ghi vào cache `import_row_errors_*` và cộng **`invalid_status`** trong metrics; job **hoàn thành thành công** để Laravel **giảm `pending_jobs`** — thanh poll **0 / N** khớp tiến độ. Nếu trước đây **throw** sau khi merge metrics, batch coi là fail / hủy → **`processedJobs` = 0** dù metrics tăng. Batch dispatch dùng **`allowFailures()`** để một job crash (timeout) không hủy cả 1000+ job còn lại.
