...
**Sales History** là luồng **stream** riêng. **Poll UI** dùng chung; rủi ro timeout khi **bật worker trong poll** + **file lớn / quá nhiều job** — xử lý bằng **`--max-time`**, giảm job mỗi poll, hoặc tắt worker trong poll và dựa **cron/worker nền**.
