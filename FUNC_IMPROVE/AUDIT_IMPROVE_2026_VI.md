...
| **AI → SO webhook**                         | **2026-05-12:** `13_` + `14_` + `15_` gộp thành **`SO_AI_WEBHOOK_PROMPTS_VI.md`** (Part 1–3); `12_AI_*` giữ riêng (phương án dài hạn). **2026-05-13:** tài liệu inbound đồng bộ **REST** `/api/integrations/orders` (gỡ mô tả legacy webhook trong các file liên quan) — xem `FUNC_LOGIC/AUDIT_LOGIC_2026_VI.md` §10. |
...
## 9) Chu kỳ 2026-05-13 — REST inbound + sync INDEX

| Việc                      | Chi tiết                                                                                                                                                                                                           |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **REST**                  | Tài liệu `12_AI_*`, `SO_AI_WEBHOOK_PROMPTS_VI.md`, `04_WH_*`, `FUNC_LOGIC/*` (PM_READY, survey, warehouse, WH_PURCHASE, audit inbound) đã chỉnh khỏi mô tả `POST /ai-order-webhook/{hash}` như endpoint đang chạy. |
