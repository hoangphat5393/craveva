...
**Không thay thế:** `BIOMIXING_DEV_PLAN.md` (roadmap chi tiết) — file này là **bối cảnh nền + điểm tích hợp + thứ tự đọc**.
...
| **Batch / HSD tồn**                 | Có bảng `warehouse_product_batches`; KPI inventory đồng bộ batch                     | `ERP_SO_PO_DO_INV_WH_QA_VI.md` §1, `WAREHOUSE_TOM_TAT_NOI_BO.md` |
| **Sales DO**                        | Reserve khi confirm; trừ tồn khi ship; batch + expiry trên dòng; idempotent outbound | `ERP_SO_PO_DO_INV_WH_QA_VI.md` §2–3, `SalesDoService` (code)     |
...
**Cập nhật QA có timestamp:** `FUNC_LOGIC/ERP_SO_PO_DO_INV_WH_QA_VI.md` (**2026-04-23**).
...
Tóm tắt khớp `BIOMIXING_DEV_PLAN.md`:
...
Chi tiết schema & legacy: `FUNC_LOGIC/ERP_SO_PO_DO_GRN_SCHEMA_MATRIX_VI.md` (nếu cần).
...
1. **File này** (baseline).
2. `BIOMIXING_DEV_PLAN.md` (roadmap + phase).
3. **`BIOMIXING_FLOW_CONCEPTS_VI.md`** — luồng nghiệp vụ RM/FG, tồn kho, PO dùng chung, DO chung B2B (tránh nhầm scope).
4. **`BIOMIXING_PLAYBOOK_P0P1_VI.md`** — chia nhỏ Phase 0–1 (migration, spike kho, test, milestone) trước khi sprint.
5. `BIOMIXING_FLOW_CRACEVA_GAP.md` (bước xưởng — đọc kèm §3 baseline).
6. `FUNC_LOGIC/ERP_SO_PO_DO_INV_WH_QA_VI.md` + `FUNC_LOGIC/QUY_TRINH_PO_DO_SO_INVOICE_WAREHOUSE_VI.md`.
7. `BIOMIXING_PROTOTYPE_PLAN_VI.md` nếu làm POC trước.
