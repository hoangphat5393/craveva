...
| **Cập nhật**              | 2026-05-24                                                                                                                                |
| **Không thay thế**        | Vận hành kỹ thuật (`FUNC_LOGIC/PRODUCTION_BUSINESS.md`), khái niệm (`BIOMIXING_FLOW_CONCEPTS.md`) — file này là **luồng bước–cửa–tồn kho** |
...
**Liên kết vận hành:** [`BIOMIXING_DOC_HUB.md`](./BIOMIXING_DOC_HUB.md) · [`P0_MINI_UAT_CHECKLIST_BIOMIXING.md`](./P0_MINI_UAT_CHECKLIST_BIOMIXING.md) · [`BIOMIXING_UAT_AND_TEST_GUIDE.md`](./BIOMIXING_UAT_AND_TEST_GUIDE.md)
...
| ---- | ------------------------- | ---------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 2.1  | Tạo SP FG + RM (Products) | SKU tồn tại            | FG = `goods` (Manufactured product); NL = `raw_material` / `packaging` / `semi_finished` | Xem [`FUNC_LOGIC/PRODUCTION_BUSINESS.md`](../FUNC_LOGIC/PRODUCTION_BUSINESS.md) · SOP mục 0–2 |
...
| 2.6  | **Release**               | Draft, có BOM + dòng   | Snapshot → `production_order_bom_snapshot_items` + **reserve RM** (FEFO)                 | Chặn nếu thiếu `available`; xem `PRODUCTION_BUSINESS.md` §3                                         |
...
| ---- | ------------------------------------- | ------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------- |
| 3.1  | Tạo / mở batch                        | Release → batch; `batches.show` | Released; snapshot trên lệnh    | **Tự** insert `production_batch_consumptions` (`PRODUCTION_BUSINESS.md` §7) | —                                     |
...
**Epic:** `PRODUCTION_BUSINESS.md` §3 (P1c)

### 3.4 Post RM — quy đổi UOM

BOM có thể nhập **g** trên SP base **kg** → post phải trừ **0,1 kg**, không **100 kg**.

**Epic:** `PRODUCTION_BUSINESS.md` §3 + `FUNC_BUG/BUG_PRODUCTION_UOM.md` — Fixed 2026-05-20.
...
**Epic:** [`13_OPENING_STOCK_VS_WAREHOUSE_STOCK.md`](./13_OPENING_STOCK_VS_WAREHOUSE_STOCK.md) · backfill `warehouse:backfill-opening-stock-to-default`.
...
**UAT:** TC-P0-05-01…06 · `P0_QA_BA_MASTER_TEST_CASE_TABLE.md` phụ lục P0-05.
...
| --------------------------------------------- | ------- | ---------------------------------------------- |
| CCP / HACCP automation                        | 3+      | `BIOMIXING_GAP_STATUS.md`                   |
...
| ---------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------- |
| 2026-05-27 | Ghi **reserve tại Release**, gán lô không reserve, cancel release reservation (đồng bộ plan đã retire) | `PRODUCTION_BUSINESS.md`, `LEGACY_ARCHIVE.md` |
...
_Cuối mỗi sprint: PM/BA rà §9 + đối chiếu `BIOMIXING_GAP_STATUS.md`. Dev: cập nhật file này **trước khi merge** nếu đổi thứ tự bước, gate, hoặc SSOT tồn._
