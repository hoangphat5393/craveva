...
> **2026-04 notice:** This **2026-02-13** draft predates the current Hub SO/PO/DO/Warehouse stack. The **Extension → Warehouse** row and **Critical → Batch Tracking** row were **refreshed 2026-04** (`warehouse_product_batches`, Sales DO); other tables/sections may still read like an older snapshot — cross-check **`BIOMIXING_BASELINE_PREP_2026_VI.md`** and **`FUNC_LOGIC/ERP_SO_PO_DO_INV_WH_QA_VI.md`**. Biomixing **process gaps** (BOM, CCP, Production domain) below remain relevant.
...
| **Warehouse** | Stock, transfers, **multi-warehouse**, **batch/expiry** (`warehouse_product_batches`), Sales DO reservation/ship integration. | `Warehouse`, batches, movements | **Ready (ops layer).** Still missing **Production-linked** RM→work order→FG batch chain + HACCP receiving QC — see `BIOMIXING_BASELINE_PREP_2026_VI.md`. |
...
| **Batch Tracking**       | Traceability for biotech safety.   | **2026 update:** Batch/expiry on hand is implemented via **`warehouse_product_batches`** and Sales DO line batch identity (see `FUNC_LOGIC/ERP_SO_PO_DO_INV_WH_QA_VI.md`). **Remaining gap:** tie **production consumption / FG output** to those batches end-to-end (Production module). |
