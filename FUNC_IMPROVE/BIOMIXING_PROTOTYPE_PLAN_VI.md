...
**Mục đích:** Trả lời: hệ thống **đang thiếu module Production**; khách cần **chức năng** (không chỉ import file demo). Đây là **phạm vi prototype** (POC) và **ước lượng thời gian**, bám `BIOMIXING_DEV_PLAN.md` nhưng **cắt scope** so với MVP/Phase 1 đầy đủ.

**Trạng thái:** Kế hoạch nội bộ — chỉnh lại khi chốt scope với khách và sau spike kỹ thuật.

**Tham chiếu:** `BIOMIXING_DEV_PLAN.md`, **`BIOMIXING_BASELINE_PREP_2026_VI.md`** (nền kho/DO 2026), `2-4-2026_BIOMIXIN_DEMO_PREP_CHECKLIST.md` (demo = dữ liệu + kịch bản; **không** thay cho build module), `FUNC_LOGIC/WAREHOUSE_INDEX.md`.
...
- CCP gate cứng, rework đầy đủ, receiving QC, quality lock DO, sampling/COA, PRP, audit export — như **§4 Phase 2+** trong `BIOMIXING_DEV_PLAN.md`.
...
4. Sau prototype, roadmap **Phase 2+** (CCP, QC, …) vẫn theo `BIOMIXING_DEV_PLAN.md`.
