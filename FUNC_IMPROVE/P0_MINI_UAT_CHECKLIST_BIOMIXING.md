...
**Regression Dev gần nhất:** P0 bundle hiện tại (`BiomixingDemoRoutesReadinessTest`, `P0BiomixingAutomatedEvidenceTest`, Production posting/policy/variance, Warehouse reconciliation/batch routes) → **43 passed / 161 assertions** (2026-06-16). Focused P0-02 variance permission → **2 passed / 7 assertions** (2026-06-16).

**Focused regression bổ sung:** Products listing stock-on-hand formatter/query alias → `php artisan test tests\Unit\PurchaseProductsDataTableTest.php` → **5 passed / 12 assertions** (2026-06-14).
...
> **Đã vá code 2026-05-20** — UAT Luồng D: BOM line **g** trên SP **kg** → sau Post RM, tồn giảm **0,1** (không **100**). Ref: `PRODUCTION_BUSINESS.md` §3.
...
> **Đã vá code 2026-05-23** — Sau Post FG receipt, SP FG phải có dòng trên **Purchase → Inventory** (tìm theo tên/SKU, không mã lô). Ref: `PRODUCTION_BUSINESS.md` §3.
...
| E3   | (Tuỳ chọn) So sánh với Products `withSum inventory` | Pass | Browser UI re-run 2026-06-11: Products search **SP-FG-000011** hiển thị **Total Net Qty = 13** cho Manufactured Product **Bánh kem**, không còn `--`. Dev fix 2026-06-14 bổ sung rule datatable: ledger qty luôn hiển thị cho tracked/non-tracked; tracked không ledger = `0.0`; non-tracked không ledger = `--`. |

**Kết luận luồng E:** **Pass** — Ngày: **2026-05-24**; cập nhật Dev 2026-06-14 — _P1c OK trên Inventory; Products listing stock-on-hand đã có focused regression_
...
**Evidence tổng hợp:** Live demo Cursor browser **2026-05-24/25** (A–E, C PO/GRN/Bill, P0-05, WUP UI/API); Pest smoke **7** + WUP cluster **25** passed (`WarehouseUpgradeP0Test`, `WarehouseAvailabilityApiTest`, `SalesDoServiceLifecycleTest`, `WarehouseInboundMutualExclusionValidationTest`, `WarehouseUnitConversionFlowTest`). Regression P0 hiện tại **43 passed / 161 assertions** (2026-06-16). Focused P0-02 variance permission **2 passed / 7 assertions** (2026-06-16). Focused Products stock-on-hand regression **5 passed / 12 assertions** (2026-06-14).
