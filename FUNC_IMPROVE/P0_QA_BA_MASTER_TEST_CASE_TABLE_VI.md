...
**Phiên bản:** 2026-05-09  
**Tài liệu chi tiết (tham chiếu):** `P0_EXECUTION_LOG.md`, `P0_MINI_UAT_CHECKLIST_BIOMIXING_VI.md`, `P0_05_TRACE_BIDIRECTIONAL_UAT_CHECKLIST_VI.md`, `P0_VARIANCE_APPROVAL_ROLE_MATRIX_VI.md`, `04_WH_RUNBOOK_UPGRADE_VI.md` (§1 + §2.1.1).
...
4. **Phần A — test trên trình duyệt:** đi **theo thứ tự dòng** trong bảng (TC-P0-01-01 → … → TC-P0-08-C). Với mỗi dòng: đọc **Điều kiện** → làm **Bước thực hiện** trên UI → so **Kỳ vọng** → ghi **P** (Pass) / **F** (Fail) / **N** (N/A) và cột **Evidence** (screenshot, URL, mã issue).
...
**Ghi chú:** Các dòng **TC-P0-08-A/B/C** chỉ tóm tắt; chi tiết từng bước A1–C4 nằm trong `P0_MINI_UAT_CHECKLIST_BIOMIXING_VI.md`. Tương tự trace hai chiều chi tiết hơn trong `P0_05_TRACE_BIDIRECTIONAL_UAT_CHECKLIST_VI.md`.
...
| Môi trường (local / staging) + URL gốc |         |
| Tenant / Company pilot                 |         |
| Tester QA                              |         |
| Tester BA / PM                         |         |
| Build / commit (nếu có)                |         |
...
| TC-P0-05-01 | P0-05 | Trace P→W — mở lệnh và batch                     | Có order + batch đã post RM hoặc có movement gắn lô                                      | Production → Orders → chọn order → mở **batch**                                                                               | Trang batch load được                                                                                    |                 |                             |
| TC-P0-05-02 | P0-05 | Trace P→W — mở trang trace                       | TC-P0-05-01 Pass                                                                         | Từ batch → **Trace** (`production.batches.trace`)                                                                             | Trang trace hiển thị dòng movement / link                                                                |                 |                             |
| TC-P0-05-03 | P0-05 | Trace P→W — link sang Warehouse batch            | Trên trace có `warehouse_product_batch_id` hiển thị link                                 | Bấm **Open warehouse batch** (hoặc tương đương)                                                                               | URL dạng `/account/warehouse-product-batches/{id}`; đúng lô / SP / kho                                   |                 |                             |
| TC-P0-05-04 | P0-05 | Trace W→P — danh sách lô                         | User có `view_warehouse_stock` (hoặc tương đương)                                        | Warehouse → **Product batches** → mở một lô có tham chiếu Production                                                          | Chi tiết lô load được                                                                                    |                 |                             |
| TC-P0-05-05 | P0-05 | Trace W→P — link sang Production trace           | Lô có movement `reference_type` Production batch                                         | Trên chi tiết lô → bấm link **Production trace** (nếu có)                                                                     | Mở `production.batches.trace` đúng batch                                                                 |                 |                             |
| TC-P0-05-06 | P0-05 | Vòng kín tham chiếu                              | TC-P0-05-03 và TC-P0-05-05 Pass                                                          | Đối chiếu mã lệnh / mã batch / số lô giữa hai chiều                                                                           | Khớp nghiệp vụ kỳ vọng pilot                                                                             |                 |                             |
| TC-P0-06-01 | P0-06 | Widget đối soát tồn vs batch (tùy pilot)         | Đã bật module Warehouse; có quyền xem stock                                              | Mở màn **Warehouse stock** (index) → xem vùng đối soát / cảnh báo (nếu cấu hình)                                              | Widget hiển thị; không lỗi 500; số liệu có thể giải thích được (ghi chú nếu chênh lệch do nghiệp vụ)     |                 |                             |
| TC-P0-08-A  | P0-08 | Luồng Estimate → Sales Order                     | Có quyền Estimate + Order; có báo giá mẫu hoặc tạo mới                                   | Làm theo `P0_MINI_UAT_CHECKLIST_BIOMIXING_VI.md` luồng A (A1–A4)                                                              | Kết luận luồng A: Pass/Fail                                                                              |                 |                             |
| TC-P0-08-B  | P0-08 | Luồng SO → DO → Invoice                          | Có SO + tồn đủ (nếu cần)                                                                 | Checklist luồng B (B1–B4)                                                                                                     | Kết luận luồng B: Pass/Fail; không double outbound                                                       |                 |                             |
| TC-P0-08-C  | P0-08 | Luồng PO → GRN → Bill                            | Có vendor + PO                                                                           | Checklist luồng C (C1–C4)                                                                                                     | Kết luận luồng C: Pass/Fail                                                                              |                 |                             |
