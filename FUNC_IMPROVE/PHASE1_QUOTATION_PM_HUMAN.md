...
_Tóm tắt ngắn. **Bản yêu cầu gộp đầy đủ (Gary + Phase 1 chat, tiếng Việt có dấu, không AI):** [`PROJECT BIOMIXING/PM_YEU_CAU_TONG_HOP.md`](../PROJECT%20BIOMIXING/PM_YEU_CAU_TONG_HOP.md)._

**Tiến độ live / trạng thái code:** `BIOMIXING_GAP_STATUS.md` § Phase 1.
_Technical gap + trạng thái đã gộp về `BIOMIXING_GAP_STATUS.md`._
...
**Task nào xung đột nếu ép cho mọi công ty?** Bảng trạng thái Phase 1 nằm trong **`BIOMIXING_GAP_STATUS.md`**. Tóm lại: BOM, duyệt 2 cấp, chặn SO, copy BOM Production **chỉ** cho tenant bật Phase 1; Miaolin-style **tắt** module → giữ Quotation thường.
...
| Production BOM (module riêng)       | Định mức master; đã có copy Production BOM → BOM báo giá khi bật Phase 1   |
...
| File                                                  | Dùng khi                         |
| ----------------------------------------------------- | -------------------------------- |
| `PROJECT BIOMIXING/PM_YEU_CAU_TONG_HOP.md`         | **Spec PM gộp** (Gary + Phase 1) |
| `BIOMIXING_GAP_STATUS.md`                          | Bảng gap / trạng thái Phase 1    |
| `BIOMIXING_GAP_STATUS.md`                          | % tiến độ cho PM                 |
| `PROJECT BIOMIXING/PHASE_BUSINESS_CONTEXT_EXAMPLE.md` | President vs VP nghĩa nghiệp vụ  |

---

_Cuối: trạng thái Phase 1 hiện theo `BIOMIXING_GAP_STATUS.md`; file này giữ phần giải thích PM dễ đọc._
