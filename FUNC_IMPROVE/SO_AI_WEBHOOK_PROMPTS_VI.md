...
> **2026-05-13 — Cập nhật hành vi:** Inbound tạo đơn **chỉ** qua **`POST /api/integrations/orders`** (`AiIntegrationOrdersController`, middleware `ai.integration.*`). Đường `POST /ai-order-webhook/{hash}` và `AiOrderWebhookController` **đã gỡ** — xem [`../FUNC_LOGIC/AI_ORDER_LEGACY_WEBHOOK_REMOVED_VI.md`](../FUNC_LOGIC/AI_ORDER_LEGACY_WEBHOOK_REMOVED_VI.md). Các bảng / prompt Part 1–2 dưới đây ghi **lịch sử rollout**; khi áp dụng, thay endpoint bằng REST và đọc [`../docs/AI_ORDER_INTEGRATION_REST.md`](../docs/AI_ORDER_INTEGRATION_REST.md).
...
| Nguồn                                                           | Kết luận                                                                                                                                                                                                                                                                                                             |
| --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `routes/api.php` (nhóm `ApiRoute`)                              | `GET purchased-module` → `HomeController@installedModule` (tiện ích).                                                                                                                                                                                                                                                |
...
| `routes/api.php` (nhóm `integrations`, trước `ApiRoute::group`) | **`POST /api/integrations/orders`** → `AiIntegrationOrdersController@store` — endpoint JSON cho luồng “bên ngoài POST vào → tạo `Order` + `OrderItems`” (`StoreAiOrderWebhookRequest`; secret **theo công ty** hoặc fallback **`.env` `AI_ORDER_WEBHOOK_SECRET`**). Kèm probe `GET /api/integrations/__route_probe`. |
| Import SO                                                       | `OrderController` import / `ImportSalesOrderChunkJob` — **upload file / queue**, không phải REST contract cho AI real-time.                                                                                                                                                                                          |

**Kết luận:** **`POST /api/integrations/orders`** là inbound canonical (REST, secret trên header). Chi tiết: [`../docs/AI_ORDER_INTEGRATION_REST.md`](../docs/AI_ORDER_INTEGRATION_REST.md). Company Settings / Purchase Settings **không** chứa endpoint tạo SO qua API trong code đã quét (ngoài tab **Sale order settings**).

**Tài liệu inbound AI (payload, Postman):** `FUNC_LOGIC/PM_READY_AI_WEBHOOK_STAGING_VI.md`, `docs/AI_ORDER_INTEGRATION_REST_SETUP_VI.md`.  
**Phương án dài hạn (API vs tích hợp khác):** `FUNC_IMPROVE/12_AI_THIRDPARTY_SO_OPTIONS_VI.md`.
...
```text
Bối cảnh repo: Laravel 11, đa company. Inbound tạo Sales Order: POST /api/integrations/orders (AiIntegrationOrdersController, StoreAiOrderWebhookRequest). Xác thực: secret trên companies.ai_order_webhook_secret (ưu tiên, UI Sale order settings) hoặc fallback AI_ORDER_WEBHOOK_SECRET trong .env. OrderController@store là web + session.
...
2) Trang có ít nhất một tab **"API"** (nhãn UI ngắn gọn; nội dung là hướng dẫn tích hợp AI / third-party tạo SO). Tab khác (ví dụ đánh số SO) tách riêng nếu sau này có. Trong tab **API**: hiển thị read-only Base URL (app.url), URL POST REST /api/integrations/orders, header X-AI-Webhook-Secret hoặc Authorization Bearer, company_id của tenant hiện tại, link PM_READY + docs/AI_ORDER_INTEGRATION_REST*.md, gợi ý external_event_id để idempotent.
...
5) (Tùy chọn pha 2, liệt kê trong PR mô tả chứ chưa code nếu chưa được yêu cầu) Sanctum POST api/v1/companies/{company}/orders tái sử dụng validation/service chung với AiIntegrationOrdersController; secret theo company.
...
**Mục đích:** Theo dõi tiến độ triển khai tích hợp **bên AI / ai.craveva.com** gọi vào ERP để tạo **Sales Order** qua **`POST /api/integrations/orders`**, và màn **Sale order settings → tab API** cung cấp thông tin copy cho bên AI.  
...
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| **Inbound REST tạo đơn**                | `app/Http/Controllers/Api/Integrations/AiIntegrationOrdersController.php`                                                                                                                              | Core app; middleware `ai.integration.auth`, `ai.integration.method`.                                  |
...
| **Route**                               | `routes/api.php` (`POST integrations/orders`, …), `routes/web.php` (`sales-order-settings`)                                                                                                            | REST inbound + trang account cần đăng nhập.                                                           |
...
- [x] Endpoint **`POST /api/integrations/orders`** + `AiIntegrationOrdersController` + `StoreAiOrderWebhookRequest`
...
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Xác thực                        | Ưu tiên secret **`companies.ai_order_webhook_secret`** (UI); fallback **`AI_ORDER_WEBHOOK_SECRET`** trong `.env` → `config('app.ai_order_webhook_secret')`. Header **`X-AI-Webhook-Secret`** hoặc **`Authorization: Bearer`**, `hash_equals`. |
| URL                             | `POST {APP_URL}/api/integrations/orders`; header `X-AI-Webhook-Secret` hoặc `Authorization: Bearer` phải khớp secret (per-company hoặc fallback env).                                                                                         |
...
3. **Ringfence:**
    ...
    - **Tier B (khuyến nghị nếu PM yêu cầu an toàn mạnh):** **Token/secret riêng theo `company_id`** trong DB; inbound REST **chỉ** chấp nhận đơn khi secret header khớp đúng công ty (bỏ hoặc so khớp cứng `company_id` trong body theo thiết kế — đã có phần lớn ở code hiện tại).
...
4) (Chỉ làm nếu PM bật Tier B) Ringfence kỹ thuật: secret/token riêng theo company.
   ...
   - `AiIntegrationOrdersController`: secret per-company trong header → chỉ tạo Order khi `company_id` trong body khớp công ty của secret (đã có trong code hiện tại).
