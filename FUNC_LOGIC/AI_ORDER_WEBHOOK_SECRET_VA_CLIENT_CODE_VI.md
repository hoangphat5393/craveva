...
Tài liệu ngắn cho PM / kỹ thuật: inbound tạo Sales Order qua `POST /ai-order-webhook/{hash}` (middleware nhóm **web**, CSRF **đã trừ** cho `ai-order-webhook/*`).

**Cập nhật nội dung:** 2026-05-12 — đối chiếu `AiOrderWebhookController`, `StoreAiOrderWebhookRequest`, `OrderObserver`, cấu hình Warehouse, UI Sale order settings.
...
| ---- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | `routes/web-public.php`      | `POST ai-order-webhook/{hash}` → `AiOrderWebhookController@store`.                                                                                                                                                                                                                   |
...
| 3    | `AiOrderWebhookController`   | So khớp `{hash}` + header `X-AI-Webhook-Secret` với secret công ty hoặc env; 401 nếu sai. Gán lại `client_id` từ `$request->input('client_id')` sau `validated()` để khớp merge sau validate. Kiểm tra `company_id` vs công ty của secret per-company → 422 JSON tùy chỉnh nếu lệch. |
...
| CSRF                                | N/A (trừ route)                | `VerifyCsrfToken` except `ai-order-webhook/*`.                                                                                 |
...
```http
POST /ai-order-webhook/YOUR_WEBHOOK_SECRET_HEX HTTP/1.1
...
(`YOUR_WEBHOOK_SECRET_HEX` phải **giống** đoạn cuối path **và** giá trị sau dấu hai chấm trong header.)
