...
**Phạm vi:** `POST /ai-order-webhook/{hash}` → `AiOrderWebhookController@store`, `StoreAiOrderWebhookRequest`, cấu hình `config('app.ai_order_webhook_secret')`, tài liệu `PM_READY_AI_WEBHOOK_STAGING_VI.md`, `AI_ORDER_WEBHOOK_SECRET_VA_CLIENT_CODE_VI.md`, UI `sales-order-settings`.  
...
| Auth           | Secret **theo công ty** (`companies.ai_order_webhook_secret`) **hoặc** fallback `.env` `AI_ORDER_WEBHOOK_SECRET`; **segment URL `{hash}`** + header **`X-AI-Webhook-Secret`**, `hash_equals`. Với secret theo công ty: `company_id` trong body **phải** trùng công ty gắn secret. | **Custom shared secret** (không phải OAuth 2.0 / RFC 6750 Bearer token chuẩn)           |
