...
**Client module hiện có:** Form thêm/sửa client; import Excel theo chunk (mặc định 100 dòng/job), **bulk insert** custom_fields_data và **cache metadata** 1 lần/chunk; khi **client_code trùng** thì **cập nhật** client cũ (updateExistingClient). Các field thương mại chuẩn (`payment_terms`, `customer_grade`, `channel_type`, `business_type`, `business_closure_date`) lưu trong `client_details`; custom field Client chỉ dùng cho field do admin tạo trong UI và được append động vào import mapping.
...
| 10        | customer_grade        | `C級客戶` (`client_details`)              |
| 11        | channel_type          | `通路別` (`client_details`)               |
| 12        | business_type         | `型態別` (`client_details`)               |
| 13        | payment_terms         | `月結30天` (`client_details`)             |
| 14        | business_closure_date | `20251231` hoặc `31/12/2025` (`client_details`) |
...
**Tham chiếu:** `../FUNC_IMPROVE/IMPORT_CHUNK_BULK_QUEUE.md` · lịch sử phân tích: `git log -- FUNC_IMPROVE/08_CLIENT_IMPORT_REVIEW_AND_IMPROVEMENTS.md`
...
| ----------------------------------------- | ---------------------------------------------------------------------------------------- |
| Phân tích import (lịch sử)                | `git log -- FUNC_IMPROVE/08_CLIENT_IMPORT_REVIEW_AND_IMPROVEMENTS.md`                    |
| Chunk vs bulk insert, queue sync/database | FUNC_IMPROVE/IMPORT_CHUNK_BULK_QUEUE.md                                               |
| Cột file Miaolin vs DB/Custom             | (đã gộp) xem `FUNC_LOGIC/MAOLIN_BUSINESS.md` + `FUNC_LOGIC/MAOLIN_IMPORT_MAPPING.md` |

---

## 9. Trường import: DB vs Custom (khuyến nghị giữ hiện trạng)

**Áp dụng cho file:** Miaolin Customer / Miaolin Product Customer (cấu trúc cột tương tự). **Tham chiếu:** `MAOLIN_BUSINESS.md` và `MAOLIN_IMPORT_MAPPING.md`.
...
### 9.2. Trường core thương mại đã đưa vào `client_details`

Các trường bên dưới là cột DB chính, không còn cần custom field mặc định. Import/form/profile/list đọc ghi trực tiếp vào `client_details`.

| Trường DB             | Cột file (vd. Miaolin)               | Ghi chú                                            |
| --------------------- | ------------------------------------ | -------------------------------------------------- |
| customer_grade        | 客戶(集團)分級 \| Customer Grade     | Cột `client_details.customer_grade`                |
| channel_type          | 通路別 \| Channel Type               | Cột `client_details.channel_type`                  |
| business_type         | 型態別 \| Business Type              | Cột `client_details.business_type`                 |
| payment_terms         | 交易條件 \| Payment Terms            | Cột `client_details.payment_terms`                 |
| business_closure_date | 歇業日期 \| Business Closure Date    | Cột `client_details.business_closure_date`; có giá trị hợp lệ → set User.status = inactive |

### 9.2a. Custom field do admin tạo trong UI

Các field như `salesperson`, `department`, `sales_assistant_name`, `last_transaction_at`, `geographical-distinction` không còn được seed mặc định bằng migration Miaolin. Nếu công ty cần các field này, admin tạo trong Settings → Custom Fields → Client. `ClientImport::mergeDynamicColumns()` sẽ tự append các custom field hiện có vào import mapping theo company.

### 9.3. Cột file khác (đã map hoặc tùy chọn)

| Cột file / trường                    | Trạng thái    | Ghi chú                                                                                                                                         |
| ------------------------------------ | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **客戶(集團)分級 \| Customer Grade** | ✅ **Đã map** | Cột DB `client_details.customer_grade`; không nhầm với `pricing_tier_id` (Tier Pricing). |

### 9.4. Kết luận

- Các field thương mại đã chuẩn hóa vào DB (`client_details`); không seed custom field mặc định cho fresh project.
- Các custom field đặc thù công ty chỉ xuất hiện khi admin tạo trong UI; import mapping tự append từ DB.
- Cải thiện chính cho file ~17k dòng vẫn là **bulk insert custom_fields_data** + cache metadata trong chunk khi công ty có custom field riêng.
...
Import client dùng **custom field động theo DB** cho nhóm Client (`ClientImportProcessor::getClientCustomFieldNames()` và `ClientImport::mergeDynamicColumns()`). Khi admin xóa hoặc thêm custom field trong Settings (Client group):

| Tình huống                                                      | Ảnh hưởng                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Xóa custom field** (vd. xóa "salesperson" khỏi nhóm Client)   | **Không gây lỗi.** Field đã xóa không còn trong DB → import mapping không còn hiển thị field đó, và processor bỏ qua khi build `custom_fields_data`. Dữ liệu cũ gắn với custom field bị xóa sẽ bị xóa theo FK/cascade nếu field bị xóa trong DB. |
| **Thêm custom field mới** (vd. thêm "region" trong nhóm Client) | Field mới sẽ được `ClientImport::mergeDynamicColumns()` append vào import mapping theo company. Khi user map cột Excel vào field đó, import ghi vào `custom_fields_data`. |

**Tóm tắt:** Xóa custom field → an toàn về runtime, chỉ mất mapping và dữ liệu CF tương ứng theo chính sách DB. Thêm custom field mới → có thể import được mà không cần sửa code.
...
- Custom field: khi **bulk insert** (chunk job) thì processRow gọi với `skip_custom_fields => true`, job xóa sẵn custom_fields_data theo model_id rồi gom và insert cuối chunk; khi **không** dùng bulk thì gọi `saveCustomFieldsFromRow()` cho các custom field động của company. Các field thương mại core đã ghi vào `client_details`.
...
## 11. Quan hệ `users` ↔ `client_details` là 1-1

### 11.1. Kết luận hiện tại

- Nghiệp vụ hiện tại: mỗi client user có đúng một dòng `client_details`.
- DB đã có `UNIQUE` trên `client_details.user_id` (`client_details_user_id_unique`) qua migration `database/migrations/2026_03_15_000000_add_unique_user_id_to_client_details_table.php`.
- FK `client_details_user_id_foreign` (`client_details.user_id` → `users.id`) vẫn tồn tại.
- Eloquent khớp nghiệp vụ: `User::clientDetails()` là `hasOne`, `ClientDetails::user()` là `belongsTo`.

### 11.2. Vì sao tool schema từng hiểu nhầm 1-many?

Một số schematic/ER tool suy luận quan hệ dựa trên constraint:

- Có FK `client_details.user_id` → `users.id` nhưng không thấy unique thì tool có thể hiểu `users` → `client_details` là 1-many.
- Nếu tool đọc `database/schema/mysql-schema.dump` cũ chưa regenerate, dump có thể chưa phản ánh migration unique mới.

Vì vậy khi tool vẫn báo 1-many, cần kiểm tra tool đang đọc schema live hay file dump cũ.

### 11.3. Kiểm tra dữ liệu từng thực hiện

Kết quả kiểm tra dev/local ngày 2026-03-24:

| Kiểm tra | Kết quả |
| --- | --- |
| Số dòng `client_details` | 17.565 |
| Số `user_id` khác nhau trong `client_details` | 17.565 |
| Nhóm `user_id` có nhiều hơn 1 dòng `client_details` | 0 |
| User có role `client` nhưng thiếu `client_details` | 0 |
| `client_details.user_id` trỏ tới user không có role `client` | 0 |

### 11.4. Multi-company và email

`UNIQUE(client_details.user_id)` không thay đổi mô hình multi-tenant:

- `users.company_id` vẫn gắn user vào một company.
- `client_details.company_id` nên khớp với `users.company_id`.
- Việc một email có dùng lại được ở nhiều company hay không phụ thuộc vào thiết kế `users.email`, không phụ thuộc vào unique trên `client_details.user_id`.

Nếu sau này cần một identity/client thuộc nhiều company, đó là thay đổi kiến trúc lớn: cần pivot membership kiểu `company_users` / `company_clients`, hoặc đổi unique email theo `(company_id, email)`.
