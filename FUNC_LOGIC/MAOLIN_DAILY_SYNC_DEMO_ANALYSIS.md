...
- **Thiếu/không khớp:** thiếu `department`; có thêm `region` (地區別), `designated warehouse` (指定庫別名稱) nhưng hệ thống chưa map sẵn.
...
- **Lưu ý kỹ thuật quan trọng:** `ImportInventoryJob` hiện đang set `$warehouseId = null` (phần map kho bị tắt), nên import inventory có nguy cơ không gán kho đúng.
...
- Inventory import vào warehouse null (do mapping kho đang tắt) -> số liệu kho sai.
...
2. Bật lại hoặc implement rõ mapping `warehouse_code -> warehouse_id` cho inventory import.
