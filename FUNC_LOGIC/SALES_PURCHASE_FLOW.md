...
**GRN / Sales DO tables (2026-04):** Purchase receiving persists to **`grns` / `grn_items`** (`Grn`, `GrnItem`); sales outbound documents use **`sales_dos` / `sales_do_items`** (`SalesDo`, `SalesDoItem`). Tables **`delivery_orders`** / **`sales_shipments`** (và bảng dòng tương ứng) **đã DROP** trên các môi trường triển khai đã xác nhận — xem [`ERP_SO_PO_DO_GRN_SCHEMA_MATRIX_VI.md`](ERP_SO_PO_DO_GRN_SCHEMA_MATRIX_VI.md).
