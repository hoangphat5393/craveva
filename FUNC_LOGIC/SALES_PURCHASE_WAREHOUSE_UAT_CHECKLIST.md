...
**Đồng bộ code/tài liệu:** [`SALES_BUSINESS.md`](SALES_BUSINESS.md) · [`WAREHOUSE_BUSINESS.md`](WAREHOUSE_BUSINESS.md) · trả hàng bán [`SALES_RETURN_BUSINESS.md`](SALES_RETURN_BUSINESS.md) · trả hàng mua [`PURCHASE_RETURN_BUSINESS.md`](PURCHASE_RETURN_BUSINESS.md).
...
| 2.1.2 | Tạo Sales DO có `warehouse_id`, dòng có SL ship; nếu SP có batch thì chọn batch trên dòng. | DO ở trạng thái cho phép Confirm; `warehouse_batch_id` được lưu, batch number/HSD do backend resolve. |                                                              |                               |
...
## Phụ lục D — Demo nhanh SO → DO → Invoice và PO → GRN

Mục tiêu: demo full flow theo nghiệp vụ hiện tại, tồn kho khớp với ship/inbound và có bằng chứng trên Stock Movements / Inventory.
...
- Env:
    ...
    - Chỉ bật một inbound canonical: `WAREHOUSE_INBOUND_FROM_PO_DELIVERED=true` hoặc `WAREHOUSE_INBOUND_FROM_DO_RECEIVED=true`
- Migration đã chạy: `php artisan migrate`
- Dữ liệu demo:
    - 1 kho active
    - 1 sản phẩm goods có batch/expiry và tồn > 0
    - 1 client, 1 vendor

### D.2. Demo luồng bán

1. Tạo Sales Order có 1 dòng hàng, ví dụ số lượng 3.
2. Tạo Sales Delivery Order từ SO.
3. Chọn kho, chọn batch theo dropdown, nhập ship qty hợp lệ.
4. Confirm DO để giữ chỗ.
5. Ship DO.
6. Tạo Invoice từ SO.
...
- DO chuyển `shipped`.
- Stock Movements có dòng outbound reference `SalesDo`.
- Inventory giảm đúng theo ship qty.
- Trong mode `shipment`, invoice sau ship không trừ tồn lần 2.

### D.3. Demo luồng mua

1. Tạo Purchase Order có `warehouse_id`.
2. Nhận hàng theo cấu hình canonical:
    - Nếu theo PO: đổi `delivery_status=delivered`.
    - Nếu theo GRN/DO nhập: tạo GRN/DO inbound và chuyển `received`.
3. Tạo Purchase Bill.

Kỳ vọng:

- Stock Movements có dòng inbound từ chứng từ mua.
- Inventory tăng đúng một lần theo sự kiện nhận hàng.
- Purchase Bill cập nhật AP nhưng không tự tạo movement kho mới.

### D.4. Lỗi thường gặp khi demo

- `Ship quantity cannot exceed remaining quantity`: qty ship lớn hơn remaining; giảm qty hoặc tạo thêm DO hợp lệ.
- `Please select a valid batch`: thiếu batch identity đầy đủ.
- Tồn trên màn hình chưa đổi: refresh table và đối chiếu Stock Movements.
...
| Mục đích                | File                                                                 |
| ----------------------- | -------------------------------------------------------------------- |
| Module Warehouse       | [`MODULE_WAREHOUSE.md`](MODULE_WAREHOUSE.md)                           |
| Index Sales/Fulfillment | [`INDEX.md`](INDEX.md) |
