...
| `Modules/Biometric/Routes/api.php`                                                        | `GET                                              | POST .../iclock/...`               | _(thiết bị / giao thức)_                                                            | **Không** REST; protocol kiểu máy chấm công. |
...
**Inbound Sales Order (AI):** `POST /ai-order-webhook/{hash}` nằm **`routes/web-public.php`**, **không** dùng prefix `/api` — xem [`AUDIT_AI_ORDER_INBOUND_SO_API_VI.md`](AUDIT_AI_ORDER_INBOUND_SO_API_VI.md).
...
- **REST + versioning + auth chuẩn B2B:** nhiều hệ thống dùng ` /api/v1/...` + Bearer / OAuth2 + OpenAPI — **repo có prefix `v1` ở vài module** nhưng **không** có bộ tài nguyên SO/Order công khai dưới `/api` (đã ghi trong `FUNC_IMPROVE/12_…`, `13_…`).
...
- Phương án REST/Sanctum tương lai: [`../FUNC_IMPROVE/12_AI_THIRDPARTY_SALES_ORDER_INTEGRATION_OPTIONS_VI.md`](../FUNC_IMPROVE/12_AI_THIRDPARTY_SALES_ORDER_INTEGRATION_OPTIONS_VI.md)
