...
| **Giao hàng / nhận (Purchase)**   | Phiếu nhận mua (GRN); phiếu giao bán (Sales DO)   | **`grns`**, **`grn_items`**; **`sales_dos`**, **`sales_do_items`** (bảng `delivery_orders*` / `sales_shipments*` legacy đã DROP trên env đã migrate — xem `ERP_SO_PO_DO_GRN_SCHEMA_AND_LEGACY_MATRIX_VI.md`) |
