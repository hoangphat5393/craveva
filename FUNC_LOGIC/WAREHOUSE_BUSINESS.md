...
**Quy trình tổng PO/DO/SO/Invoice/kho (đọc trước):** [`SALES_BUSINESS.md`](SALES_BUSINESS.md)  
**Runbook + kế hoạch nâng cấp (WUP):** [`WAREHOUSE_RUNBOOK_AND_UPGRADE_PLAN.md`](../FUNC_IMPROVE/04_WH_RUNBOOK_UPGRADE.md)  
**Checklist UAT E2E (Mua · Bán · Kho):** [`SALES_PURCHASE_WAREHOUSE_UAT_CHECKLIST.md`](SALES_PURCHASE_WAREHOUSE_UAT_CHECKLIST.md)  
...
### Mô hình cơ bản cần nhớ

| Luồng đơn giản | Ý nghĩa trong Craveva |
| -------------- | --------------------- |
| **SO → Sales DO → Ship → Invoice** | Đơn bán → phiếu giao bán → xuất kho thật → hóa đơn/công nợ. Ở mode `shipment`, invoice không trừ tồn thêm. |
| **PO → GRN/nhận hàng → Bill NCC** | Đặt mua → nhận hàng vào kho → hóa đơn/công nợ NCC. Bill không tự cộng kho. |
| **Transfer** | Chuyển hàng giữa hai kho; không thay thế PO/GRN hoặc Sales DO. |

Đa kho không đổi logic trên; chỉ thêm yêu cầu chứng từ phải gắn đúng `warehouse_id`, và tồn được lưu theo **kho + sản phẩm + lô/HSD nếu có**.
...
**Bật tính năng:** env **`WAREHOUSE_SALES_OUTBOUND_ENABLED=true`**, module Warehouse bật, user có module warehouse, chạy migration bảng posting. Chi tiết file & env: [`../docs/WAREHOUSE_PURCHASE_ENV_REFERENCE.md`](../docs/WAREHOUSE_PURCHASE_ENV_REFERENCE.md) · [`MODULE_WAREHOUSE.md`](MODULE_WAREHOUSE.md).
...
| “Đã chuẩn UAT report chưa?”         | **Chưa** cho đến khi QA chạy checklist và ký; xem [`WAREHOUSE_RUNBOOK_AND_UPGRADE_PLAN.md`](../FUNC_IMPROVE/04_WH_RUNBOOK_UPGRADE.md) và [`SALES_PURCHASE_WAREHOUSE_UAT_CHECKLIST.md`](SALES_PURCHASE_WAREHOUSE_UAT_CHECKLIST.md). |
