...
| Module bật cho user | User cần có quyền module **Purchase** (và phần mở rộng Warehouse nếu menu được gắn). |
...
2. Phù hợp **điều chỉnh**, **kiểm kê** theo quy trình Purchase — không thay thế hoàn toàn màn Warehouse nếu bạn chỉ quản lý “tồn vật lý” nhanh.
...
- Trường **kho mặc định** trên Client có thể **chưa có trên form** (tùy phase UI). Khi có, dùng để gợi ý kho giao hàng — **chưa bắt buộc** tự động nếu chưa tích hợp Order/DO.
...
1. Đăng nhập đúng user có quyền.
2. Thử **URL trực tiếp** theo bảng mục 1.
3. Nhờ IT: module Warehouse đã `enabled`, route cache `php artisan route:clear`.
