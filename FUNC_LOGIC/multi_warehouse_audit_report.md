...
| ------------------ | ---------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| Client             | Động theo tenant                         | Trùng với cột core (đã tài liệu hóa trong `WAREHOUSE_MASTER_GUIDE.md`) — nên dọn CF trùng.               |
...
2. **`InvoiceController`:** Khi **không** bật outbound warehouse, kiểm tra tồn vẫn có thể dùng `PurchaseStockAdjustment::sum` theo `product_id` — **không** phản ánh từng kho. Khi bật `WAREHOUSE_SALES_OUTBOUND_ENABLED`, nhánh kiểm tra dùng `WarehouseProductStock` (xem `WAREHOUSE_FLOW_VA_NGHIEP_VU_VI.md`).
...
2. Đồng bộ kỳ vọng BA/QA với code hiện tại: xem [`WAREHOUSE_FLOW_VA_NGHIEP_VU_VI.md`](WAREHOUSE_FLOW_VA_NGHIEP_VU_VI.md) (bán hàng Scope B) và [`WAREHOUSE_MASTER_GUIDE.md`](WAREHOUSE_MASTER_GUIDE.md) (kỹ thuật).
...
- Tài liệu liên quan trong repo: `FUNC_LOGIC/WAREHOUSE_MASTER_GUIDE.md`, `FUNC_LOGIC/MAOLIN_MASTER_GUIDE.md`.
...
## 9. Giai thich logic `Default Warehouse` trong Client (thuc te code)

### 9.1 Muc dich nghiep vu

- `client_details.default_warehouse_id` la kho mac dinh/uu tien gan voi khach hang.
- Dung de:
    - luu metadata phuc vu van hanh (CSKH, dieu phoi, doi soat);
    - map tu file import khach (neu co cột designated warehouse);
    - lam input cho cac buoc tu dong hoa tiep theo (nếu sau nay bat luong auto chon kho tren Order/Invoice/DO).

### 9.2 Luong quy trinh hien tai trong he thong

1. **UI Client Create/Edit** hien field `Default Warehouse` neu co module/bang kho.
2. **Request validation** nhan `default_warehouse_id` (`nullable|integer|exists:warehouses,id`).
3. **ClientController store/update** luu gia tri vao `client_details.default_warehouse_id`.
4. **Import Client**:
    - Neu file co `designated_warehouse_code` hoac `designated_warehouse_name`,
    - `ClientImportProcessor::resolveDefaultWarehouseId()` se tim kho theo `company_id` + `code`/`name`,
    - tim thay thi luu `default_warehouse_id`, khong tim thay thi de `null`.
5. **Downstream Order/Invoice (hien tai):**
    - Trong pham vi code da quet, chua thay luong bat buoc su dung `default_warehouse_id` de auto outbound stock.
    - Vi vay field nay dang o vai tro **data foundation + mapping import**, chua la rule tu dong xuat kho dau-cuoi.

### 9.3 Doi chieu voi file Miaolin trong `PROJECT MAOLIN/`

Da kiem tra `PROJECT MAOLIN/Miaolin Customer.xlsx` (sheet `Custom`), header hien co:

- Customer code, customer name, salesperson, department, tax id, phone, delivery address, payment terms...
- **Khong thay cot designated warehouse** (`designated_warehouse_code` / `designated_warehouse_name`).

Ket luan doi chieu:

- **Dung ve logic he thong:** he thong da san sang map default warehouse tu import.
- **Chua khop bo du lieu file hien tai:** file customer Miaolin trong `PROJECT MAOLIN/` chua co cot kho chi dinh, nen import se de `default_warehouse_id = null` cho cac dong nay (tru khi set tay tren form Client).

### 9.4 De xuat de khop nghiep vu Miaolin

- Neu Miaolin muon moi khach co kho uu tien, can bo sung it nhat 1 cot trong `customers.csv`:
    - `designated_warehouse_code` (uu tien), hoac
    - `designated_warehouse_name`.
- Quy uoc import:
    - Co code/name hop le -> map sang kho theo company.
    - Sai code/name -> de null + ghi log dong loi de doi soat.
- Neu chua the bo sung cot ngay, co the:
    - import nhu hien tai (khong kho mac dinh),
    - sau do cap nhat hang loat `default_warehouse_id` bang script/map bang tam.
