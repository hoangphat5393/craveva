...
| `backend_loc_per_file_full.txt` / `.csv`            | Same data and merge as the recommended view, kept as a separate full-reference copy.                                                   | 4,184 |
| `backend_loc_per_file_lp_by_filename.txt` / `.csv`  | Same data; backward-compatible alias of the recommended view.                                                                          | 4,184 |
...
| `backend_loc_per_file_no_languagepack.txt` / `.csv` | Same content as `_no_i18n` (file kept for backward compat).                                                                            | 3,858 |
| `backend_loc_per_module.txt` / `.csv`               | Rollup by top-level dir / each `Modules/*`.                                                                                            |    32 |

All files are sorted by line count descending.

> **Heads up — duplicate content**: `backend_loc_per_file.*`,
> `backend_loc_per_file_full.*`, and `backend_loc_per_file_lp_by_filename.*`
> contain the **same data** (different headers only).
> `backend_loc_per_file_no_i18n.*` and `backend_loc_per_file_no_languagepack.*`
> are also identical. Tell me if you want the duplicates removed.
