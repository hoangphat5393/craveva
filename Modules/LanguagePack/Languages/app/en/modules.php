...
return [
    ...
    'orders' => [
        ...
        'apiNoWebhookSecret' => 'No webhook secret is available yet for this company and no server-wide fallback is configured. Click “Generate new webhook secret” below, or ask an administrator to set AI_ORDER_WEBHOOK_SECRET in .env for a temporary global fallback.',
        'apiUsingLegacyGlobalSecretHint' => 'This page is showing the server-wide secret from .env (AI_ORDER_WEBHOOK_SECRET). For stronger isolation per company, click “Generate new webhook secret” — the URL and header will then be unique to this company.',
        'apiSecretRegenerated' => 'A new webhook secret was generated for this company. Update all external systems that call this webhook.',
        ...
        'apiDocHint' => 'Full JSON payload, fields, and curl examples: repository file FUNC_LOGIC/PM_READY_AI_WEBHOOK_STAGING_VI.md. REST routes and method toggles: docs/AI_ORDER_INTEGRATION_REST.md.',
        ...
        'apiFillAiStep2' => 'POST / webhook: use “POST URL (webhook)” for the legacy path, or POST {Base URL}/api/integrations/orders for REST create (same body rules).',
        'apiFillAiStep3' => 'Authentication: add header X-AI-Webhook-Secret with exactly the value shown under “HTTP header”.',
        'apiFillAiStep4' => 'JSON body: include company_id, client_code or client_id, items, etc. Exact names and examples: documentation links at the bottom.',
        ...
        'apiRingfenceIntro' => 'This company’s webhook secret ties traffic to this company. A server-wide .env secret is only a fallback until you generate a secret here.',
        ...
        'apiCurlCopyHint' => 'Copy the block below (**JSON** body, application/json). Use client_code **or** client_id (not both unless they match the same client). Replace YOUR_CLIENT_CODE or set client_id to a valid users.id for this company.',
        'apiCurlExampleMissingSecret' => 'Example curl appears after a webhook secret exists (generate one for this company, or configure AI_ORDER_WEBHOOK_SECRET in .env for a global fallback).',
        ...
        'apiRestPostmanHint' => 'Use “Copy template” for a block you can mirror in Postman: request line, Headers tab, and Body → raw → JSON (when shown). Replace YOUR_CLIENT_CODE and item_name with real values from your company.',
        ...
        'apiCrudPermissionsIntro' => 'Uncheck a verb to block it with HTTP 403 and code INTEGRATION_METHOD_DISABLED. Legacy POST /ai-order-webhook/{secret} respects “Create (POST)” here.',
