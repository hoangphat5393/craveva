...
return [
    ...
    'orders' => [
        ...
        'apiTab' => 'API',
        'apiIntegrationIntro' => 'Craveva ERP creates Sales Orders from one inbound HTTP POST (not REST CRUD /v1/orders). Copy the values below into your external system (AI, middleware, or any HTTP client). JSON body rules are in the linked doc.',
        ...
        'apiFillAiStep2' => 'POST / webhook: use “POST URL (webhook)” as the single outbound POST target — not separate CRUD paths like /orders unless your client maps them to this same URL.',
        'apiFillAiStep3' => 'Authentication: add header X-AI-Webhook-Secret with exactly the value shown under “HTTP header”.',
        'apiFillAiStep4' => 'JSON body: must include company_id (number below), client_code **or** client_id (at least one), items, etc. Follow PM_READY_AI_WEBHOOK_STAGING_VI.md — do not send random field names unless they match validation.',
        ...
        'apiCompanyIdHelp' => 'This number is your current workspace company id (database). It must appear as company_id in the JSON body. If you use a company-specific webhook secret, company_id must match this company or the server will reject the request. If you use multiple companies, switch company in the ERP header, reload this page, then copy again.',
        'apiRingfenceTitle' => 'One external integration = one ERP company',
        'apiRingfenceIntro' => 'Each company can have its own webhook secret (stored on this company). If you use that secret, requests are bound to this company. A legacy server-wide secret in .env is still accepted when this company has no secret yet.',
        'apiRingfenceBullet1' => 'Create a separate integration profile on the external side for each ERP company — do not reuse one profile with a different company_id by mistake.',
        'apiRingfenceBullet2' => 'Always select the correct ERP company (workspace) before copying the values on this page.',
        'apiRingfenceBullet3' => 'client_code or client_id must identify an active client on this company_id; if you send both, they must be the same client (the server validates this).',
        ...
        'apiCurlCopyHint' => 'Copy the block below; use client_code **or** client_id (not both unless they match the same client). Replace YOUR_CLIENT_CODE or set client_id to a valid users.id for this company.',
        ...
        'apiHttpCodesHint' => 'Typical responses: 201 success, 401 wrong secret/header, 422 validation or stock error (including company_id mismatch for a company-specific secret), duplicate: true when external_event_id was already processed.',
    ],
