    ...
    <x-form id="updateSalary" method="POST">
        ...
        <div class="form-body">
            <div class="row">
                ...
                <div class="col-lg-4">
                    <x-forms.number fieldId="amount" class="annualIncrementAmount" :fieldLabel="__('payroll::modules.payroll.annualIncrementAmount')" fieldName="annual_salary" fieldRequired="true">
                    </x-forms.number>
                </div>

                <div class="col-lg-4">
                    <x-forms.datepicker fieldId="date" class="incrementDate" :fieldLabel="__('payroll::modules.payroll.incrementDate')" fieldName="date" :fieldPlaceholder="__('placeholders.date')" :fieldValue="now($company->timezone)->format($company->date_format)" />
                </div>
...
<script>
    ...
    $('#update-salary').click(function() {
        window.apiHttp.postUrlEncoded("{{ route('employee-salary.increment-store', $employee->id) }}", $('#updateSalary').serialize())
