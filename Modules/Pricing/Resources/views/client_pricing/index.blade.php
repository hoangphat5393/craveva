...
@section('content')
    <div class="content-wrapper">
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if (in_array('admin', user_roles()) || user()->permission('add_client_pricing') == 'all' || user()->permission('add_client_pricing') == 'added')
                    <x-forms.link-primary :link="route('pricing.client_pricing.create')" class="mr-3 openRightModal float-left" icon="plus">
        ...
        <div class="d-flex flex-column w-tables rounded mt-3 bg-white table-responsive">
            <table class="table table-hover border-0 w-100" id="client-pricing-table">
                <thead>
                    <tr>
                        <th>
                        ...
                        <th>@lang('pricing::app.discountValue')</th>
                ...
                <tbody>
                    @foreach ($pricings as $row)
                        <tr id="row-{{ $row->id }}">
                                ...
                                <div class="media align-items-center">
                                    <div class="media-body">
                                        <h5 class="mb-0 f-13 text-darkest-grey">
                                            <a href="{{ route('clients.show', $row->client_id) }}">{{ $row->client?->name ?? ($row->client?->clientDetails?->company_name ?? '--') }}</a>
                                        </h5>
                                        <p class="mb-0 f-12 text-dark-grey">{{ $row->client?->email }}</p>
                                        @if ($row->client?->clientDetails?->client_code)
                                            <p class="mb-0 f-12 text-dark-grey">Code: {{ $row->client->clientDetails->client_code }}</p>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td data-order="{{ $row->start_date?->format('Y-m-d') ?? '' }}">{{ $row->start_date ? $row->start_date->format(company()->date_format) : '--' }}</td>
                            <td data-order="{{ $row->end_date?->format('Y-m-d') ?? '' }}">{{ $row->end_date ? $row->end_date->format(company()->date_format) : '--' }}</td>
                            <td>
                                ...
                                @if (in_array('admin', user_roles()) || user()->permission('add_client_pricing') == 'all' || user()->permission('add_client_pricing') == 'added' || user()->permission('edit_client_pricing') != 'none')
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actionDropdown{{ $row->id }}">
                                            <a class="dropdown-item openRightModal" href="{{ route('pricing.client_pricing.edit', $row->id) }}">
                                                <i class="fa fa-edit mr-2"></i> @lang('app.edit')
                                            </a>
                                            <a href="javascript:;" class="dropdown-item delete-client-pricing" data-row-id="{{ $row->id }}">
...
@push('scripts')
    ...
    <script>
        $(function() {
            $('#client-pricing-table').DataTable({
            ...
            const applyQuickAction = () => {
                var rowdIds = $("#client-pricing-table input:checkbox:checked").map(function() {
                    return $(this).val();
                }).get();

                var url = "{{ route('pricing.client_pricing.apply_quick_action') }}?row_ids=" + rowdIds;
        ...
        $('body').on('click', '.delete-client-pricing', function() {
            ...
            var url = "{{ route('pricing.client_pricing.destroy', ':id') }}";
