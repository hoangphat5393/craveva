...
- **Parameters**:
    ...
    - `end_date` (optional, date): End date of the pricing (Format: Company Date Format, must be after or equal to start_date).
- **Validation**:
    ...
    - If `end_date` is omitted, the system stores an open-ended range (current implementation uses a far-future default).
...
- **Date Fields**: `start_date` is required; `end_date` is optional.
