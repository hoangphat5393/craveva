...
class SalesDoMigrateRollbackCommand extends Command
{
    protected $signature = 'purchase:sales-do-migrate-rollback
        {--manifest= : Manifest JSON path from purchase:sales-do-migrate-data execute run}
        {--execute : Execute rollback (default is dry-run)}
        {--force : Required with --execute to avoid accidental run}';

    protected $description = 'Rollback for sales-do migration by manifest (dry-run by default)';

    public function handle(): int
    {
        if (! Schema::hasTable('sales_dos') || ! Schema::hasTable('sales_do_items')) {
            $this->error('Required tables not found: sales_dos and/or sales_do_items.');
        ...
        $summary = [
            'command' => 'purchase:sales-do-migrate-rollback',
            ...
            'exists_now' => [
                'headers_count' => (int) DB::table('sales_dos')->whereIn('id', $headerIds ?: [0])->count(),
                'items_count' => (int) DB::table('sales_do_items')->whereIn('id', $itemIds ?: [0])->count(),
            ],
        ];
        ...
        try {
            DB::transaction(function () use ($headerIds, $itemIds): void {
                if ($itemIds !== []) {
                    DB::table('sales_do_items')->whereIn('id', $itemIds)->delete();
                }

                if ($headerIds !== []) {
                    DB::table('sales_dos')->whereIn('id', $headerIds)->delete();
                }
            });
        } catch (Throwable $e) {
