...
class PurchaseInventoryDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        // 3. Warehouse name + code (same pattern as Sales DO: "Name (CODE)")
    ...
    public function query(PurchaseInventory $model)
    {
        ...
        if ($request->warehouseId != 'all' && ! is_null($request->warehouseId) && $request->warehouseId !== '') {
            $model->where('purchase_stock_adjustments.warehouse_id', '=', (int) $request->warehouseId);
        }
