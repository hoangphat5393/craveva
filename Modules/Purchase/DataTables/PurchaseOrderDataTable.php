...
class PurchaseOrderDataTable extends BaseDataTable
{
    ...
    public function query(PurchaseOrder $model)
    {
        ...
        $model = $model->select(
            'purchase_vendors.primary_name',
            'purchase_vendors.currency_id',
            'purchase_orders.purchase_date',
            'purchase_orders.billed_status',
            ...
            'purchase_orders.send_status'
        )
            ->join('purchase_vendors', 'purchase_vendors.id', 'purchase_orders.vendor_id');

        if ($request->searchText != '') {
            $safeSearchText = trim((string) request('searchText'));
            $model = $model->where(function ($query) use ($safeSearchText) {
                $query->where('purchase_orders.purchase_order_number', 'like', '%' . $safeSearchText . '%')
                    ->orWhere('purchase_vendors.primary_name', 'like', '%' . $safeSearchText . '%');
            });
        }

        if ($request->startDate !== null && $request->startDate != 'null' && $request->startDate != '') {
            $startDate = Carbon::createFromFormat($this->company->date_format, $request->startDate)->toDateString();
            $model = $model->where('purchase_orders.purchase_date', '>=', $startDate);
        }

        if ($request->endDate !== null && $request->endDate != 'null' && $request->endDate != '') {
            $endDate = Carbon::createFromFormat($this->company->date_format, $request->endDate)->toDateString();
            $model = $model->where('purchase_orders.purchase_date', '<=', $endDate);
        }
