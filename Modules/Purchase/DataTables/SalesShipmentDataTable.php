        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($canShip && $row->status === 'shipped') {
                    $action .= '<a class="dropdown-item f-14 text-dark sales-shipment-deliver" data-id="'.$row->id.'" href="javascript:;"><i class="fa fa-box mr-2"></i>'.trans('purchase::modules.salesShipment.markDelivered').'</a>';
                }
                ...
                if (
                    $canAddInvoice
                    && in_array($row->status, ['shipped', 'delivered'], true)
                    && (int) ($row->order_id ?? 0) > 0
            ...
            ->editColumn('status', fn ($row) => SalesDoStatusBadge::html((string) $row->status))
            ->addIndexColumn()
