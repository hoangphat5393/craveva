...
class PurchaseInventoryController extends AccountBaseController
{
    ...
    public function show($id)
    {
        ...
        if ($this->inventory->getCustomFieldGroupsWithFields()) {
            $this->fields = $this->inventory->getCustomFieldGroupsWithFields()->fields;
        }

        // Populate custom fields data for the view
        $this->inventory->withCustomFields();
    ...
    public function adjustInventory(Request $request)
    {
        ...
        $this->stockAdjustment = PurchaseStockAdjustment::where('product_id', $request->id)
            ->when($warehouseId, function ($q) use ($warehouseId) {
                return $q->where('warehouse_id', $warehouseId);
            })
            ->first();

        // If specific warehouse is selected, try to get exact stock from WarehouseProductStock
        if ($warehouseId) {
            $whStock = WarehouseProductStock::where('warehouse_id', $warehouseId)
    ...
    {
        $rvalue = $this->importFileProcess($request, InventoryImport::class);
    ...
    protected function deleteRecords($request)
    {
        ...
        foreach ($inventories as $inventory) {
            if ($deletePermission == 'all' || ($deletePermission == 'added' && $inventory->added_by == user()->id)) {
                $inventory->files()->each(function ($file) {
                    $file->delete();
                });

                $inventory->stocks()->each(function ($stock) {
                    $stock->delete();
                });
