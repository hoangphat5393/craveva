...
class SalesShipmentController extends AccountBaseController
{
    ...
    public function shipForm($id)
    {
        ...
        $this->shipment = $this->queryByCompany()
            ->with(['items.orderItem.product', 'order.items.product', 'order.items.unit', 'order.clientdetails', 'warehouse'])
            ->findOrFail($id);
        abort_if(! in_array($this->shipment->status, ['draft', 'confirmed'], true), 403);

        $this->pageTitle = __('purchase::app.shipDeliveryOrder');
        $this->order = $this->shipment->order;
        abort_if(! $this->order, 404);

        $this->remainingByItem = $this->remainingQtyByOrderItem($this->order->id, $this->shipment->id);
        $this->batchOptionsByOrderItem = $this->buildBatchOptionsByOrderItem($this->order, (int) $this->shipment->warehouse_id, $this->shipment);

        if (request()->ajax()) {
            $html = view('purchase::sales-shipment.ajax.ship', $this->data)->render();

            return Reply::dataOnly(['status' => 'success', 'html' => $html, 'title' => $this->pageTitle]);
        }

        $this->view = 'purchase::sales-shipment.ajax.ship';
    ...
    public function ship(Request $request, $id, SalesDoService $salesDoService)
    {
        abort_403(! FlowPermission::allowsAlias('sales_do.ship'));
        $shipment = $this->queryByCompany()->with('items')->findOrFail($id);
        ...
        if ($request->has('order_item_id')) {
            $payload = $this->validateForm($request, $shipment->id);
            $payload['shipment_number'] = $payload['shipment_number'] ?: $shipment->shipment_number;
            $shipment = app(SalesDoService::class)->update($shipment, $payload, user()->id)->fresh(['items']);
        }

        $error = $salesDoService->ship($shipment);
        if ($error) {
            if ($error === 'messages.salesDoShipQuantityRequired') {
                return Reply::error(__($error), 'sales_do_ship_quantity_required', [
                    'editUrl' => route($this->salesDoRouteName('edit'), $shipment->id),
                    'shipUrl' => route($this->salesDoRouteName('ship-form'), $shipment->id),
                ]);
            }
    ...
    protected function validateForm(Request $request, ?int $shipmentId = null): array
    {
        ...
        foreach ($validated['order_item_id'] as $idx => $orderItemId) {
            ...
            if ($requestQty > $left) {
                abort(422, __('messages.salesDoShipQtyExceedsRemaining', [
                    'remaining' => number_format($left, 2, '.', ''),
                    'requested' => number_format($requestQty, 2, '.', ''),
                ]));
            }
            ...
            if ($requestQty > 0 && $requiresBatch && $batchId <= 0) {
                abort(422, __('messages.salesDoBatchRequired'));
            }

            if ($requestQty > 0 && $batchId > 0) {
                $batch = $batchMap->get($batchId);
                if (! $batch) {
                    abort(422, __('messages.salesDoBatchNotFoundForCompany'));
                }

                if ((int) $batch->warehouse_id !== (int) $validated['warehouse_id']) {
                    abort(422, __('messages.salesDoBatchWrongWarehouse'));
                }

                if (! empty($lineProductId) && (int) $batch->product_id !== (int) $lineProductId) {
                    abort(422, __('messages.salesDoBatchWrongProduct'));
                }

                $availableQty = max(0, (float) $batch->quantity - (float) $batch->reserved_quantity);
                if ($requestQty > $availableQty) {
                    abort(422, __('messages.salesDoBatchQtyExceedsAvailable', [
                        'available' => number_format($availableQty, 4, '.', ''),
                        'requested' => number_format($requestQty, 4, '.', ''),
                    ]));
                }
                ...
                $validated['warehouse_batch_id'][$idx] = null;
                $validated['batch_number'][$idx] = null;
                $validated['expiration_date'][$idx] = null;
            }
        }
