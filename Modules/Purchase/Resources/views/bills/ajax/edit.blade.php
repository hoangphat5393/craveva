    ...
    <x-form class="c-inv-form" id="save-bill-data-form">
        ...
        <div class="row px-lg-4 px-md-4 px-3 pt-3">
            ...
            <div class="col-md-4">
                <div class="form-group mb-lg-0 mb-md-0 mb-4">
                    <x-forms.label class="mb-12" fieldId="bill_number" :fieldLabel="__('purchase::app.menu.billNumber')" fieldRequired="true">
                    </x-forms.label>
                    <x-forms.input-group>
                        <x-slot name="prepend">
                            <span class="input-group-text">{{ $purchaseSetting->bill_prefix }}{{ $purchaseSetting->bill_number_separator }}{{ $zero }}
                            </span>
                        </x-slot>
                        <input readOnly type="number" name="bill_number" id="bill_number" class="form-control height-35 f-15" value="{{ $purchaseBill->original_bill_number }}">
                    </x-forms.input-group>
                </div>
            </div>
            ...
            <div class="col-md-4">
                ...
                <input type="text" readOnly name="vendor_name" id="vendor_id" class="form-control height-35 f-15" value="{{ $purchaseBill->purchaseVendor->primary_name }}">
            </div>
            <!-- SELECT VENDOR -->
            ...
            <div class="col-md-4">
                <div class="form-group mb-lg-0 mb-md-0 mb-4">
                    ...
                    <div class="input-group">
                        <input type="text" id="bill_date" name="issue_date" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="@lang('placeholders.date')" value="{{ $purchaseBill->bill_date->translatedFormat(company()->date_format) }}">
                    </div>
                </div>
            </div>
            <!-- BILL DATE END -->
        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
            <div class="col-md-3">
                ...
                <div class="form-group c-inv-select mb-2">
                    <input readOnly type="text" id="purchase_order" name="purchase_order" class="px-6 positioissue_daten-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" value="{{ $purchaseBill->order->purchase_order_number }}">
                </div>
            </div>
        </div>
                        ...
                        <table width="100%">
                            <tbody>
                                <tr class="text-dark-grey font-weight-bold f-14">
                                    <td width="{{ $item->hsn_sac_code_show ? '40%' : '50%' }}" class="border-0 inv-desc-mbl btlr">@lang('app.description')</td>
                                ...
                                <tr>
                                    <td class="border-bottom-0 btrr-mbl btlr">
                                        <input type="text" class="form-control f-14 border-0 w-100 item_name" name="item_name[]" placeholder="@lang('modules.expenses.itemName')" value="{{ $item->item_name }}" readonly>
                                    </td>
                                    <td class="border-bottom-0 d-block d-lg-none d-md-none">
                                        <textarea class="form-control f-14 border-0 w-100 mobile-description" name="item_summary[]" placeholder="@lang('placeholders.invoices.description')">{{ strip_tags($item->item_summary) }}</textarea>
                                    </td>
                                    @if ($item->hsn_sac_code_show)
                                        <td class="border-bottom-0">
                                            <input type="text" min="1" class="form-control f-14 border-0 w-100 text-right hsn_sac_code" data-item-id="{{ $item->id }}" value="{{ $item->hsn_sac_code }}" name="hsn_sac_code[]" readonly>
                                        </td>
                                    @endif
                                    <td class="border-bottom-0">
                                        <input type="number" min="1" class="form-control f-14 border-0 w-100 text-right quantity mt-3" data-item-id="{{ $item->id }}" value="{{ $item->quantity }}" name="quantity[]" readonly>
                                        <span class="text-dark-grey float-right border-0 f-12">{{ $item->unit->unit_type }}</span>
                                    </td>
                                    <td class="border-bottom-0">
                                        <input type="number" min="1" class="f-14 border-0 w-100 text-right cost_per_item form-control" data-item-id="{{ $item->id }}" placeholder="{{ $item->unit_price }}" value="{{ $item->unit_price }}" name="cost_per_item[]" readonly>
                                    </td>
                                    <td class="border-bottom-0">
                                        @foreach ($item->taxes as $tax)
                                            <span data-rate="{{ $tax->rate_percent }}" data-tax-text="{{ strtoupper($tax->tax_name) . ':' . $tax->rate_percent }}%">{{ $tax->tax_name . ': ' . $tax->rate_percent . '%' }}</span><br>
                                        @endforeach
                                    </td>
                                    <td rowspan="2" align="right" valign="top" class="bg-amt-grey btrr-bbrr">
                                        <span class="amount-html" data-item-id="{{ $item->id }}">0.00</span>
                                        <input type="hidden" class="amount" name="amount[]" data-item-id="{{ $item->id }}" value="0">
                                    </td>
                                </tr>
                                <tr class="d-none d-md-table-row d-lg-table-row">
                                    <td colspan="{{ $item->hsn_sac_code_show ? '4' : '3' }}" class="dash-border-top bblr">
                                        <textarea class="form-control f-14 border-0 w-100 desktop-description" name="item_summary[]" placeholder="@lang('placeholders.invoices.description')" readonly>{{ strip_tags($item->item_summary) }}</textarea>
                                    </td>
                                    @if (isset($item->purchaseItemImage->file_url))
                                        <td class="border-left-0">
                                            <input type="file" class="dropify" id="dropify" name="order_item_image[]" data-allowed-file-extensions="png jpg jpeg" data-messages-default="test" data-height="70" data-default-file="{{ $item->purchaseItemImage->file_url }}" />
                                ...
                                <table width="100%">
                                    <tbody>
                                        ...
                                        <tr>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td width="70%" class="c-inv-sub-padding">
                                                                <input readonly type="number" min="0" name="discount_value" class="form-control f-14 border-0 w-100 text-right discount_value" placeholder="0" value="{{ isset($purchaseBill) ? $purchaseBill->discount : '0' }}">
                                                            </td>
                                                            <td width="30%" align="left" class="c-inv-sub-padding">
                                                                <input type="text" class="form-control f-14 border-0 w-100 text-right discount_value" value="{{ $item->purchaseOrder->discount_type == 'percent' ? '%' : 'Amt.' }}" readonly>
                                            ...
                                            <td><span id="discount_amount">{{ isset($purchaseBill) ? number_format((float) $purchaseBill->discount, 2, '.', '') : '0.00' }}</span>
                                            </td>
                                        </tr>
                                        <tr>
        ...
        <div class="d-flex flex-wrap px-lg-4 px-md-4 px-3 py-3">
            <div class="col-md-6 col-sm-12 c-inv-note-terms p-0 mb-lg-0 mb-md-0 mb-3">
                ...
                <textarea class="form-control" name="note" id="note" rows="4" placeholder="@lang('placeholders.invoices.note')">{{ $purchaseBill->note }}</textarea>
            </div>
        </div>
                ...
                <div class="inv-action dropup mr-3">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuBtn" tabindex="0">
                        ...
                        @else
                            ...
                            <li>
                                <a class="dropdown-item f-14 text-dark save-form" href="javascript:void(0);" data-type="open">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.save-form').click(function() {
            ...
            $.easyBlockUI('#save-bill-data-form');
            window.apiHttp.postUrlEncoded(url, $('#save-bill-data-form').serialize())
                .then(function(response) {
                    if (response.status === 'success') {
                        window.location.href = response.redirectUrl || "{{ route('bills.index') }}";
                    }
                })
                .catch(function(err) {
                ...
                .finally(function() {
                    $.easyUnblockUI('#save-bill-data-form');
                });
        });
