<div class="d-flex c-inv-desc item-row">
        ...
        <table width="100%">
            <tbody>
                <tr class="text-dark-grey font-weight-bold f-14">
                    <td width="{{ $invoiceSetting->hsn_sac_code_show ? '40%' : '50%' }}" class="border-0 inv-desc-mbl btlr text-left">@lang('app.description')
                    </td>
                ...
                <tr>
                    ...
                    @endif
                    <td class="border-bottom-0">
                        <input type="number" min="1" class="f-14 border-0 w-100 text-right quantity mt-3 bg-additional-grey" value="{{ $item->price }}" name="current_value[]" id="current_value{{ $item->id }}" readonly>
                    ...
                    <td class="border-bottom-0">
                        <input type="text" class="form-control height-35 f-14 border-0 w-100 text-right bg-additional-grey" name="adjusted_value[]" id="adjusted_value{{ $item->id }}" readonly placeholder="@lang('purchase::placeholders.adjustedValue')">
                    </td>
                </tr>
                <tr class="d-md-block d-lg-table-row">
                    ...
                    <td class="border-left-0">
                        @if ($item->image_url != '')
                            <input type="file" class="dropify" disabled name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg pdf xls" data-messages-default="test" data-height="70" data-default-file="{{ $item->image_url }}" data-show-remove="false" />
                        @endif
                    </td>
                </tr>
                <!-- manufacturing_date & expiration_date removed per request -->
            </tbody>
        </table>
    ...
    <script>
        $(function() {
            ...
            datepicker('.date-picker-inventory', {
                position: 'bl',
                ...datepickerConfig
            });
