    ...
    <x-form class="c-inv-form" id="saveOrderForm">
        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
                ...
                <div class="form-group mb-4">
                    ...
                    <div class="input-group">
                        <div class="input-group-prepend  height-35 ">
                            <span class="input-group-text border-grey f-15 bg-additional-grey px-3 text-dark" id="basic-addon1">{{ $purchaseSetting->purchase_order_prefix }}{{ $purchaseSetting->purchase_order_number_separator }}{{ $zero }}</span>
                        </div>
                        <input type="text" name="purchase_order_number" id="purchase_order_number" class="form-control height-35 f-15" value="@if (is_null($lastOrder)) 1 @else{{ $lastOrder }} @endif" placeholder="0019" aria-label="0019" aria-describedby="basic-addon1">
                    </div>
                </div>
            </div>
            <!-- ORDER NUMBER END -->
            ...
            <div class="col-md-6 col-lg-4 mt-3">
                ...
                <input type="number" id="exchange_rate" name="exchange_rate" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" value="{{ $companyCurrency->exchange_rate }}" readonly>
            ...
            <div class="col-md-6 col-lg-4 mt-3">
                <div class="form-group mb-4">
                    ...
                    <div class="input-group">
                        <input type="text" id="purchase_date" name="purchase_date" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="@lang('placeholders.date')" value="{{ now(company()->timezone)->translatedFormat(company()->date_format) }}">
                    </div>
                </div>
            </div>
                ...
                <div class="form-group c-inv-select mb-4">
                    <x-forms.input-group>
                        ...
                        @if ($addProductPermission == 'all' || $addProductPermission == 'added')
                            <x-slot name="append">
                                <a href="{{ route('purchase-products.create') }}" data-redirect-url="{{ url()->full() }}" class="btn btn-outline-secondary border-grey openRightModal" data-toggle="tooltip" data-original-title="{{ __('app.add') . ' ' . __('modules.dashboard.newproduct') }}">@lang('app.add')</a>
                            </x-slot>
                        @endif
                    </x-forms.input-group>
                </div>
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td width="70%" class="c-inv-sub-padding">
                                                            <input type="number" min="0" name="discount_value" class="form-control f-14 border-0 w-100 text-right discount_value" placeholder="0" </td>
...
<script>
    $(document).ready(function() {
        ...
        $('#add-products').on('changed.bs.select', function(e, clickedIndex, isSelected, previousValue) {
            ...
            var existingRow = $('#sortable').find('input[name="product_id[]"][value="' + id + '"]').closest('.item-row');
            ...
            if (existingRow.length > 0) {
                ...
                var perItemCost = parseFloat(existingRow.find('.cost_per_item').val() || 0);
                var amount = newQty * perItemCost;

                existingRow.find('.amount').val(decimalupto2(amount));
                existingRow.find('.amount-html').html(decimalupto2(amount));

                calculateTotal();
            } else if (previousValue != id) {
                addProduct(id);
            }
