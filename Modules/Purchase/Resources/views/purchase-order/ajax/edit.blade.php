        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
            ...
            <div class="col-md-6 col-lg-4">
                <div class="form-group mb-lg-0 mb-md-0 mb-4">
                    <x-forms.label class="mb-12" fieldId="purchase_order_number" :fieldLabel="__('app.orderNumber')" fieldRequired="true">
                    </x-forms.label>
                    <x-forms.input-group>
                        <x-slot name="prepend">
                            <span class="input-group-text border-grey f-15 bg-additional-grey px-3 text-dark" id="basic-addon1">{{ $purchaseSetting->purchase_order_prefix }}{{ $purchaseSetting->purchase_order_number_separator }}</span>
                        </x-slot>
                        <input type="number" name="purchase_order_number" id="purchase_order_number" class="form-control height-35 f-15" value="{{ $order->original_order_number }}" readonly>
                    </x-forms.input-group>
                </div>
            </div>
            <!-- INVOICE NUMBER END -->
            ...
            <div class="col-md-6 col-lg-4 mt-3">
                ...
                <input type="number" id="exchange_rate" name="exchange_rate" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" value="{{ $order->exchange_rate }}" @if ($companyCurrency->id == $order->currency_id) readonly @endif>
            ...
            <div class="col-md-6 col-lg-4 mt-3">
                <div class="form-group mb-4">
                    ...
                    <div class="input-group">
                        <input type="text" id="purchase_date" name="purchase_date" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="@lang('placeholders.date')" value="{{ $order->purchase_date->translatedFormat(company()->date_format) }}">
                    </div>
                </div>
            </div>
            <!-- PURCHASE DATE END -->
            ...
            <div class="col-md-6 col-lg-4 mt-3">
                <div class="form-group mb-4">
                    ...
                    <div class="input-group">
                        <input type="text" id="expected_date" name="expected_date" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="@lang('placeholders.date')" value="{{ $order->expected_delivery_date->translatedFormat(company()->date_format) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4 mt-3">
                <div class="form-group c-inv-select mb-4">
                    ...
                    <div class="select-others height-35 rounded">
                        <select class="form-control select-picker" data-live-search="true" data-size="8" name="delivery_status" id="delivery_status">
                            <option data-content="<i class='fa fa-circle mr-2 text-dark'></i> @lang('purchase::modules.purchaseOrder.notStarted')" value="not_started" @if ($order->delivery_status == 'not_started')
                                selected
                                @endif></option>
                            <option data-content="<i class='fa fa-circle mr-2 text-yellow'></i> @lang('purchase::modules.purchaseOrder.inTransaction')" value="in_transaction" @if ($order->delivery_status == 'in_transaction')
                                selected
                                @endif></option>
                            <option data-content="<i class='fa fa-circle mr-1 f-15 text-red'></i> @lang('purchase::modules.purchaseOrder.deliveryFailed')" value="delivery_failed" @if ($order->delivery_status == 'delivery_failed')
                                selected
                                @endif></option>
                            <option data-content="<i class='fa fa-circle mr-1 f-15 text-light-green'></i> @lang('purchase::modules.purchaseOrder.delivered')"value="delivered" @if ($order->delivery_status == 'delivered')
                                selected
                                @endif></option>
                        </select>
                    </div>
                ...
                <div class="form-group c-inv-select mb-4">
                    <x-forms.input-group>
                        ...
                        @if ($addProductPermission == 'all' || $addProductPermission == 'added')
                            <x-slot name="append">
                                <a href="{{ route('products.create') }}" data-redirect-url="{{ url()->full() }}" class="btn btn-outline-secondary border-grey openRightModal" data-toggle="tooltip" data-original-title="{{ __('app.add') . ' ' . __('modules.dashboard.newproduct') }}">@lang('app.add')</a>
                            </x-slot>
                        @endif
                    </x-forms.input-group>
                </div>
                            ...
                            <tbody>
                                        ...
                                        <div class="select-others height-35 rounded border-0">
                                            <select id="multiselect{{ $key }}" name="taxes[{{ $key }}][]" multiple="multiple" class="select-picker type customSequence border-0" data-size="3">
                                                ...
                                                @foreach ($taxes as $tax)
                                                    <option data-rate="{{ $tax->rate_percent }}" data-tax-text="{{ strtoupper($tax->tax_name) . ':' . $tax->rate_percent }}%" @if (isset($item->itemTaxes) && in_array($tax->id, $taxArray) !== false) selected @endif value="{{ $tax->id }}">
                                ...
                                <tr class="d-none d-md-block d-lg-table-row">
                                    ...
                                    <td class="border-left-0">
                                        <input type="file" class="dropify" name="order_item_image[]" data-allowed-file-extensions="png jpg jpeg" data-messages-default="test" data-height="70" data-id="{{ $item->id }}" id="{{ $item->id }}" data-default-file="{{ $item->purchaseItemImage ? $item->purchaseItemImage->file_url : '' }}"
                                            @if ($item->purchaseItemImage && $item->purchaseItemImage->external_link) data-show-remove="false" @endif />
                            ...
                            <table width="100%">
                                <tbody>
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td width="70%" class="c-inv-sub-padding">
                                                            <input type="number" min="0" name="discount_value" class="form-control f-14 border-0 w-100 text-right discount_value" placeholder="0" value="{{ $order->discount }}">
                                                        </td>
                                    ...
                                    <tr class="bg-amt-grey f-16 f-w-500">
                                        ...
                                        <td>
                                            <span class="total-currency mr-1">
                                                {{ $order->currency ? $order->currency->currency_code : (companyOrGlobalSetting()->currency ? companyOrGlobalSetting()->currency->currency_code : '') }}
                                            </span>
                                            <span class="total">{{ number_format((float) $order->total, 2, '.', '') }}</span>
                                        </td>
...
<script>
    $(document).ready(function() {
        ...
        $('.save-form').click(function() {
            ...
            $.easyBlockUI('#saveOrderForm');
            window.apiHttp.postUrlEncoded("{{ route('purchase-order.update', $order->id) }}" + "?type=" + type, updateBody).then(function(response) {
