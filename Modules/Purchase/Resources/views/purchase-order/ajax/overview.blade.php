        ...
        <div class="invoice-table-wrapper">
            <table width="100%">
                <tr class="inv-logo-heading">
                    <td><img src="{{ invoice_setting()->logo_url }}" alt="{{ mb_ucwords(company()->company_name) }}" id="logo" /></td>
                                        ...
                                        <tr class="text-dark f-12">
                                            <td colspan="{{ $invoiceSetting->hsn_sac_code_show ? '6' : '5' }}" class="border-bottom-0">
                                                ...
                                                @if ($item->purchaseItemImage)
                                                    <p class="mt-2">
                                                        <a href="javascript:;" class="img-lightbox" data-image-url="{{ $item->purchaseItemImage->file_url }}">
            ...
            <div class="inv-action mr-3 mr-lg-3 mr-md-3 dropup">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" tabindex="0">
                    ...
                    @if ($order->delivery_status != 'delivered')
                        <li>
                            <a class="dropdown-item f-14 text-dark" href="{{ route('purchase-order.edit', [$order->id]) }}">
                    ...
                    @if ($order->delivery_status != 'delivered')
                        <li>
                            <a class="dropdown-item f-14 text-dark delete-order" href="javascript:;" data-order-id="{{ $order->id }}">
                                <i class="fa fa-trash f-w-500 mr-2 f-11"></i> @lang('app.delete')
                            </a>
                        </li>
                    @endif
                    <li>
                        <a class="dropdown-item f-14 text-dark" href="{{ route('purchase_order.download', [$order->id]) }}">
                    ...
                    @if ($order->purchase_status != 'canceled')
                        <li>
                            <a class="dropdown-item f-14 text-dark sendButton" href="javascript:;" data-invoice-id="{{ $order->id }}" data-type="send">
                                <i class="fa fa-paper-plane f-w-500 mr-2 f-11"></i> @lang('app.send')
                            </a>
                        </li>
                        @if ($order->send_status == 0)
                            <li>
                                <a class="dropdown-item f-14 text-dark sendButton" href="javascript:;" data-toggle="tooltip" data-original-title="@lang('messages.markSentInfo')" data-invoice-id="{{ $order->id }}" data-type="mark_as_send">
...
<script>
    ...
    $('body').on('click', '.sendButton', function() {
        ...
        var ovSendBody = '_token=' + encodeURIComponent(token) +
            '&data_type=' + encodeURIComponent(type) +
            '&type=send';
        window.apiHttp.postUrlEncoded(url, ovSendBody).then(function(response) {
