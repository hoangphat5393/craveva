...
if (!Number.isFinite(factor) || factor <= 0) { fieldErrors.push({ selector: $row.find('.unit-conversion-factor'), message: purchaseProductValidationMessages.unitConversionFactorInvalid, }); } }); } function purchaseProductShowClientInvalidToast() { if (typeof Swal==='undefined' ) { return; } Swal.fire({ icon: 'error' , text: purchaseProductValidationMessages.invalidDataToast, toast: true,
    ...
    fieldErrors.push({ selector: '#selling_price' , message: purchaseProductValidationMessages.sellingPriceRequired, }); } if ($form.find('#purchase_information').prop('checked')) { const purchaseRaw=($form.find('#purchase_price').val() || '' ).toString().trim(); const purchasePrice=purchaseRaw==='' ? NaN : parseFloat(purchaseRaw); if (!Number.isFinite(purchasePrice)) { fieldErrors.push({
    selector: '#purchase_price' , message: purchaseProductValidationMessages.purchasePriceRequired, }); } } if ($form.find('#track_inventory').prop('checked')) { const stockRaw=($form.find('#opening_stock').val() || '' ).toString().trim(); const openingStock=stockRaw==='' ? NaN : parseFloat(stockRaw); if (!Number.isFinite(openingStock)) { fieldErrors.push({ selector: '#opening_stock' , message:
