...
(function() {
    ...
    function purchaseProductValidateUnitConversions($form, fieldErrors) {
        ...
        $form.find('#product-unit-conversions-body tr').each(function() {
            ...
            if (!Number.isFinite(factor) || factor <= 0) {
                fieldErrors.push({
                    selector: $row.find('.unit-conversion-factor'),
                    message: purchaseProductValidationMessages.unitConversionFactorInvalid,
                });
            }
        });
    }

    function purchaseProductShowClientInvalidToast() {
        ...
        Swal.fire({
            icon: 'error',
            text: purchaseProductValidationMessages.invalidDataToast,
            toast: true,
            position: 'top-end',
            timer: 4500,
    ...
    window.validatePurchaseProductForm = function(formSelector) {
        const $form = $(formSelector);
        if (!$form.length) {
            return true;
        }

        purchaseProductClearFieldErrors($form);

        const fieldErrors = [];
        const productType = ($form.find('#type').val() || '').toString();
        const isService = productType === 'service';

        const name = ($form.find('#name').val() || '').toString().trim();
        if (name === '') {
            fieldErrors.push({
                selector: '#name',
                message: purchaseProductValidationMessages.nameRequired,
            });
        }

        if (!isService) {
            const sellingRaw = ($form.find('#selling_price').val() || '').toString().trim();
            const sellingPrice = sellingRaw === '' ? NaN : parseFloat(sellingRaw);

            if (!Number.isFinite(sellingPrice)) {
                fieldErrors.push({
                    selector: '#selling_price',
                    message: purchaseProductValidationMessages.sellingPriceRequired,
                });
            }

            if ($form.find('#purchase_information').prop('checked')) {
                const purchaseRaw = ($form.find('#purchase_price').val() || '').toString().trim();
                const purchasePrice = purchaseRaw === '' ? NaN : parseFloat(purchaseRaw);

                if (!Number.isFinite(purchasePrice)) {
                    fieldErrors.push({
                        selector: '#purchase_price',
                        message: purchaseProductValidationMessages.purchasePriceRequired,
                    });
                }
            }

            if ($form.find('#track_inventory').prop('checked')) {
                const stockRaw = ($form.find('#opening_stock').val() || '').toString().trim();
                const openingStock = stockRaw === '' ? NaN : parseFloat(stockRaw);

                if (!Number.isFinite(openingStock)) {
                    fieldErrors.push({
                        selector: '#opening_stock',
                        message: purchaseProductValidationMessages.openingStockRequired,
                    });
                }
            }
        }

        purchaseProductValidateUnitConversions($form, fieldErrors);
        ...
        fieldErrors.forEach(function(err) {
            purchaseProductMarkFieldError($form, err.selector, err.message);
        });

        purchaseProductShowClientInvalidToast();
    ...
    window.preparePurchaseProductFormForSubmit = function($form) {
    ...
    window.submitPurchaseProductForm = function(options) {
        const $form = $(options.formSelector);
        const url = options.url;

        if (!window.validatePurchaseProductForm(options.formSelector)) {
            return;
        }

        window.preparePurchaseProductFormForSubmit($form);
