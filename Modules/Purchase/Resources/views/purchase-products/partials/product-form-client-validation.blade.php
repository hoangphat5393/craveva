...
if (!Number.isFinite(factor) || factor <= 0) { fieldErrors.push({ selector: $row.find('.unit-conversion-factor'), message: purchaseProductValidationMessages.unitConversionFactorInvalid, }); } }); } function purchaseProductShowClientInvalidToast() { if (typeof Swal==='undefined' ) { return; } Swal.fire({ icon: 'error' , text: purchaseProductValidationMessages.invalidDataToast, toast: true,
    ...
    name=($form.find('#name').val() || '' ).toString().trim(); if (name==='' ) { fieldErrors.push({ selector: '#name' , message: purchaseProductValidationMessages.nameRequired, }); } if (isService) { const sellingRaw=($form.find('#selling_price').val() || '' ).toString().trim(); const sellingPrice=sellingRaw==='' ? NaN : parseFloat(sellingRaw); if (!Number.isFinite(sellingPrice)) { fieldErrors.push({ selector: '#selling_price' , message: purchaseProductValidationMessages.sellingPriceRequired, }); } } else { const usesCostOnlyPricing=typeof window.purchaseProductTypeUsesCostUom==='function' && window.purchaseProductTypeUsesCostUom(productType); if (!usesCostOnlyPricing) { const
    sellingRaw=($form.find('#selling_price').val() || '' ).toString().trim(); const sellingPrice=sellingRaw==='' ? NaN : parseFloat(sellingRaw); if (!Number.isFinite(sellingPrice)) { fieldErrors.push({ selector: '#selling_price' , message: purchaseProductValidationMessages.sellingPriceRequired, }); } } if ($form.find('#purchase_information').prop('checked')) { const
    purchaseRaw=($form.find('#purchase_price').val() || '' ).toString().trim(); const purchasePrice=purchaseRaw==='' ? NaN : parseFloat(purchaseRaw); if (!Number.isFinite(purchasePrice)) { fieldErrors.push({ selector: '#purchase_price' , message: purchaseProductValidationMessages.purchasePriceRequired, }); } } if ($form.find('#track_inventory').prop('checked')) { const
    stockRaw=($form.find('#opening_stock').val() || '' ).toString().trim(); const openingStock=stockRaw==='' ? NaN : parseFloat(stockRaw); if (!Number.isFinite(openingStock)) { fieldErrors.push({ selector: '#opening_stock' , message: purchaseProductValidationMessages.openingStockRequired, }); } } } purchaseProductValidateUnitConversions($form, fieldErrors); if (fieldErrors.length===0) { return
    true; } fieldErrors.forEach(function(err) { purchaseProductMarkFieldError($form, err.selector, err.message); }); purchaseProductShowClientInvalidToast(); const $firstInvalid=$form.find('.is-invalid').first(); if ($firstInvalid.length) { $('html, body').animate({ scrollTop: Math.max(0, $firstInvalid.offset().top - 120), }, 200); $firstInvalid.trigger('focus'); } return false; };
    window.preparePurchaseProductFormForSubmit=function($form) { $form.find('select.select-picker').each(function() { const $select=$(this); if (typeof $select.selectpicker==='function' ) { $select.val($select.selectpicker('val')); } }); }; window.submitPurchaseProductForm=function(options) { const $form=$(options.formSelector); const url=options.url; if
    (!window.validatePurchaseProductForm(options.formSelector)) { return; } window.preparePurchaseProductFormForSubmit($form); window.apiHttp.postUrlEncoded(url, $form.serialize()) .then(options.onSuccess) .catch(function(err) { $.handleApiFormError(err); }); }; })();
