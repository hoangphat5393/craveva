        ...
        <div class="row px-lg-4 px-md-4 px-3 pt-3">
            ...
            <div class="col-md-4 mb-4">
                ...
                <div class="select-others height-35 rounded">
                    ...
                    <select class="form-control select-picker vendors" data-live-search="true" data-size="8" name="vendor_id" id="vendor_id" disabled>
                        <option value="">--</option>
                        @foreach ($vendor as $item)
                            <option @if (isset($item) && $getbill->purchase_vendor_id == $item->id) selected @endif value="{{ $item->id }}">
            ...
            <div class="col-md-4 mb-4">
                <x-forms.label class="mb-12" fieldId="credit_note_no" :fieldLabel="__('purchase::modules.vendor.vendorCreditNumber')" fieldRequired="true">
                </x-forms.label>

                <div class="select-others height-35 rounded">
                    <x-forms.input-group>
                        <x-slot name="prepend">
                            <span class="input-group-text">{{ $purchaseSetting->vendor_credit_prefix }}{{ $purchaseSetting->vendor_credit_separator }}{{ $zero }}</span>
                        </x-slot>
                        <input type="number" name="credit_note_no" id="credit_note_no" class="form-control height-35 f-15" value="{{ is_null($lastVendorCredit) ? 1 : $lastVendorCredit }}" readonly>
                    </x-forms.input-group>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-4">
                ...
                <div class="input-group">
                    <input type="text" id="vendor_credit_date" name="vendor_credit_date" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="@lang('placeholders.date')" value="{{ now(company()->timezone)->translatedFormat(company()->date_format) }}">
                </div>
            </div>
        </div>
        <div class="row px-lg-4 px-md-4 px-3 pt-3">
            <div class="col-md-3">
                <div class="form-group c-inv-select mb-5">
                    ...
                    <select class="form-control select-picker vendors" data-live-search="true" data-size="8" name="currency_id" id="currency_id">
                        @foreach ($currencies as $currency)
                            <option @if (isset($proposalTemplate) && $currency->id == $proposalTemplate->currency_id) selected
                                @elseif ($currency->id == company()->currency_id)
                                    selected @endif value="{{ $currency->id }}">
            ...
            <div class="col-md-3">
                <div class="form-group c-inv-select mb-5">
                    ...
                    <select class="form-control select-picker vendors" data-live-search="true" data-size="8" name="bill_id" id="add-bills" disabled>
                        <option value="">{{ __('app.select') . ' ' . __('app.bill') }}</option>
                        @foreach ($bills as $item)
                            <option @if (isset($item) && $getbill->id == $item->id) selected @endif value="{{ $item->id }}" disabled>
                            ...
                            <tbody>
                                        ...
                                        <div class="select-others height-35 rounded border-0">
                                            <select id="multiselect" multiple="multiple" class="select-picker type customSequence border-0" data-size="3" disabled>
                                                @foreach ($taxes as $tax)
                                                    <option data-rate="{{ $tax->rate_percent }}" data-tax-text="{{ strtoupper($tax->tax_name) . ':' . $tax->rate_percent }}%" @if (isset($item->taxes) && array_search($tax->id, $item->taxes->pluck('id')->toArray()) !== false) selected @endif value="{{ $tax->id }}">
                                ...
                                <tr class="d-none d-md-block d-lg-table-row">
                                    ...
                                    <td class="border-left-0">
                                        <input type="file" class="dropify" name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg" data-messages-default="test" data-height="70" data-id="{{ $item->id }}" id="{{ $item->id }}" data-default-file="{{ $item->purchaseItemImage ? $item->purchaseItemImage->file_url : '' }}" disabled="disabled" />
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td width="50%" class="c-inv-sub-padding">
                                                            <input type="hidden" min="0" name="discount_value" class="form-control f-14 border-0 w-100 text-right discount_value" placeholder="0" value="{{ $order->discount }}">
...
<script>
    $(document).ready(function() {
        ...
        $('.save-form').click(function() {
            ...
            window.apiHttp.postUrlEncoded("{{ route('vendor-credits.store') }}" + "?type=" + type, $('#saveInvoiceForm').serialize())
