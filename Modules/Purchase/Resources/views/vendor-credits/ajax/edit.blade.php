...
<div class="bg-white rounded b-shadow-4 create-inv">
    ...
    <x-form class="c-inv-form" id="saveInvoiceForm">
        ...
        <div class="row px-lg-4 px-md-4 px-3 pt-3">
            <div class="col-md-4 mb-4">
                ...
                <div class="select-others height-35 rounded">
                    <select class="form-control select-picker" disabled data-live-search="true" data-size="8" name="vendor_id" id="vendor_id" readonly>
                        <option value="">--</option>
            ...
            <div class="col-md-4 mb-4">
                ...
                <div class="select-others height-35 rounded">
                    <x-forms.input-group>
                        <input type="text" name="credit_note_no" id="credit_note_no" class="form-control height-35 f-15" value="{{ $vendorCredit->vendor_credit_number }}" readonly>
                    </x-forms.input-group>
                </div>
            </div>
            <div class="col-md-4 mb-4">
        ...
        <div class="row px-lg-4 px-md-4 px-3 pt-3">
                ...
                <div class="form-group c-inv-select mb-5">
                    <x-forms.select :fieldLabel="__('modules.invoices.currency')" fieldName="currency_id" field_id="currency_id">
                        @foreach ($currencies as $currency)
                            <option @if ($currency->id == $vendorCredit->currency_id) selected @endif value="{{ $currency->id }}">
            ...
            <div class="col-md-3">
                <div class="form-group c-inv-select mb-5">
                    <x-forms.select :fieldLabel="__('app.bill')" fieldName="bill_id" field_id="add-bills">
                        <option value="">{{ __('app.select') . ' ' . __('app.bill') }}</option>
                        @foreach ($bills as $item)
                            <option @if (isset($item) && $vendorCredit->bill_id == $item->id) selected @endif value="{{ $item->id }}" readonly>
                                ...
                                <tbody>
                                            ...
                                            <div class="select-others height-35 rounded border-0">
                                                <select id="multiselect" multiple="multiple" class="select-picker type customSequence border-0" data-size="3" disabled>
                                                    @foreach ($taxes as $tax)
                                                        <option data-rate="{{ $tax->rate_percent }}" data-tax-text="{{ strtoupper($tax->tax_name) . ':' . $tax->rate_percent }}%" @if (isset($item->taxes) && array_search($tax->id, json_decode($item->taxes)) !== false) selected @endif value="{{ $tax->id }}">
                                    ...
                                    <tr class="d-none d-md-block d-lg-table-row">
                                        ...
                                        <td class="border-left-0">
                                            <input type="file" class="dropify" name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg" data-messages-default="test" data-height="70" data-id="{{ $item->id }}" id="{{ $item->id }}" data-default-file="{{ $item->purchaseVendorCreditItemImage ? $item->purchaseVendorCreditItemImage->file_url : '' }}"
                                                disabled="disabled" />
                        ...
                        <table width="100%">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @endif
                                    <td class="border-bottom-0" align="right">
                                        <input type="hidden" class="form-control f-14 border-0 w-100 text-right quantity" value="{{ 1 }}" name="quantity[]">
                                        1
                                        @if (!is_null($item->unit_id) && $item->unit_id != 0)
                                            <span class="text-dark-grey border-0 f-12">{{ $item->unit->unit_type }}</span>
                                            <input type="hidden" name="product_id[]" value="{{ $item->product_id }}">
                                    ...
                                    <td class="border-bottom-0" align="right">
                                        <input type="hidden" class="f-14 border-0 w-100 text-right cost_per_item" placeholder="0.00" value="{{ currency_format($vendorCredit->total, $vendorCredit->currency_id, false) }}" name="cost_per_item[]">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td width="50%" class="c-inv-sub-padding">
                                                            <input type="hidden" min="0" name="discount_value" class="form-control f-14 border-0 w-100 text-right discount_value" placeholder="0" value="{{ $vendorCredit->discount }}">
        ...
        <div class="d-flex flex-wrap px-lg-4 px-md-4 px-3 py-3">
            <div class="col-md-6 col-sm-12 c-inv-note-terms p-0 mb-lg-0 mb-md-0 mb-3">
                ...
                <textarea class="form-control" name="note" id="note" rows="4" placeholder="@lang('placeholders.invoices.note')">{{ $vendorCredit->note }}</textarea>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('.save-form').click(function() {
            ...
            window.apiHttp.postUrlEncoded("{{ route('vendor-credits.update', $vendorCredit->id) }}", $('#saveInvoiceForm').serialize())
