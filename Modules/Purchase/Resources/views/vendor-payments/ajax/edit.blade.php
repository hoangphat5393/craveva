                                    ...
                                    <div class="select-others height-35 rounded">
                                        ...
                                        <select class="form-control select-picker" name="vendor_id" id="vendor_id" disabled>
                                            @foreach ($vendors as $vendor)
                                                <option @if ($payment->purchase_vendor_id == $vendor->id) selected @endif
                                                    value="@if ($payment->purchase_vendor_id) " {{ $payment->purchase_vendor_id }}  @else {{ $vendor->vendor_id }} @endif>{{ $vendor->primary_name }}</option>
@endforeach
                                        </select>
                                    </div>
                                </div>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-4">
                                                            <div class="form-group mb-lg-0 mb-md-0 mb-4 mt-3">
                                                                <x-forms.label class="mb-12" fieldId="payment_made" :fieldLabel="__('purchase::modules.vendorPayment.paymentMade')" fieldRequired="true">
                                                                </x-forms.label>
                                                                <x-forms.input-group>
                                                                    <x-slot name="prepend">
                                                                        <span class="input-group-text">{{ $vendor->currency->currency_code }}</span>
                                                                    </x-slot>
                                                                    <input type="number" name="payment_made" id="payment_made" onkeyup="updateAmountPaid(this.value)" class="form-control height-35 f-15" value="{{ $payment->received_payment }}" readonly>
                                                                </x-forms.input-group>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-4">
                                                            <x-forms.datepicker fieldId="payment_date" :fieldLabel="__('purchase::modules.vendorPayment.paymentDate')" fieldName="payment_date" :fieldValue="$payment->payment_date ? $payment->payment_date->timezone(company()->timezone)->format(company()->date_format) : ''" :fieldPlaceholder="__('placeholders.date')" />
                                                        </div>
                                                        <div class="col-md-4">
                                                            ...
                                                            @forelse ($bills as $bill)
                                                                <tr class="row{{ $bill->bill->id }}">
                                                                    ...
                                                                    <td class="text-right col-md-2">
                                                                        <input type="number" class="form-control height-35 f-14" onkeyup="changeDue(this.value, {{ $bill->bill->id }})" value="{{ $bill->total_paid }}" data-bill-total="{{ $bill->bill->total }}" id="payment{{ $bill->bill->id }}" readonly>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                <x-forms.textarea class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('purchase::modules.vendorPayment.internalNote')" :fieldValue="$payment->internal_note" fieldName="internal_note" fieldId="internal_note" fieldPlaceholder="">
                                                                </x-forms.textarea>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3">
...
<script>
    ...
    $(document).ready(function() {
        ...
        function validate(data, url) {
            const $btn = $('#update-vendor-payment');
            $btn.prop('disabled', true);
            $.easyBlockUI('#update-vendor-payment-data-form');
            window.apiHttp.postUrlEncoded(url, data)
                ...
                .finally(function() {
                    ...
                    $.easyUnblockUI('#update-vendor-payment-data-form');
                });
        };
