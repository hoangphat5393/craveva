                ...
                <div class="card-header bg-white border-bottom-grey  justify-content-between p-20">
                    <div class="row">
                        ...
                        <div class="col-lg-3 col-md-2 col-2 text-right">
                            @if ($editPermission == 'all' || ($editPermission == 'added' && $vendorPayment->added_by == user()->id) || ($deletePermission == 'all' || ($deletePermission == 'added' && $vendorPayment->added_by == user()->id)))
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                        @if ($editPermission == 'all' || ($editPermission == 'added' && $vendorPayment->added_by == user()->id))
                                            <a class="dropdown-item openRightModal" href="{{ route('vendor-payments.edit', $vendorPayment->id) }}"><i class="fa fa-edit mr-2"></i>@lang('app.edit')</a>
                                        @endif
                                        @if (($deletePermission == 'all' || ($deletePermission == 'added' && $vendorPayment->added_by == user()->id)) && $vendorPayment->status != 'complete')
                                            <a class="dropdown-item delete-table-row"><i class="fa fa-trash mr-2"></i>@lang('app.delete')</a>
                                        @endif
                                        <a class="dropdown-item" href="{{ route('vendor-payments.download', $vendorPayment->id) }}"><i class="fa fa-download mr-2"></i>@lang('app.download')</a>
                                    </div>
                                </div>
                            @endif
                        </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-xl-9 col-lg-8 col-md-12 mb-4 mb-xl-0 mb-lg-4 mb-md-0">
                            <x-cards.data-row :label="__('purchase::modules.vendorPayment.paymentDate')" :value="$vendorPayment->payment_date->translatedFormat(company()->date_format) ?? '--'" />
                            ...
                            @endphp
                            <x-cards.data-row :label="__('app.menu.bankaccount')" :value="$vendorPayment->bank_account_id ? $bankName . ' ' . mb_ucwords($vendorPayment->bankAccount->account_name) : '--'" />

                            <x-cards.data-row :label="__('purchase::modules.vendorPayment.paymentMade')" :value="currency_format($vendorPayment->received_payment, $currencyId) ?? '--'" />

                            <x-cards.data-row :label="__('purchase::modules.vendorPayment.internalNote')" :value="$vendorPayment->internal_note ?? '--'" />
