    ...
    <div class="row p-20">
        <div class="col-md-12">
            @if ($addApplicationNotePermission == 'all' || ($addApplicationNotePermission == 'owned' && user()->id == $application->job->recruiter_id))
                <a class="f-15 f-w-500" href="javascript:;" id="add-notes"><i class="icons icon-plus font-weight-bold mr-1"></i>@lang('app.add')
                    @lang('app.note')</a>
            @endif
                    ...
                    <div class="media">
                        <div class="media-body bg-white">
                            <div class="form-group">
                                <x-forms.textarea class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.add') . ' ' . __('app.note')" fieldName="note" fieldId="note-text" fieldPlaceholder="">
                                </x-forms.textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="d-flex flex-grow-1">
                            ...
                            <div class="dropdown ml-auto comment-action">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                    @if ($editApplicationNotePermission == 'all' || ($editApplicationNotePermission == 'added' && $comment->user_id == user()->id) || ($editApplicationNotePermission == 'owned' && user()->id == $application->job->recruiter_id) || ($editApplicationNotePermission == 'both' && user()->id == $application->job->recruiter_id) || $comment->user_id == user()->id)
                                        <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 edit-comment" href="javascript:;" data-row-id="{{ $comment->id }}">@lang('app.edit')</a>
                                    @endif

                                    @if ($deleteApplicationNotePermission == 'all' || ($deleteApplicationNotePermission == 'added' && $comment->user_id == user()->id) || ($deleteApplicationNotePermission == 'owned' && user()->id == $application->job->recruiter_id) || ($deleteApplicationNotePermission == 'both' && user()->id == $application->job->recruiter_id) || $comment->user_id == user()->id)
                                        <a class="cursor-pointer d-block text-dark-grey f-13 pb-3 px-3 delete-comment" data-row-id="{{ $comment->id }}" href="javascript:;">@lang('app.delete')</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                        <div class="card-text f-14 text-dark-grey text-justify ql-editor">{!! $comment->note_text !!}</div>
                    </div>
                </div>
            </div>
        @empty
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#submit-note").click(function() {
            ...
            window.apiHttp.postUrlEncoded(url, $('#save-note-data-form').serialize())
