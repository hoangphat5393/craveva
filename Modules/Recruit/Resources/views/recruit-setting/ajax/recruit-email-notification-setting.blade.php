...
<div id="mail-setting" class="col-lg-12 py-20">
    <x-forms.label class="" fieldId="about" :fieldLabel="__('recruit::modules.setting.sendmail')" :popover="__('recruit::modules.setting.mailSetting')">
    </x-forms.label>
    <div class="d-flex">
        @foreach ($mail->mail_setting as $item)
            <x-forms.checkbox :checked="$item['status'] == 'true'" :fieldLabel="ucwords($item['name'])" fieldName="checkBoardColumn[]" class="module_checkbox" :fieldId="'mail-column-name-' . $item['id']" :fieldValue="$item['id']" />
        @endforeach
    </div>
</div>
<div class="col-xl-8 col-lg-12 col-md-12 ntfcn-tab-content-left w-100 p-4 ">
    ...
    @foreach ($emailSettings as $emailSetting)
        <div class="mb-3 d-flex">
            <x-forms.checkbox :checked="$emailSetting->send_email == 'yes'" :fieldLabel="__('recruit::modules.emailNotification.' . str_slug($emailSetting->setting_name))" fieldName="send_email[]" :fieldId="'send_email_' . $emailSetting->id" :fieldValue="$emailSetting->id" />
        </div>
    @endforeach
</div>
