...
@if (
    !in_array('client', user_roles()) &&
        in_array(\Modules\ServerManager\Entities\ServerSetting::MODULE_NAME, user_modules()) &&
        ($hostingViewPermission != 'none' ||
            $hostingAddPermission != 'none' ||
            $hostingEditPermission != 'none' ||
            $hostingDeletePermission != 'none' ||
            $domainViewPermission != 'none' ||
            $domainAddPermission != 'none' ||
            $domainEditPermission != 'none' ||
            $domainDeletePermission != 'none' ||
            $providerViewPermission != 'none' ||
            $providerAddPermission != 'none' ||
            $providerEditPermission != 'none' ||
            $providerDeletePermission != 'none'))
    <x-menu-item icon="server" :text="__('servermanager::app.menu.serverManager')" :addon="App::environment('demo')">
        ...
        <div class="accordionItemContent">
            ...
            <!-- NAV ITEM - DASHBOARD -->
            <x-sub-menu-item :link="route('server-manager.index')" :text="__('servermanager::app.menu.dashboard')" :permission="($hostingViewPermission != 'none' && $hostingViewPermission != '') ||
                ($domainViewPermission != 'none' && $domainViewPermission != '')" />

            <!-- NAV ITEM - HOSTING -->
            <x-sub-menu-item :link="route('hosting.index')" :text="__('servermanager::app.menu.hosting')" :permission="$hostingViewPermission != 'none' && $hostingViewPermission != ''" />

            <!-- NAV ITEM - DOMAINS -->
            <x-sub-menu-item :link="route('domain.index')" :text="__('servermanager::app.menu.domains')" :permission="$domainViewPermission != 'none' && $domainViewPermission != ''" />

            <x-sub-menu-item :link="route('provider.index')" :text="__('servermanager::app.menu.providerList')" :permission="$providerViewPermission != 'none' && $providerViewPermission != ''" />
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#sideMenuScroll li a[href*="server-manager"]').closest('li').removeClass('openIt').addClass(
            'closeIt');
        ...
        if (currentUrl.includes('/server-manager/hosting')) {
            ...
            $('#sideMenuScroll li a[href*="server-manager/hosting"]').closest('li').removeClass('closeIt')
                .addClass('openIt');
            $('#sideMenuScroll li a[href*="server-manager"]').closest('li').removeClass('closeIt').addClass(
                'openIt');
        } else if (currentUrl.includes('/server-manager/domain')) {
            $('#sideMenuScroll li a[href*="server-manager/domain"]').addClass('active');
            $('#sideMenuScroll li a[href*="server-manager/domain"]').closest('li').removeClass('closeIt')
                .addClass('openIt');
            $('#sideMenuScroll li a[href*="server-manager"]').closest('li').removeClass('closeIt').addClass(
                'openIt');
        } else if (currentUrl.includes('/server-manager/provider')) {
            $('#sideMenuScroll li a[href*="server-manager/provider"]').addClass('active');
            $('#sideMenuScroll li a[href*="server-manager/provider"]').closest('li').removeClass('closeIt')
                .addClass('openIt');
            $('#sideMenuScroll li a[href*="server-manager"]').closest('li').removeClass('closeIt').addClass(
                'openIt');
