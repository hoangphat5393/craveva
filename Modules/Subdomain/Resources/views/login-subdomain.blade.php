<x-auth>
    <form id="login-form" action="{{ route('login') }}" class="ajax-form" method="POST">
        ...
        <div id="password-section " class="mb-3 mt-5">
            <x-forms.input-group>
                <input type="text" name="sub_domain" id="sub_domain" placeholder="Subdomain" tabindex="3" class="form-control height-50 f-15 light_text @error('sub_domain') is-invalid @enderror">

                <x-slot name="append">
                    <span class="btn btn-outline-secondary border-grey height-50">.{{ getDomain() }}</span>
                </x-slot>
        ...
        <button type="submit" id="save-form" class="btn-primary f-w-500 rounded w-100 height-50 f-18">
        ...
        <x-slot name="outsideLoginBox">
            <div class="form-group mt-2">
                <div class="col-sm-12 text-center">
                    ...
                    <span class="my-1"><a href="{{ route('front.forgot-company') }}" class="text-primary ">
                            <b>
            ...
            @if (!$globalSetting->frontend_disable)
                <p class="mt-2 f-12"><a href="{{ route('front.home') }}" class="text-dark-grey">@lang('superadmin.goToWebsite')</a></p>
            @endif

            @if ($global->enable_register)
                <p class="f-12"><a href="{{ route('front.signup.index') }}" class="text-dark-grey">@lang('subdomain::app.core.dontHaveAccount') </a>
                </p>
            @endif
        </x-slot>
    </form>
        ...
        <script>
            $(document).ready(function() {
                $('#save-form').on('click', function(e) {
                    e.preventDefault();
                    window.apiHttp.postUrlEncoded("{{ route('front.check-domain') }}", $('#login-form').serialize())
                        .catch(function(err) {
