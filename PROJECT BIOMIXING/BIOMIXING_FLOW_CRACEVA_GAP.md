...
| 2                             | Raw materials / packaging in (Building A or B) | Nhập nguyên liệu / bao bì (PO receipt) | **Một phần** | Purchase: PO receipt; Warehouse: stock in. Building A/B cần multi-warehouse hoặc location |
...
| **Lưu kho & công thức**       |
| 6                             | (1) Material & packaging warehousing           | Putaway, nhiều vị trí kho              | **Một phần** | Warehouse: kho, chuyển kho; chưa có location chi tiết như “恆溫空調室”                    |
...
| 28                            | Batch / lot traceability                       | Truy xuất lô nguyên liệu → TP → khách  | **Một phần** | `warehouse_product_batches` (staging); chưa end-to-end                                    |
...
- Purchase (PO, receipt)
- Warehouse (stock in/out, transfer)
- Projects / Tasks (production order, QC task)
- Delivery Order
- Invoice
...
Tham chiếu thêm: `BIOMIXING_GAP_ANALYSIS.md` (gap tổng thể), `manual_mixing_250kg_flowchart.mmd` (flow đã verify PDF).
