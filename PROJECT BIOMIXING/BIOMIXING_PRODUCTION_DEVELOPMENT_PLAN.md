...
| **Warehouse**            | `Modules/Warehouse`: `Warehouse`, movement, transfer, **đa kho**; tồn theo lô qua `warehouse_product_batches` | **Đã có:** nền **multi-warehouse**, tồn theo kho, movement/transfer, batch/HSD trên bảng batch (khác thời điểm bản kế hoạch đầu chưa có nền này). **Vẫn thiếu cho HACCP đầy đủ:** khép kín RM lô → production batch → FG → DO; **FG hold** theo QA; receiving QC; bin/location chi tiết có thể vẫn partial (xem `FUNC_LOGIC/WAREHOUSE_RUNBOOK_AND_UPGRADE_PLAN_VI.md` — WUP-09). |
...
_Tài liệu này bổ sung cho `BIOMIXING_GAP_ANALYSIS.md` (tiếng Anh, gap pilot) và `BIOMIXING_FLOW_CRACEVA_GAP.md` (đối chiếu từng bước flow). Chi tiết nâng cấp Warehouse vận hành: `FUNC_LOGIC/WAREHOUSE_RUNBOOK_AND_UPGRADE_PLAN_VI.md`. Cập nhật ngày khi chốt scope với khách hàng._
