...
- **SCSS gốc:** `resources/scss/sidebar.scss` — tham chiếu thiết kế hover (xem comment trong `theme_css.blade.php`); file này **hiện không** được khai báo trong `webpack.mix.js` / `resources/sass/app.scss`, nên **sửa file đó không tự động** ra `public/` trừ khi team gắn thêm vào pipeline Mix. **App chính:** sau khi sửa asset trong `resources/js/*.js` hoặc `resources/sass/app.scss`, chạy **`pnpm run production`** (hoặc `pnpm run dev` khi dev) — root `package.json` dùng **Laravel Mix**, **không** có script `pnpm run build`. **Module có Vite riêng** (ví dụ `Modules/Warehouse/package.json`): `pnpm run build` chỉ đúng **khi chạy trong thư mục module đó**.
