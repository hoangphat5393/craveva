...
class EstimatesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) use ($firstEstimate, $userId) {
            ...
            $isReadyForCommercialConversion = estimates_phase1_review_enabled()
                ? (($row->president_review_status === null && $row->vp_pricing_review_status === null)
        ...
        $datatables->editColumn('status', function ($row) {
            ...
            } elseif ($row->status == Estimate::STATUS_REVISION_REQUIRED) {
                $status .= '<i class="fa fa-circle mr-1 text-info f-10"></i>'.__('modules.estimates.revision_required').'</label>';
            } else {
            ...
            if (estimates_phase1_review_enabled() && ($row->president_review_status !== null || $row->vp_pricing_review_status !== null)) {
                $stage = $row instanceof Estimate
                    ? $row->workflowStagePresentation()
                    : ['label' => '', 'badge_class' => 'badge-secondary'];
                if ($stage['label'] !== '') {
                    $status .= ' <span class="badge '.$stage['badge_class'].' my-1">'.e($stage['label']).'</span>';
                }
            } elseif ($row->president_review_status !== null || $row->vp_pricing_review_status !== null) {
    ...
    public function query()
    {
        ...
        $model = Estimate::with([
            ...
            ->leftJoin('users as president_reviewer_users', 'president_reviewer_users.id', '=', 'estimates.president_reviewed_by')
            ->leftJoin('users as vp_pricing_reviewer_users', 'vp_pricing_reviewer_users.id', '=', 'estimates.vp_pricing_reviewed_by')
            ...
            ->select([
                ...
                'president_reviewer_users.name as president_reviewer_name',
                'vp_pricing_reviewer_users.name as vp_reviewer_name',
            ]);
