...
class TasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('heading', function ($row) {
            ...
            return BaseModel::clickAbleLink(route('tasks.show', [$row->id]) . $this->tabUrl, $name, $private . ' ' . $pin . ' ' . $timer);
        });
        ...
        $datatables->addColumn('progress', function ($row) {
            if ($row->subtasks_count == 0) {
                return '--';
            }
            $percent = ($row->completed_subtasks_count / $row->subtasks_count) * 100;
            return round($percent) . '%';
        });

        $datatables->editColumn('dependent_task_id', function ($row) {
            if (is_null($row->dependent_task_id) || is_null($row->dependentTask)) {
                return '--';
            }
            return '<a href="' . route('tasks.show', [$row->dependent_task_id]) . '" class="openRightModal text-dark-grey">' . $row->dependentTask->heading . '</a>';
        });
        ...
        $datatables->editColumn('board_column', function ($row) use ($taskBoardColumns, $userId, $clientIds) {
            ...
            if (
                ...
                if (($row->board_column_id == $taskBoardColumn->id) && (in_array('admin', user_roles()) || (in_array('employee', user_roles()) && $row->project_admin == user()->id))) {
                    ...
                    $status = '<select class="form-control select-picker change-status" data-size="10" data-need-approval="' . $row->need_approval_by_admin . '" data-project-admin="' . $row->project_admin . '" data-task-id="' . $row->id . '">';
        ...
        $datatables->addColumn('milestone', function ($row) use ($incompleteMilestones) {
            $status = ($row->milestone_id && $row->milestone_status == 'complete') ? $row->milestone_name : null;
        ...
        $datatables->rawColumns(array_merge(['short_code', 'board_column', 'completed_on', 'action', 'clientName', 'due_date', 'users', 'heading', 'check', 'timeLogged', 'timer', 'start_date', 'milestone', 'priority', 'labels', 'dependent_task_id'], $customFieldColumns));
    ...
    public function query(Task $model)
    {
        ...
        $model->leftJoin('users as creator_user', 'creator_user.id', '=', 'tasks.created_by')
            ...
            ->with('users', 'activeTimerAll', 'boardColumn', 'activeTimer', 'timeLogged', 'timeLogged.breaks', 'userActiveTimer', 'userActiveTimer.activeBreak', 'labels', 'taskUsers', 'category', 'dependentTask')
        ...
        if ($request->has('custom_fields') && $request->custom_fields != null) {
            $customFieldsData = $request->custom_fields;
                ...
                if ($value != '' && $value != 'all') {
                    ...
                    if ($customField) {
                        if ($customField->type == 'date') {
                            try {
                                $value = \Carbon\Carbon::createFromFormat(company()->date_format, $value)->format('Y-m-d');
                            } catch (\Exception $e) {
                                // If format fails, ignore or keep original
                            }
                        }

                        $model->whereExists(function ($query) use ($fieldId, $value) {
                            $query->select(DB::raw(1))
                                ->from('custom_fields_data')
                                ->whereColumn('custom_fields_data.model_id', 'tasks.id')
                                ->where('custom_fields_data.model', 'App\Models\Task')
                                ->where('custom_fields_data.custom_field_id', $fieldId)
                                ->where('custom_fields_data.value', $value);
                        });
                    }
                }
            }
    ...
    public function html()
    {
        $dataTable = $this->setBuilder('allTasks-table')
            ->parameters(
                ...
                $("#allTasks-table_processing").hide();
                $("#allTasks-table .select-picker").selectpicker();
                $(".bs-tooltip-top").removeClass("show");
                $("#allTasks-table .select-picker.change-status").siblings("button").removeAttr("title");
            }',
                ]
            );
        ...
        if (canDataTableExport()) {
            ...
            $dataTable->buttons(Button::make(['extend' => 'colvis', 'text' => '<i class="fa fa-columns"></i> Columns']));
        }
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('modules.taskCode') => ['data' => 'short_code', 'name' => 'task_short_code', 'title' => __('modules.taskCode'), 'visible' => false]
        ];

        // Essential Columns
        $data[__('app.task')] = ['data' => 'heading', 'name' => 'heading', 'exportable' => false, 'title' => __('app.task')];
        $data[__('app.menu.tasks') . ' '] = ['data' => 'task', 'name' => 'heading', 'visible' => false, 'title' => __('app.menu.tasks')];
        $data[__('app.project')] = ['data' => 'task_project_name', 'name' => 'task_project_name', 'title' => __('app.project')];
        $data[__('modules.tasks.assigned')] = ['data' => 'users', 'name' => 'member.name', 'exportable' => false, 'title' => __('modules.tasks.assigned')];

        // Status & Priority
        $data[__('app.columnStatus')] = ['data' => 'board_column', 'name' => 'board_column', 'exportable' => false, 'searchable' => false, 'title' => __('app.columnStatus')];
        $data[__('modules.tasks.priority')] = ['data' => 'priority', 'name' => 'priority', 'title' => __('modules.tasks.priority')];
        ...
        // Optional/Hidden
        $data[__('app.menu.taskLabel')] = ['data' => 'labels', 'name' => 'labels', 'title' => __('app.menu.taskLabel'), 'visible' => false];
        $data[__('app.completedOn')] = ['data' => 'completed_on', 'name' => 'completed_on', 'title' => __('app.completedOn'), 'visible' => false];
        $data[__('app.progress')] = ['data' => 'progress', 'name' => 'progress', 'title' => __('app.progress'), 'visible' => false];
        $data[__('app.dependentTask')] = ['data' => 'dependent_task_id', 'name' => 'dependent_task_id', 'title' => __('app.dependentTask'), 'visible' => false];
        $data[__('modules.projects.milestones')] = ['data' => 'milestone', 'name' => 'milestone_name', 'title' => __('modules.projects.milestones'), 'visible' => false];

        // Role-based adjustments and Settings
        if (in_array('client', user_roles())) {
            if (in_array('timelogs', user_modules())) {
                $data[__('modules.tasks.estimateTime')] = ['data' => 'estimateTime', 'name' => 'estimateTime', 'title' => __('modules.tasks.estimateTime'), 'visible' => false];
            }
            if (in_array('timelogs', user_modules())) {
                $data[__('modules.employees.hoursLogged')] = ['data' => 'timeLogged', 'name' => 'timeLogged', 'title' => __('modules.employees.hoursLogged'), 'visible' => false];
            }

            // Handle specific client settings that might hide columns we enabled by default
            if ($taskSettings->status == 'no') {
                unset($data[__('app.columnStatus')]);
            }
            if ($taskSettings->assigned_to == 'no') {
                unset($data[__('modules.tasks.assigned')]);
            }
        } else {
