...
class BillingController extends AccountBaseController
{
    ...
    public function stripeValidate(StripeValidateRequest $request)
    {
        ...
        try {
            ...
            if (str_contains($e->getMessage(), 'No such customer')) {
                ...
                if ($lock->get()) {
                    try {
                        // 1. Search for existing customer by email to preserve history
                        ...
                        if (! empty($customers->data)) {
                            ...
                            Log::warning("Stripe Self-Healing: Linked existing customer {$existingCustomer->id} to Company {$this->company->id}");
                        } else {
                            // Case B: Not found -> Reset ID to trigger creation of new customer
                            $this->company->stripe_id = null;
                            $this->company->save();

                            Log::warning("Stripe Self-Healing: Reset stripe_id for Company {$this->company->id} (Not found on Stripe)");
                        }
                    ...
                    // Could not get lock, another request is fixing it.
                    // Wait briefly and try creating intent again
                    sleep(2);
    ...
    public function offlinePaymentSubmit(OfflinePaymentRequest $request)
    {
        ...
        $offlinePlanChange->amount = $package->package == 'lifetime' ? ($package->price ?? 0) : ($request->type == 'monthly' ? ($package->monthly_price ?? 0) : ($package->annual_price ?? 0));
    ...
    public function freePlan(Request $request)
    {
        ...
        $offlinePrice = ($request->type === 'monthly') ? ($package->monthly_price ?? 0) : ($package->annual_price ?? 0);
        $offlineInvoice->amount = $offlinePrice;
        $offlineInvoice->total = $offlinePrice;
