...
class TaskBoardController extends AccountBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            $boardColumns = $boardColumns->get()->map(function ($query) {
                $query->setRelation('tasks', $query->tasks->take($this->taskBoardColumnLength + 40));
        ...
        if (!in_array('admin', user_roles()) && in_array('employee', user_roles())) {
            ...
            $this->waitingApprovalCount = Task::where('board_column_id', $taskBoardColumn->id)->where('company_id', $this->company->id)->count();
        }
    ...
    {
        Task::where('board_column_id', $id)->update(['board_column_id' => $this->company->default_task_status]);
