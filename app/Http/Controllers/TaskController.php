...
class TaskController extends AccountBaseController
{
    ...
    public function update(UpdateTask $request, $id)
    {
        ...
        if ($request->select_value == 'Waiting Approval') {
            ...
            $taskBoardColumn = TaskboardColumn::where('column_name', $request->select_value)->where('company_id', user()->company_id)->first();
            $task->board_column_id = $taskBoardColumn->id;
            $task->approval_send = 1;
        }
    ...
    public function show($id)
    {
        ...
        $this->task = Task::with(
            [
                ...
                'comments',
                'activeUsers',
                'comments.commentEmoji',
                'comments.like',
                'comments.dislike',
                'comments.likeUsers',
                'comments.dislikeUsers',
                'comments.user',
                'subtasks.files',
                'userActiveTimer',
                'dependentTask',
