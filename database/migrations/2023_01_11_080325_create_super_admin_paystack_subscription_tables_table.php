...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        if (Schema::hasTable('global_currencies')) {
            ...
            $this->setBigIncrementsPrimary('global_currencies', 'id');
            $this->setUnsignedBigIntegerNullable('global_settings', 'currency_id', true);
            Schema::table('global_settings', function ($table) {
            ...
            $this->setUnsignedBigIntegerNullable('packages', 'currency_id', true);
            Schema::table('packages', function ($table) {
                $table->foreign('currency_id')
                    ->references('id')
                    ->on('global_currencies')
        ...
        if (Schema::hasColumn('offline_plan_changes', 'invoice_id')) {
            $this->setUnsignedBigIntegerNullable('offline_plan_changes', 'invoice_id', true);
        }
    ...
    private function setUnsignedBigIntegerNullable(string $table, string $column, bool $nullable): void
    {
        ...
        if (in_array($driver, ['mysql', 'mariadb'], true)) {
            DB::statement("ALTER TABLE `{$table}` MODIFY `{$column}` BIGINT UNSIGNED {$nullSql} DEFAULT NULL");
            return;
        }
