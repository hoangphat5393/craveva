...
Webhook cũ và REST mới **dùng chung secret theo công ty** (`companies.ai_order_webhook_secret`), nhưng **cách gửi secret khác nhau**.

| Cách                 | URL                                        | Secret                                           |
| -------------------- | ------------------------------------------ | ------------------------------------------------ |
| **Webhook (legacy)** | `POST https://…/ai-order-webhook/{secret}` | Nằm **trong path** (ví dụ `…/32442221d321e90e…`) |
| **REST (mới)**       | `POST https://…/api/integrations/orders`   | Gửi trong **header** (không có trong URL)        |

Nếu webhook chạy mà REST báo **404**, thường là request **chưa vào đúng Laravel app** có route REST (sai thư mục Herd, code cũ, `route:cache`). Nếu là **401/403**, xem mục 3–4 bên dưới.

---

## 1. Trong app: Settings → Sale order settings

1. Đảm bảo đã **tạo / copy secret** (cùng giá trị với đoạn trong URL webhook đang chạy, hoặc regenerate nếu cần — lúc đó phải cập nhật cả webhook lẫn Postman).
2. Mục **Allowed HTTP methods** (quyền theo verb): bật **Create (POST)**. REST create và webhook create **đều** tuân theo tick này (tắt = 403 `INTEGRATION_METHOD_DISABLED`).
