...
| Triển khai tính năng mới theo Playbook đã phân tích (vd. `FUNC_IMPROVE/BIOMIXING_PRODUCTION_IMPLEMENTATION_PLAYBOOK_PHASE0_1_VI.md`) | **Agent**                                       | Playbook giảm vòng “thiết kế” nhưng checklist vẫn map sang **ghi repo** (migrate, module, Pest, doc/log). **Multitask** khi test/build/`graphify` lâu. **Ask** chỉ khi chỉ **đọc–giải thích** Playbook, chưa triển khai. |
...
| So sánh `FUNC_IMPROVE/BIOMIXING_PRODUCTION_PREP_INDEX_EN.md` với `PROJECT BIOMIXING/README.md`, **không** sửa file                         | **Ask**                  | **Tắt**                                                                                                                                                                                             |
...
| Cập nhật `FUNC_IMPROVE/P0_NEXT_ACTION_BIOMIXING_VI.md` + `P0_EXECUTION_LOG.md` (chỉ doc)                                                   | **Agent**                | **Tắt** trừ khi cùng lúc chạy lệnh nặng khác                                                                                                                                                        |
| Triển khai theo checklist trong `FUNC_IMPROVE/BIOMIXING_PRODUCTION_IMPLEMENTATION_PLAYBOOK_PHASE0_1_VI.md` (code + test + doc/log theo mẻ) | **Agent**                | **Nên bật** nếu có `php artisan test` rộng / `pnpm run production` / `graphify update .` chạy lâu; **Tắt** nếu từng bước nhỏ, test ngắn. **Ask** khi chỉ hỏi nội dung Playbook, **không** sửa repo. |
