...
| Triển khai tính năng mới theo living plan đã phân tích (vd. `FUNC_IMPROVE/BIOMIXING_GAP_STATUS.md`) | **Agent**                                       | Living plan giảm vòng “thiết kế” nhưng checklist vẫn map sang **ghi repo** (migrate, module, Pest, doc/log). **Multitask** khi test/build/`graphify` lâu. **Ask** chỉ khi chỉ **đọc–giải thích** plan, chưa triển khai. |
...
| ------------------------------------------------------------------------------------------------------------------------ | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Đọc `FUNC_LOGIC/SALES_BUSINESS.md` hoặc `FUNC_LOGIC/WAREHOUSE_INDEX.md` và hỏi thứ tự nghiệp vụ | **Ask**                  | **Tắt** (đủ) — không có lệnh nặng bắt buộc                                                                                                                                                          |
...
| Cập nhật `FUNC_IMPROVE/P0_BIOMIXING_NEXT_STEPS.md` (chỉ doc)                                                          | **Agent**                | **Tắt** trừ khi cùng lúc chạy lệnh nặng khác                                                                                                                                                        |
| Triển khai theo checklist trong `FUNC_IMPROVE/BIOMIXING_GAP_STATUS.md` / `P0_QA_BA_MASTER_TEST_CASE_TABLE.md` (code + test + doc/log theo mẻ) | **Agent**                | **Nên bật** nếu có `php artisan test` rộng / `pnpm run production` / `graphify update .` chạy lâu; **Tắt** nếu từng bước nhỏ, test ngắn. **Ask** khi chỉ hỏi nội dung plan, **không** sửa repo. |
...
**Thói quen công việc cá nhân (audit / gộp MD / browser / SSH…):** [`CURSOR_TASKS.md`](CURSOR_TASKS.md)
