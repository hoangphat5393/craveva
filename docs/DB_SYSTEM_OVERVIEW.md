...
| **Giao hàng / nhận (Purchase)**   | Phiếu nhận mua (GRN); phiếu giao bán (Sales DO)   | **`grns`**, **`grn_items`**; **`sales_dos`**, **`sales_do_items`** (bảng `delivery_orders*` / `sales_shipments*` legacy đã DROP trên env đã migrate — xem `SALES_FULFILLMENT_SCHEMA_MATRIX.md`) |
...
- [SALES_BUSINESS.md](../FUNC_LOGIC/SALES_BUSINESS.md)

Luồng **multi-warehouse / lô / hạn**:

- [WAREHOUSE_MASTER_GUIDE.md](../FUNC_LOGIC/WAREHOUSE_MASTER_GUIDE.md)
...
| Tài liệu                                                                                                     | Nội dung                                                                   |
| ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
| [AUTH_USERS_CLIENT_FLOW.md](AUTH_USERS_CLIENT_FLOW.md) | Quan hệ `users` ↔ `client_details` / client (có phần kiểm tra dữ liệu mẫu) |
| [SYSTEM_LIBRARIES_AND_MODULE_NAMES.md](SYSTEM_LIBRARIES_AND_MODULE_NAMES.md) | Thư viện Composer, tên module trong `modules` / package |
| [FUNC_LOGIC/README.md](../FUNC_LOGIC/README.md) | Mục lục FUNC_LOGIC |
