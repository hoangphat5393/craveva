...
2. **Thanh toán / Stripe / Mollie:** chỉ thử trên **môi trường test** (key test, số tiền nhỏ). Xem **`docs/LARAVEL_11_UPGRADE_GUIDE.md` §7.7** và lệnh `php artisan payment:stripe-verify` (do kỹ thuật chạy).
...
| File                                            | Nội dung                                                                        |
| ----------------------------------------------- | ------------------------------------------------------------------------------- |
| **`docs/LARAVEL_11_UPGRADE_GUIDE.md`**          | Chi tiết kỹ thuật & checklist.                                                  |
| **`docs/PROCEDURE_CI_PEST_SAFE.md`**            | Chạy test tự động trên GitHub (không ảnh hưởng Staging nếu làm đúng hướng dẫn). |
| **`docs/NOTE_CONTROLLER_CONSTRUCTOR_AUDIT.md`** | Ghi chú an toàn khi chạy lệnh hệ thống.                                         |

---

_Nếu cần hỗ trợ: nhắc team mở **`docs/LARAVEL_11_UPGRADE_GUIDE.md` §7.6–7.7** (checklist & QA thanh toán)._
