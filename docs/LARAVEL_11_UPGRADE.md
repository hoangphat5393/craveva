...
17. **CI — chạy Pest** — khung lệnh và triển khai an toàn (manual only, không đụng Staging): **`docs/CI_PEST_SAFE.md`** + **`.github/workflows/pest-mysql-manual.yml`**.
...
- [x] ~~Phương án B: `BillingController` + `SuperAdminPaypalController` bỏ tải credential khi `runningInConsole()` — xem `docs/LARAVEL_11_UPGRADE.md`~~
- [x] ~~Chuẩn bị PHPUnit 12: `ChatboxToggleTest` + `FuncNewsTest` dùng `#[Test]` thay `/** @test */`~~
- [ ] (Tùy chọn) Trên GitHub: chạy workflow **“Pest (MySQL, manual only)”** theo **`docs/CI_PEST_SAFE.md`** — không bắt buộc, không ảnh hưởng Staging nếu chỉ dùng như hướng dẫn.
...
**Người dùng không chuyên kỹ thuật:** xem **`docs/LARAVEL_11_TOM_TAT.md`**.

---

_Tài liệu: `docs/LARAVEL_11_UPGRADE.md` — gồm **§7.7–7.9**, **QA thanh toán**, và lệnh **`payment:stripe-verify`**._

---

## Phụ lục — constructor controller khi chạy Artisan

Mục tiêu: tránh lỗi khi `php artisan route:list` hoặc CLI resolve controller, đặc biệt lỗi decrypt `MAC invalid` nếu controller đọc model có cast `encrypted` trong `__construct`.

### Đã an toàn

- `app/Http/Controllers/Payment/*`: constructor không đọc DB credential; Stripe đã tách sang `configureStripeForInvoice()`.
- `PaymentGatewayCredentialController` company và super-admin: `PaymentGatewayCredentials::first()` nằm trong `index()`, không nằm trong `__construct`.

### Đã chỉnh

| File | Thay đổi |
| --- | --- |
| `SuperAdmin/BillingController.php` | Không query `GlobalPaymentGatewayCredentials` và không gọi `setStripConfigs()` khi `app()->runningInConsole()`. |
| `SuperAdmin/SuperAdminPaypalController.php` | Không đọc credential và không tạo PayPal `ApiContext` khi `runningInConsole()`. |

Request web vẫn khởi tạo đầy đủ như trước.

### Quy tắc sau này

- Không thêm DB query, decrypt credential, hoặc gọi external SDK trong `__construct` nếu controller có thể bị Artisan resolve.
- Nếu bắt buộc cần dữ liệu, ưu tiên lazy load trong action hoặc bỏ qua khi `app()->runningInConsole()` tùy nghiệp vụ.
