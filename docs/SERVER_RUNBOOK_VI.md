...
**Tài liệu chuyên sâu (không gộp vào đây):** import/poll/queue trong code (+ tracker SO/PO–Inventory) → `FUNC_IMPORT/IMPORT_POLL_TRACKERS_VI.md`. Rehearsal Phase 3 SO/DO trên staging → `docs/STAGING_OPERATIONS.md` (chỉ còn mục dài).
...
| Nội dung                                         | File                                                  |
| ------------------------------------------------ | ----------------------------------------------------- |
| Import / poll / queue trong code                 | `FUNC_IMPORT/IMPORT_POLL_TRACKERS_VI.md`              |
...
| GCP VM + Cloud SQL + allowlist (snapshot)        | `SPECIFICATION/GCP_AND_CLOUDSQL_SNAPSHOT_VI.md`       |
...
**Tài liệu nghiệp vụ:** `FUNC_IMPROVE/05_SO_DO_PO_GRN_REFACTOR_VI.md`, `docs/PHAN_TICH_MODULE_WAREHOUSE_SO_PO_DO_INVOICE_GRN_VI.md`.
...
_Cập nhật: 2026-04-04 — gộp nội dung từ `LIVE_HUB_L11_UPLOAD_AND_UPGRADE_CHECKLIST.md` và `LARAVEL_PHP_FPM_QUEUE_PERMISSIONS_VI.md` (đã xóa để tránh trùng). 2026-04-05 — mục 9 cutover schema + bẫy #7. **2026-05-12** — gộp thư mục `deploy/` vào mục 10._
