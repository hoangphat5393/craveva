...
**Runbook chính (deploy hub/staging, quyền, queue, bẫy go-live):** [`SERVER_RUNBOOK.md`](SERVER_RUNBOOK.md)

**Host SSH:** `craveva-staging`  
**App:** `/var/www/craveva-staging/current/craveva`  
**URL:** `https://staging.craveva.com`

**Nguyên tắc:** Staging **luôn lấy mã từ Git** (`git pull` trên `main`, working tree sạch). **Không** sửa tay / `scp` từng file PHP cho thay đổi tính năng. Cấu hình server-only (cron, systemd, Supervisor, snippet Nginx/PHP) xem **`docs/SERVER_RUNBOOK.md` mục 10** (mẫu đã gộp từ thư mục `deploy/` cũ).

- **Ngôn ngữ `en` (không `eng`):** xem [SERVER_RUNBOOK §6](SERVER_RUNBOOK.md#6-ngôn-ngữ-en-không-dùng-eng).
- **Git deploy, `sudo -u www-data`, Supervisor, quyền storage:** xem [SERVER_RUNBOOK](SERVER_RUNBOOK.md).
...
- Đầy disk / `git pull` lỗi: **`docs/STAGING_DEPLOY_PROGRESS.md`** (mục **Phụ lục — Recovery nhanh**) và incident trong cùng file / `SERVER_RUNBOOK.md`.
...
**Sao lưu GCP (Cloud SQL):** tần suất, số bản giữ, khi nào bản cũ bị xoay vòng, binary log / PITR — xem **[`CLOUD_SQL_BACKUP.md`](CLOUD_SQL_BACKUP.md)**.
...
Chi tiết allowlist / AI → DB: **`docs/GCP_INVENTORY.md`**.
...
| File                                              | Nội dung                                                                           |
| ------------------------------------------------- | ---------------------------------------------------------------------------------- |
| `docs/SERVER_RUNBOOK.md`                       | Deploy hub/staging, quyền, queue, bẫy                                              |
| `docs/STAGING_DEPLOY_PROGRESS.md`       | Tiến độ PHP 8.3 / L11 + incident log + **phụ lục recovery** (gộp stub recovery cũ) |
| `docs/GCP_INVENTORY.md` | DB staging / allowlist / AI connectivity (EN)                                      |
| `scripts/AUDIT_2026.md`                        | Rà soát `scripts/`: mục đích từng nhóm, legacy đã dọn, đổi tên                     |
...
4. **Backup DB:** theo quy trình trong `docs/SERVER_RUNBOOK.md` (mysqldump / snapshot), lưu vào thư mục backup chuẩn của đội (ví dụ `storage/app/backups/phase3/`).

Gợi ý biến môi trường khi tự viết wrapper (nội bộ): `MIN_FREE_MB`, `BACKUP_DIR`, `NO_BACKUP` (không khuyến nghị bỏ backup).

### 5.7 Từ máy Windows

SSH vào host staging (`gcloud compute ssh` hoặc host trong `~/.ssh/config`), `cd` tới app path trong `SERVER_RUNBOOK.md`, rồi chạy **§5.1 → §5.2**. Không còn `run_staging_phase3_safe_execute.ps1` trong repo.
...
_Cập nhật: 2026-04-04 — nội dung deploy/quyền/queue chuyển sang `SERVER_RUNBOOK.md`; file này giữ zip upload, disk, Cloud SQL, rehearsal Phase 3. **2026-05-12** — bỏ tham chiếu script shell không còn trong `scripts/`, thay bằng gate/precheck thủ công._
