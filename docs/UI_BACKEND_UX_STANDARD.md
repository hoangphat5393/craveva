...
**Tên file:** `UI_BACKEND_UX_STANDARD.md` (trước đây: `HUB_BACKEND_UI_UX_DESIGN_SPEC.md`, `HUB_FORM_UI_CONVENTIONS.md`).
...
**Tham chiếu kỹ thuật layout/JS:** `UI_FRONTEND_LAYOUT_JS.md` (mục §5 trỏ tới file này).
...
- `UI_FRONTEND_LAYOUT_JS.md` §5 → trỏ tới **file này**.
- Form validation AJAX (inline + toast + `Reply::redirect`): **mục §12**.
- Không dùng tên `FRONTEND_UI.md`, `HUB_BACKEND_UI_UX_DESIGN_SPEC.md`, `HUB_FORM_UI_CONVENTIONS.md` (đã đổi tên).
...
| Client create/edit/contact/notes/import | (tùy form)                    | `$.handleApiFormError`; rule chính nằm ở `StoreClientRequest` / `UpdateClientRequest` |
