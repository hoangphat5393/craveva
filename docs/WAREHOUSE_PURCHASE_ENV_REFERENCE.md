# Tham chiếu biến môi trường — Kho, PO / GRN (DO nhập), Sales DO (phiếu giao bán), SO / inbound AI (REST)
...
| `WAREHOUSE_AI_ORDER_WEBHOOK_CHECK_STOCK` | `true`     | Inbound AI tạo SO (**`POST /api/integrations/orders`**): kiểm tra **sellable** trước khi tạo SO. `false` nếu tích hợp chưa gửi `unit_id` / chưa sẵn dữ liệu tồn.       |
...
| `PURCHASE_DO_GRN_CUTOVER_ENABLED`           | `false`     | Ảnh hưởng **`FlowPermission`** (alias quyền legacy vs mới). **`GrnRuntime` / `SalesDoRuntime` đã pin luôn dùng bảng `grns` / `sales_dos`** — biến này **không** tắt lại bảng cũ qua runtime. Chi tiết: [`SALES_FULFILLMENT_SCHEMA_MATRIX.md`](../FUNC_LOGIC/SALES_FULFILLMENT_SCHEMA_MATRIX.md). |
...
## 3) Tích hợp AI → tạo Sales Order (SO) qua REST

Không nằm trong `config/warehouse.php` nhưng **dùng chung** với kiểm tồn (WUP-05) khi bật check stock.

| Biến / cấu hình           | Ghi chú                                                                                                                                                                                                                                                                                                                            |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AI_ORDER_WEBHOOK_SECRET` | Chuỗi bí mật **fallback toàn instance** (khi công ty chưa có secret DB). Gửi trong header **`X-AI-Webhook-Secret`** hoặc **`Authorization: Bearer`** khi gọi **`POST /api/integrations/orders`** — **không** còn secret trên path URL. Đọc trong code qua `config('app.ai_order_webhook_secret', env('AI_ORDER_WEBHOOK_SECRET'))`. |
| (payload) `check_stock`   | Không phải env: client gửi `false` để bỏ qua kiểm sellable trên inbound AI.                                                                                                                                                                                                                                                        |
| Gói company               | Kiểm tồn inbound chỉ chạy nếu package company có module **`warehouse`** (theo JSON `module_in_package`).                                                                                                                                                                                                                           |

**Cập nhật:** 2026-05-13 — đối chiếu `Modules/Warehouse/Config/config.php`, `Modules/Purchase/Config/config.php`, `AiIntegrationOrdersController`.
...
- Runbook & trạng thái WUP: [`WAREHOUSE_RUNBOOK_AND_UPGRADE_PLAN.md`](../FUNC_IMPROVE/04_WH_RUNBOOK_UPGRADE.md)
- Quy trình PO / DO / SO / Invoice / Kho: [`SALES_BUSINESS.md`](../FUNC_LOGIC/SALES_BUSINESS.md)
- QA đối chiếu code: [`SALES_FULFILLMENT_QA_CHECKLIST.md`](../FUNC_LOGIC/SALES_FULFILLMENT_QA_CHECKLIST.md)
