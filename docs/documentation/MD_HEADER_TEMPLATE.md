...
| Doc ID               | <DOC_ID>                                                |
| Module               | <MODULE_NAME>                                           |
| Group                | <FUNC_BUG \| FUNC_IMPROVE \| FUNC_LOGIC> |
...
- Do not add `_VI` suffix for Vietnamese docs; use clear Vietnamese titles/content instead. Keep language suffixes only when a non-Vietnamese variant must coexist, e.g. `_EN`.
