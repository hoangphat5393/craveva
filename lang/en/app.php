...
return [
    ...
    'backToQuotationList' => 'Back to Quotations',
    ...
    'importQuotationExcelInfo' => 'Map columns to system fields. ROC dates (e.g. 114/12/01) are converted automatically. One quotation can span multiple rows: header columns may be blank on continuation lines (forward-fill by quotation number). Client code must match an existing client; product code is optional but links the line to inventory when matched. Header fields (quotation/document dates, exchange rate, source totals, payment terms, etc.) are stored on the estimate on the first row of each quotation number; line free quantity and effective/expiry dates are stored per item.',
    ...
    'importRowErrorsTitle' => 'Row-level errors',
    'importRowErrorsHelp' => 'Each badge is one row in the file that could not be imported. Fix the data or master records, then re-import.',
    'downloadImportRowErrorsCsv' => 'Download row errors (CSV)',
    'importFailedJobsSummary' => 'Failed job summary (one line per background task; details are in the badges above).',
