  ...
  $.easyAjax({
    ...
    error: function error(request, status, _error) {
      ...
      if (request.status == 403) {
        $(RIGHT_MODAL_CONTENT).html('<div class="align-content-between d-flex justify-content-center mt-105 f-21">403 | Permission Denied</div>');
      } else if (request.status == 404) {
        $(RIGHT_MODAL_CONTENT).html('<div class="align-content-between d-flex justify-content-center mt-105 f-21">404 | Not Found</div>');
      } else if (request.status == 500) {
        $(RIGHT_MODAL_CONTENT).html('<div class="align-content-between d-flex justify-content-center mt-105 f-21">500 | Something Went Wrong</div>');
      }
    }
  });
});
...
if (typeof pathArray[1] !== 'undefined') {
  ...
  var element = $('#sideMenuScroll li a').filter(function () {
    return this.href == currentUrl.href;
  }).addClass('active').closest('li').removeClass('closeIt').addClass('openIt');
