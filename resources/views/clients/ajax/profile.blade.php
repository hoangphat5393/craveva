        ...
        <div class="row">
                        ...
                        <div class="col-2 text-right">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                    <a class="dropdown-item openRightModal" href="{{ route('clients.edit', $client->id) }}">@lang('app.edit')</a>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            @if (in_array('projects', user_modules()))
                <div class="col-xl-5 col-lg-6 col-md-6">
                    <x-cards.widget :title="__('modules.dashboard.totalProjects')" :value="$clientStats->totalProjects" icon="layer-group" />
                </div>
            @endif
        </div>
    </div>
        ...
        <div class="row">
            @if (in_array('payments', user_modules()))
                <div class="col-lg-6 col-md-6 col-sm-12 mb-4 mb-lg-0 mb-md-0">
                    <x-cards.widget :title="__('modules.dashboard.totalEarnings')" :value="$earningTotal" icon="coins" />
                </div>
            @endif
            @if (in_array('invoices', user_modules()))
                <div class="col-lg-6 col-md-6 col-sm-12">
                    <x-cards.widget :title="__('modules.dashboard.totalUnpaidInvoices')" :value="$clientStats->totalUnpaidInvoices" icon="file-invoice-dollar" />
                </div>
            @endif
        </div>
    </div>
...
<div class="row mt-4">
    <div class="col-xl-7 col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4">
        <x-cards.data :title="__('modules.client.profileInfo')">
            ...
            <x-cards.data-row :label="__('modules.client.companyName')" :value="$client->clientDetails->company_name ?? '--'" />
            ...
            <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                ...
                <p class="mb-0 text-dark-grey f-14 w-70">
                    @if ($client->clientDetails->company_logo)
                        <img data-toggle="tooltip" style="height:50px;" src="{{ $client->clientDetails->image_url }}">
                    @else
                        --
                    @endif
                </p>
            ...
            <x-cards.data-row :label="__('app.mobile')" :value="$client->mobile_with_phonecode" />
            ...
            <x-cards.data-row :label="__('modules.client.officePhoneNumber')" :value="$client->clientDetails->office ?? '--'" />
            ...
            <x-cards.data-row :label="__('modules.stripeCustomerAddress.city')" :value="$client->clientDetails->city ?? '--'" />

            <x-cards.data-row :label="__('modules.stripeCustomerAddress.postalCode')" :value="$client->clientDetails->postal_code ?? '--'" />

            <x-cards.data-row :label="__('app.language')" :value="$clientLanguage->label . ' ' . $clientLanguage->language_name ?? '--'" />
        ...
        <div class="row">
            <div class="col-md-12">
                <x-cards.data :title="__('app.menu.projects')">
                    ...
                    @endif
                    <x-pie-chart id="project-chart" :labels="$projectChart['labels']" :values="$projectChart['values']" :colors="$projectChart['colors']" height="220" width="220" />
                </x-cards.data>
            </div>
        </div>
        @if (in_array('invoices', user_modules()))
                ...
                <div class="col-md-12">
                    <div class="card bg-white border-0 b-shadow-4">
                        <x-cards.data :title="__('app.menu.invoices')">
                            ...
                            @endif
                            <x-pie-chart id="invoice-chart" :labels="$invoiceChart['labels']" :values="$invoiceChart['values']" :colors="$invoiceChart['colors']" height="230" width="220" />
                        </x-cards.data>
                    </div>
                </div>
            </div>
