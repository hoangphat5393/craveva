            ...
            <nav class="tabs">
                <ul class="-primary">
                    ...
                    @if (in_array('projects', user_modules()))
                        <li>
                            <x-tab :href="route('clients.show', $client->id) . '?tab=projects'" ajax="false" :text="__('app.menu.projects')" class="projects" />
                        </li>
                    @endif

                    @if (in_array('invoices', user_modules()))
                        <li>
                            <x-tab :href="route('clients.show', $client->id) . '?tab=invoices'" ajax="false" :text="__('app.menu.invoices')" class="invoices" />
                        </li>
                    @endif
                    ...
                    <li>
                        <x-tab :href="route('clients.show', $client->id) . '?tab=creditnotes'" ajax="false" :text="__('app.menu.credit-note')" class="creditnotes" />
                    </li>

                    @if (in_array('payments', user_modules()))
                        <li>
                            <x-tab :href="route('clients.show', $client->id) . '?tab=payments'" ajax="false" :text="__('app.menu.payments')" class="payments" />
                        </li>
                    @endif

                    @if ($viewClientContact == 'all' || $viewClientContact == 'added')
                        <li>
                            <x-tab :href="route('clients.show', $client->id) . '?tab=contacts'" ajax="false" :text="__('app.menu.contacts')" class="contacts" />
                        </li>
                    @endif
                    ...
                    @if ($viewClientDoc == 'all' || ($viewClientDoc == 'added' && $client->clientDetails->added_by == user()->id) || ($viewClientDoc == 'owned' && $client->clientDetails->user_id == user()->id) || ($viewClientDoc == 'both' && ($client->clientDetails->added_by == user()->id || $client->clientDetails->user_id == user()->id)))
                        <li>
                            <x-tab :href="route('clients.show', $client->id) . '?tab=documents'" ajax="false" :text="__('app.menu.documents')" class="documents" />
                        </li>
                    @endif

                    @if ($viewClientNote != 'none')
                        <li>
                            <x-tab :href="route('clients.show', $client->id) . '?tab=notes'" ajax="false" :text="__('app.menu.notes')" class="notes" />
                        </li>
                    @endif

                    @if ($viewTicket == 'all' || $viewTicket == 'added')
                        <li>
                            <x-tab :href="route('clients.show', $client->id) . '?tab=tickets'" ajax="false" :text="__('app.menu.tickets')" class="tickets" />
                        </li>
                    @endif
                    ...
                    @if ($gdpr->enable_gdpr)
                        <li>
                            <x-tab :href="route('clients.show', $client->id) . '?tab=gdpr'" ajax="false" :text="__('app.menu.gdpr')" class="gdpr" />
                        </li>
                    @endif
                </ul>
            </nav>
