    ...
    <x-forms.input-group>
        <select class="form-control select-picker" data-live-search="true" data-size="8" name="client_id" id="client_list_id">
            ...
            @foreach ($clients as $clientOpt)
                <option @selected(!is_null($selected) && $selected == $clientOpt->id) data-content="<x-client-search-option :user='$clientOpt' />" value="{{ $clientOpt->id }}">{{ $clientOpt->name }} </option>
            @endforeach
        </select>

        @if ($addClientPermission == 'all' || $addClientPermission == 'added')
            <x-slot name="append">
                <a href="javascript:;" id="quick-create-client" data-toggle="tooltip" data-original-title="{{ __('modules.client.addNewClient') }}" class="btn btn-outline-secondary border-grey" data-redirect-url="{{ url()->full() }}">@lang('app.add')</a>
            </x-slot>
        @endif
    </x-forms.input-group>
</div>
