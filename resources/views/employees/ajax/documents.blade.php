...
<div class="tab-pane fade show active mt-5" role="tabpanel" aria-labelledby="nav-email-tab">
    <x-cards.data :title="__('app.menu.documents')">
        ...
        @if ($addDocumentPermission == 'all' || $addDocumentPermission == 'added')
            <div class="row">
                <div class="col-md-12">
                    <a class="f-15 f-w-500" href="javascript:;" id="add-task-file"><i class="icons icon-plus font-weight-bold mr-1"></i>@lang('modules.lead.addFile')
                    </a>
                </div>
            </div>
            ...
            <x-form id="save-taskfile-data-form" class="d-none">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <x-forms.file :fieldLabel="__('modules.projects.uploadFile')" fieldName="file" fieldRequired="true" fieldId="employee_file" allowedFileExtensions="txt pdf doc xls xlsx docx rtf png jpg jpeg svg" :popover="__('messages.fileFormat.multipleImageFile')" />
                    </div>
                    <div class="col-md-12">
                            ...
                            <div class="dropdown ml-auto file-action">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                    @if ($viewDocumentPermission == 'all' || ($viewDocumentPermission == 'added' && $file->added_by == user()->id) || ($viewDocumentPermission == 'owned' && ($file->user_id == user()->id && $file->added_by != user()->id)) || ($viewDocumentPermission == 'both' && ($file->added_by == user()->id || $file->user_id == user()->id)))
                                        ...
                                        <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 " href="{{ route('employee-docs.download', md5($file->id)) }}">@lang('app.download')</a>
                                    @endif
...
<script>
    ...
    $('#submit-document').click(function() {
        ...
        var $subDoc = $('#submit-document');
        $subDoc.prop('disabled', true);
        $.easyBlockUI('#save-taskfile-data-form');
        window.apiHttp.postForm(url, document.getElementById('save-taskfile-data-form')).then(function(response) {
            ...
            $subDoc.prop('disabled', false);
            $.easyUnblockUI('#save-taskfile-data-form');
        });
    });
    ...
    $('body').off('click', '#submit-document-expiry').on('click', '#submit-document-expiry', function(e) {
        ...
        var $subExp = $('#submit-document-expiry');
        $.easyBlockUI('#save-document-expiry-form');
        window.apiHttp.postForm(url, document.getElementById('save-document-expiry-form')).then(function(response) {
            if (response.status == 'success') {
                $('#document-expiry-list-container').html(response.view);
                $(MODAL_XL).modal('hide');
            }
            $subExp.data('submitting', false);
        }).catch(function(err) {
            $subExp.data('submitting', false);
            ...
            $.easyUnblockUI('#save-document-expiry-form');
        });
    });
    ...
    $('body').off('click', '#update-document-expiry').on('click', '#update-document-expiry', function(e) {
        ...
        var $updExp = $('#update-document-expiry');
        $.easyBlockUI('#update-document-expiry-form');
        window.apiHttp.postForm(url, document.getElementById('update-document-expiry-form')).then(function(response) {
            if (response.status == 'success') {
                $(MODAL_XL).modal('hide');
                $('#document-expiry-list-container').html(response.view);

            }
            $updExp.data('submitting', false);
        }).catch(function(err) {
            $updExp.data('submitting', false);
            if (err.status === 422 && err.errors) {
                $(MODAL_XL).find('.invalid-feedback').remove();
            ...
            $.easyUnblockUI('#update-document-expiry-form');
        });
    });
    ...
    $('body').on('click', '.delete-document-expiry', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.easyBlockUI('#document-expiry-list-container');
                window.apiHttp.delete(url, '{{ csrf_token() }}').then(function(response) {
                    ...
                    $.easyUnblockUI('#document-expiry-list-container');
                });
            }
        });
    });
