    ...
    <div class="portlet-body">
        <x-form id="save-passport-data-form" method="PUT" class="ajax-form">
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <x-forms.text :fieldLabel="__('modules.employees.passportNumber')" fieldName="passport_number" fieldId="passport_number" :fieldValue="$passport->passport_number ?? ''" :fieldRequired="true" />
                </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    <x-forms.datepicker fieldId="issue_date" fieldRequired="true" :fieldLabel="__('modules.employees.issueDate')" fieldName="issue_date" :fieldValue="$passport ? $passport->issue_date->format(company()->date_format) : now(company()->timezone)->format(company()->date_format)" :fieldPlaceholder="__('placeholders.date')" />
                </div>

                <div class="col-lg-4">
                    <x-forms.datepicker fieldId="expiry_date" fieldRequired="true" :fieldLabel="__('modules.employees.expiryDate')" fieldName="expiry_date" :fieldValue="$passport ? $passport->expiry_date->format(company()->date_format) : now(company()->timezone)->format(company()->date_format)" :fieldPlaceholder="__('placeholders.date')" />
                </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <x-forms.file allowedFileExtensions="png jpg jpeg svg pdf doc docx" class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('modules.employees.scanCopy')" fieldName="file" :fieldValue="$passport->file ? $passport->image_url : ''" fieldId="file">
                    </x-forms.file>
                </div>
            </div>
        </x-form>
...
<script>
    ...
    $('#save-passport-form').click(function() {
        ...
        window.apiHttp.postForm("{{ route('passport.update', $passport->id) }}", document.getElementById('save-passport-data-form')).then(function(response) {
