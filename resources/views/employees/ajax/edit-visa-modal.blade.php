    ...
    <div class="portlet-body">
        <x-form id="save-visa-data-form" method="PUT" class="ajax-form">
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <x-forms.text :fieldLabel="__('modules.employees.visaNumber')" fieldName="visa_number" fieldId="visa_number" :fieldValue="$visa->visa_number ?? ''" :fieldRequired="true" />
                </div>

                <div class="col-lg-6">
                    <x-forms.select fieldId="country" :fieldLabel="__('app.country')" fieldName="country" search="true" :fieldRequired="true">
                        <option value="">--</option>
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    <x-forms.datepicker fieldId="issue_date" fieldRequired="true" :fieldLabel="__('modules.employees.issueDate')" fieldName="issue_date" :fieldValue="$visa ? $visa->issue_date->format(company()->date_format) : now(company()->timezone)->format(company()->date_format)" :fieldPlaceholder="__('placeholders.date')" />
                </div>

                <div class="col-lg-4">
                    <x-forms.datepicker fieldId="expiry_date" fieldRequired="true" :fieldLabel="__('modules.employees.expiryDate')" fieldName="expiry_date" :fieldValue="$visa ? $visa->expiry_date->format(company()->date_format) : now(company()->timezone)->format(company()->date_format)" :fieldPlaceholder="__('placeholders.date')" />
                </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <x-forms.file allowedFileExtensions="png jpg jpeg svg pdf doc docx" class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('modules.employees.scanCopy')" fieldName="file" :fieldValue="$visa->file ? $visa->image_url : ''" fieldId="file">
                    </x-forms.file>
                </div>
            </div>
        </x-form>
...
<script>
    ...
    $('#save-visa-form').click(function() {
        var $btn = $('#save-visa-form');
        $btn.prop('disabled', true);
        $.easyBlockUI('#save-visa-data-form');
        window.apiHttp.postForm("{{ route('employee-visa.update', $visa->id) }}", document.getElementById('save-visa-data-form')).then(function(response) {
            ...
            $.easyUnblockUI('#save-visa-data-form');
        });
    });
</script>
