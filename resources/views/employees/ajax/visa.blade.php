    ...
    <div class="col-xl-12 col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4 mb-md-0 mt-3">
        <x-cards.data :title="__('modules.employees.visaDetails')">
            <x-slot name="action">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                        @if ($editImmigrationPermission == 'all' || ($editImmigrationPermission == 'added' && $visa->added_by == user()->id) || ($editImmigrationPermission == 'owned' && ($visa->user_id == user()->id && $visa->added_by != user()->id)) || ($editImmigrationPermission == 'both' && ($visa->added_by == user()->id || $visa->user_id == user()->id)))
                            <a class="dropdown-item edit-visa" data-id = "{{ $visa ? $visa->id : '' }}" href="javascript:;">@lang('app.edit')</a>
                        @endif
                        @if ($deleteImmigrationPermission == 'all' || ($deleteImmigrationPermission == 'added' && $visa->added_by == user()->id) || ($deleteImmigrationPermission == 'owned' && ($visa->user_id == user()->id && $visa->added_by != user()->id)) || ($deleteImmigrationPermission == 'both' && ($visa->added_by == user()->id || $visa->user_id == user()->id)))
                            <a class="dropdown-item delete-visa" data-id = "{{ $visa ? $visa->id : '' }}" href="javascript:;">@lang('app.delete')</a>
                        @endif
                    </div>
                </div>
            </x-slot>
            ...
            <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                ...
                <p class="mb-0 text-dark-grey f-14 w-70">
                    @if ($visa->file)
                        <a target="_blank" class="text-dark-grey" href="{{ $visa->image_url }}"><i class="fa fa-external-link-alt"></i> <u>@lang('app.viewScanCopy')</u></a>
                    @else
                        --
                    @endif
                </p>
            ...
            if (result.isConfirmed) {
                ...
                window.apiHttp.delete(url, token).then(function(response) {
                    if (response.status == "success") {
                        var redirectUrl = " {{ route('employees.show', $visa->user_id) . '?tab=immigration' }}";
