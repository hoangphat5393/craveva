        ...
        <x-form class="c-inv-form" id="saveInvoiceForm">
            ...
            <div class="row px-lg-4 px-md-4 px-3 py-3">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <div class="input-group">
                            <div class="input-group-prepend  height-35 ">
                                <span class="input-group-text border-grey f-15 bg-additional-grey px-3 text-dark" id="basic-addon1">{{ $invoiceSetting->estimate_prefix }}{{ $invoiceSetting->estimate_number_separator }}{{ $zero }}</span>
                            </div>
                            <input type="text" name="estimate_number" id="estimate_number" class="form-control height-35 f-15" value="@if (is_null($lastEstimate)) 1 @else{{ $lastEstimate }} @endif" placeholder="0019" aria-label="0019" aria-describedby="basic-addon1">
                        </div>
                    </div>
                </div>
                <!-- INVOICE NUMBER END -->
                        ...
                        <div class="select-others height-35 rounded">
                            <select class="form-control select-picker" name="currency_id" id="currency_id">
                                ...
                                @else @selected($currency->id == company()->currency_id)
                                      @selected($estimateTemplate && $currency->id == $estimateTemplate->currency_id) @endif value="{{ $currency->id }}">
                ...
                <div class="col-md-6 col-lg-4">
                    ...
                    @else
                        <x-client-selection-dropdown :clients="$clients" :selected="isset($estimate) ? $estimate->client_id : (request()->has('default_client') ? request()->input('default_client') : $estimateTemplate->client_id ?? null)" />
                    @endif
                </div>
                <!-- CLIENT END -->
                        ...
                        <div class="form-group c-inv-select mb-4">
                            <x-forms.input-group>
                                ...
                                @if ($addProductPermission == 'all' || $addProductPermission == 'added')
                                    <x-slot name="append">
                                        <a href="{{ route('products.create') }}" data-redirect-url="no" class="btn btn-outline-secondary border-grey openRightModal" data-toggle="tooltip" data-original-title="{{ __('modules.dashboard.addNewProduct') }}">@lang('app.add')</a>
                                    </x-slot>
                                @endif
                            </x-forms.input-group>
                        </div>
                                ...
                                <table width="100%">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            @endif
                                            <td class="border-bottom-0">
                                                <input type="number" min="1" class="form-control f-14 border-0 w-100 text-right quantity" value="{{ $item->quantity }}" name="quantity[]">
                                ...
                                <table width="100%">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            @endif
                                            <td class="border-bottom-0">
                                                <input type="number" min="1" class="f-14 border-0 w-100 text-right quantity form-control" name="quantity[]" value="{{ $item->quantity }}">
                                                @if (!is_null($item->product_id) && $item->product_id != 0)
                                                    <span class="text-dark-grey float-right border-0 f-12">{{ $item->unit->unit_type }}</span>
                                                    <input type="hidden" name="product_id[]" value="{{ $item->product_id }}">
                                                    <input type="hidden" name="unit_id[]" value="{{ $item->unit_id }}">
                                        ...
                                        <tr class="d-none d-md-table-row d-lg-table-row">
                                            ...
                                            <td class="border-left-0">
                                                ...
                                                <input type="file" class="dropify" name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg bmp" data-messages-default="test" data-height="70" data-id="{{ $item->id }}" id="{{ $item->id }}" data-default-file="{{ $item->estimateTemplateItemImage ? $item->estimateTemplateItemImage->file_url : '' }}"
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td width="70%" class="c-inv-sub-padding">
                                                                <input type="number" min="0" name="discount_value" class="form-control f-14 border-0 w-100 text-right discount_value" placeholder="0" {{ isset($estimteTemplate) ? $estimateTemplate->discount : '0' }} </td>
                                                            <td width="30%" align="left" class="c-inv-sub-padding">
                                                                <div class="select-others select-tax height-35 rounded border-0">
                                                                    <select class="form-control select-picker" id="discount_type" name="discount_type">
                                                                        <option @selected(isset($estimateTemplate) && $estimateTemplate->discount_type == 'percent') value="percent">%
                                                                        </option>
                                                                        <option @selected(isset($estimateTemplate) && $estimateTemplate->discount_type == 'fixed') value="fixed">
                                        ...
                                        <tr class="bg-amt-grey f-16 f-w-500">
                                            ...
                                            <td>
                                                <span class="total-currency mr-1">
                                                    {{ companyOrGlobalSetting()->currency ? companyOrGlobalSetting()->currency->currency_code : '' }}
                                                </span>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function addProduct(id) {
                ...
                $.easyBlockUI('#saveInvoiceForm');
                window.apiHttp.get("{{ route('estimates.add_item') }}", {
                    params: {
                        id: id,
            ...
            $('.save-form').click(function() {
                ...
                var storeUrl = "{{ route('estimates.store') }}?type=" + type;
