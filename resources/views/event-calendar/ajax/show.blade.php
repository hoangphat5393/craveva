    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card bg-white border-0 b-shadow-4">
                    ...
                    <div class="row">
                        <div class="col-lg-8 col-10">
                            @if ($event->status == 'pending')
                                @if ($editPermission == 'all' || ($editPermission == 'added' && $event->added_by == user()->id) || ($editPermission == 'owned' && in_array(user()->id, $attendeesIds)) || ($editPermission == 'both' && (in_array(user()->id, $attendeesIds) || $event->added_by == user()->id)) || $event->host == user()->id)
                                    <x-forms.button-primary icon="check" data-event-id="{{ $event->id }}" id="event-status" value="completed" class="mr-2 mb-2 mb-lg-0 mb-md-0">
                                        @lang('app.markComplete')
                                    </x-forms.button-primary>

                                    <x-forms.button-secondary icon="times" data-event-id="{{ $event->id }}" id="event-status2" value="cancelled" class="mr-3">
                                        @lang('app.markCancel')
                        ...
                        <div class="col-lg-4 col-2 text-right">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                    @if ($editPermission == 'all' || ($editPermission == 'added' && $event->added_by == user()->id) || ($editPermission == 'owned' && (in_array(user()->id, $attendeesIds) || $event->host == user()->id)) || ($editPermission == 'both' && (in_array(user()->id, $attendeesIds) || $event->added_by == user()->id || $event->host == user()->id)))
                                        <a class="dropdown-item openRightModal" href="{{ route('events.edit', $event->id) }}">@lang('app.edit')
                                        </a>
                                    @endif

                                    @if ($deletePermission == 'all' || ($deletePermission == 'added' && $event->added_by == user()->id) || ($deletePermission == 'owned' && (in_array(user()->id, $attendeesIds) || $event->added_by == user()->id)) || ($deletePermission == 'both' && (in_array(user()->id, $attendeesIds) || $event->added_by == user()->id)))
                ...
                <div class="card-body">
                    <x-cards.data-row :label="__('modules.events.eventName')" :value="$event->event_name" html="true" />
                                ...
                                @foreach ($event->attendee as $item)
                                    @if (in_array('employee', $item->user->roles->pluck('name')->toArray()))
                                        <div class="taskEmployeeImg rounded-circle mr-1">
                                            <img data-toggle="tooltip" data-original-title="{{ $item->user->name }}" src="{{ $item->user->image_url }}">
                                        </div>
                                    @endif
                                @endforeach
                            </p>
                    ...
                    <div class="col-12 px-0 pb-3 d-flex">
                        ...
                        <p class="mb-0 text-dark-grey f-14">
                            @if ($event->user)
                                <div class="taskEmployeeImg rounded-circle mr-1">
                                    <img data-toggle="tooltip" data-original-title="{{ $event->user->name }}" src="{{ $event->user->image_url }}">
                                </div>
                            @else
                                --
                            @endif
                    ...
                    <x-cards.data-row :label="__('app.description')" :value="$event->description" html="true" />
                    <x-cards.data-row :label="__('app.where')" :value="$event->where" html="true" />
                    <x-cards.data-row :label="__('modules.events.startOn')" :value="$event->start_date_time->translatedFormat(company()->date_format . ' - ' . company()->time_format)" html="true" />
                    <x-cards.data-row :label="__('modules.events.endOn')" :value="$event->end_date_time->translatedFormat(company()->date_format . ' - ' . company()->time_format)" html="true" />
                    @php
                    ...
                    @endif
                    @if ($event->note)
                        <x-cards.data-row :label="__('app.note')" :value="$event->note" html="true" />
                    @endif
                    <x-cards.data-row :label="__('modules.events.eventLink')" html="true" :value="$link" />
                    ...
                    @if ($event->files->count() > 0)
                        <x-cards.data-row :label="__('app.file')" html="true" :value="''" />
