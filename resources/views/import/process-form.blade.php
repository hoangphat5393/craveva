            ...
            <div class="col-12">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <a href="javascript:void(0);" onclick="skipall()">@lang('app.skipAll')</a>
                    </div>
                    <div class="col-md-6">
                    ...
                    @forelse ($importSample[0] as $key => $item)
                        <div class="col-md-3 importBox  border-grey {{ !empty($heading) ? (collect($columns)->whereIn('id', $heading[$key])->first() ? 'matched' : 'unmatched') : 'unmatched' }}" id="box_{{ $key }}" data-key="{{ $key }}">
                            <div class="importOptions w-100">
                                ...
                                <div class="selectColumnNameBox" id="selectColumnNameBox_{{ $key }}" style="{{ !empty($heading) && collect($columns)->whereIn('id', $heading[$key])->first() ? 'display:none;' : '' }}">
                                    <div class="col-sm-12 p-0">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div id="selectOptionList_{{ $key }}">
                                                <select class="form-control select-picker mb-2" id="columnName_{{ $key }}" name="columns[{{ $key }}]" data-live-search="true">
                                    ...
                                    <div class="col-sm-12 p-0">
                                        <div class="form-group">
                                            ...
                                            <button onclick="saveColumnBox({{ $key }})" class="btn btn-dark btn-sm" type="button">@lang('app.save')</button>
                                            <a href="javascript:void(0);" onclick="skipColumnBox({{ $key }})">@lang('app.skip')</a>
                                        </div>
                                    </div>
                                </div>

                                <div class="row columnDescriptionBox" id="columnDescriptionBox_{{ $key }}" style="{{ !empty($heading) && collect($columns)->whereIn('id', $heading[$key])->first() ? '' : 'display:none;' }}">
                                    <div class="col-sm-12">
                                ...
                                <div class="row editAndSkipBox" id="editAndSkipBox_{{ $key }}" style="{{ !empty($heading) && collect($columns)->whereIn('id', $heading[$key])->first() ? '' : 'display:none;' }}">
                                    <div class="col-sm-12">
                                        <a href="javascript:void(0);" onclick="showColumnBox({{ $key }})">@lang('app.edit')</a>&nbsp;
                                        <a href="javascript:void(0);" onclick="skipColumnBox({{ $key }})" id="skipButton_{{ $key }}">@lang('app.skip')</a>
                                    </div><!-- col-sm-12 -->
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    function requiredMatchAction() {
        ...
        if (requiredMatched.length == 0) {
            ...
            let str = requiredMatched.map(item => item.name).join(', ');
    ...
    function generateSelectList(columnID) {
        ...
        let text = '<select id="columnName_' + columnID + '" class="form-control select-picker mb-2 ' + skippedClass + '" name="columns[' + columnID + ']" data-live-search="true">' +
            '<option value="">@lang('app.selectAColumn')</option>';
