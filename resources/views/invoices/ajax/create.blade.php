                ...
                <div class="col-md-3">
                    <div class="form-group mb-lg-0 mb-md-0 mb-4">
                        ...
                        <x-forms.input-group>
                            ...
                            <input type="number" name="invoice_number" id="invoice_number" class="form-control height-35 f-15" value="{{ is_null($lastInvoice) ? 1 : $lastInvoice }}">
                        </x-forms.input-group>
                    </div>
                </div>
                        ...
                        <div class="select-others height-35 rounded">
                            <select class="form-control select-picker" name="currency_id" id="currency_id">
                                @foreach ($currencies as $currency)
                                    <option @if (isset($estimate)) @selected($currency->id == $estimate->currency_id)
                                @elseif (isset($invoice))
                                    @selected($currency->id == $invoice->currency_id)
                                @else
                                    @selected($currency->id == company()->currency_id) @endif value="{{ $currency->id }}"
            ...
            <div class="row px-lg-4 px-md-4 px-3 pt-3">
                    ...
                    @if (isset($client) && !is_null($client))
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" value="{{ $client->name_salutation }}" class="form-control height-35 f-15 readonly-background" readonly>
                            </div>
                        </div>
                                    ...
                                    <select class="form-control select-picker" data-live-search="true" data-size="8" name="project_id" id="project_id">
                                        ...
                                        @if (isset($invoice) && $invoice->client)
                                            @foreach ($invoice->client->projects as $item)
                                                <option @selected($invoice->project_id == $item->id) value="{{ $item->id }}" data-content="{!! '<strong>' . $item->project_short_code . '</strong> ' . $item->project_name !!}">
                                                    {{ $item->project_name }}</option>
                                            @endforeach
                                        @elseif (isset($estimate) && $estimate->client)
                                            @foreach ($estimate->client->projects as $item)
                                                <option @selected($estimate->project_id == $item->id) value="{{ $item->id }}" data-content="{!! '<strong>' . $item->project_short_code . '</strong> ' . $item->project_name !!}">
                                                    {{ $item->project_name }}</option>
                                            @endforeach
                                        @elseif (isset($isClient) && $isClient == true)
                                            @foreach ($client->projects as $item)
                                                <option @selected($client->project_id == $item->id) value="{{ $item->id }}" data-content="{!! '<strong>' . $item->project_short_code . '</strong> ' . $item->project_name !!}">
                                                    {{ $item->project_name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                ...
                @if ($linkInvoicePermission == 'all')
                    <div class="col-md-4">
                        <div class="form-group c-inv-select mb-4">
                            ...
                            <div class="select-others height-35 rounded">
                                <select class="form-control select-picker" data-live-search="true" data-size="8" name="bank_account_id" id="bank_account_id">
                                    <option value="">--</option>
                ...
                <div class="col-md-4">
                    <div class="form-group c-inv-select mb-4">
                        ...
                        <div class="select-others height-35 rounded">
                            <select class="form-control select-picker" data-live-search="true" data-size="8" name="invoice_payment_id" id="invoice_payment_id">
                                <option value="">--</option>
            ...
            <div class="row px-lg-4 px-md-4 px-3 py-3">
                ...
                <div class="col-md-4">
                    <div class="form-group c-inv-select mb-0">
                        <label class="f-14 text-dark-grey mb-12  w-100" for="usr">@lang('modules.invoices.billingAddress')</label>
                ...
                <div class="col-md-4">
                    <div class="form-group c-inv-select mb-lg-0 mb-md-0 mb-4">
                        <label class="f-14 text-dark-grey mb-12  w-100" for="usr">@lang('modules.invoices.shippingAddress') </label>
                        ...
                        <p class="d-none" id="add-shipping-field">
                            <textarea class="form-control f-14 pt-2" rows="3" placeholder="@lang('placeholders.address')" name="shipping_address" id="shipping_address">
@if (isset($invoice) && $invoice->client)
{!! nl2br($invoice->client->clientDetails->shipping_address) !!}
@endif
</textarea>
                            ...
                            <x-forms.input-group>
                                ...
                                @if ($addProductPermission == 'all' || $addProductPermission == 'added')
                                    @if (module_enabled('Purchase') && in_array('purchase', user_modules()))
                                        <x-slot name="append">
                                            <a href="{{ route('purchase-products.create') }}" data-redirect-url="no" class="btn btn-outline-secondary border-grey openRightModal" data-toggle="tooltip" data-original-title="{{ __('app.add') . ' ' . __('modules.dashboard.newproduct') }}">@lang('app.add')</a>
                                        </x-slot>
                                    @else
                                        <x-slot name="append">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="border-bottom-0 btrr-mbl btlr">
                                                <input type="text" class="form-control f-14 border-0 w-100 item_name" name="item_name[]" placeholder="@lang('modules.expenses.itemName')" value="{{ $item->item_name }}">
                                            </td>
                                            ...
                                            @if ($invoiceSetting->hsn_sac_code_show)
                                                <td class="border-bottom-0">
                                                    <input type="text" class="form-control f-14 border-0 w-100 text-right hsn_sac_code" value="{{ $item->hsn_sac_code }}" name="hsn_sac_code[]">
                                                </td>
                                            @endif
                                            <td class="border-bottom-0">
                                            ...
                                            <td class="border-bottom-0">
                                                <input type="number" class="f-14 border-0 w-100 text-right cost_per_item form-control" placeholder="0.00" value="{{ $item->unit_price }}" name="cost_per_item[]" min="1">
                                            </td>
                                            <td class="border-bottom-0">
                                                <div class="select-others height-35 rounded border-0">
                                                    <select id="multiselect" name="taxes[{{ $key }}][]" multiple="multiple" class="select-picker type customSequence border-0" data-size="3">
                                                        @foreach ($taxes as $tax)
                                                            <option data-rate="{{ $tax->rate_percent }}" data-tax-text="{{ $tax->tax_name . ':' . $tax->rate_percent }}%" @selected(isset($item->taxes) && array_search($tax->id, json_decode($item->taxes)) !== false) value="{{ $tax->id }}">
                                                                {{ $tax->tax_name }}:
                                        ...
                                        <tr class="d-none d-md-block d-lg-table-row">
                                            ...
                                            <td class="border-left-0">
                                                <input type="file" class="dropify itemImage" name="invoice_item_image[]" id="image{{ $item->id }}" data-index="{{ $loop->index }}" data-allowed-file-extensions="png jpg jpeg bmp" data-item-id="image{{ $item->id }}" data-default-file="{{ $item->invoiceItemImage ? $item->invoiceItemImage->file_url : '' }}" data-height="70" />
                                ...
                                <table width="100%">
                                    <tbody>
                                        ...
                                        <tr class="d-none d-md-block d-lg-table-row">
                                            ...
                                            <td class="border-left-0">
                                                @if (isset($type) && $type == 'proposal')
                                                    <input type="hidden" id="imageId_{{ $item->id }}" class="itemOldImage" name="image_id[]" value={{ isset($item->proposalItemImage->id) ? $item->proposalItemImage->id : '' }} />

                                                    <input type="file" class="dropify itemImage" name="invoice_item_image[]" id="image{{ $item->id }}" data-index="{{ $loop->index }}" data-allowed-file-extensions="png jpg jpeg bmp" data-item-id="{{ $item->id }}" data-default-file="{{ $item->proposalItemImage ? $item->proposalItemImage->file_url : '' }}" data-height="70"
                                                        multiple />
                                                ...
                                                @else
                                                    ...
                                                    <input type="file" class="dropify itemImage" name="invoice_item_image[]" id="image{{ $item->id }}" data-index="{{ $loop->index }}" data-allowed-file-extensions="png jpg jpeg bmp" data-item-id="{{ $item->id }}" data-default-file="{{ $item->estimateItemImage ? $item->estimateItemImage->file_url : '' }}" data-height="70"
                                                        multiple />
                            ...
                            <table width="100%">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="border-bottom-0 d-block d-lg-none d-md-none">
                                            <textarea class="form-control f-14 border-0 w-100 mobile-description form-control" name="item_summary[]" placeholder="@lang('placeholders.invoices.description')"></textarea>
                                        </td>
                                        @if ($invoiceSetting->hsn_sac_code_show)
                                            <td class="border-bottom-0">
                                                <input type="text" class="form-control f-14 border-0 w-100 text-right hsn_sac_code" placeholder="" name="hsn_sac_code[]">
                                            </td>
                                        @endif
                                        <td class="border-bottom-0">
                                        ...
                                        <td class="border-bottom-0">
                                            <div class="select-others height-35 rounded border-0">
                                                <select id="multiselect" name="taxes[0][]" multiple="multiple" class="select-picker type customSequence border-0" data-size="3">
                                                    @foreach ($taxes as $tax)
                                                        <option data-rate="{{ $tax->rate_percent }}" data-tax-text="{{ $tax->tax_name . ':' . $tax->rate_percent }}%" value="{{ $tax->id }}">{{ $tax->tax_name }}:
                                                            {{ $tax->rate_percent }}%</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    <tr class="d-none d-md-table-row d-lg-table-row">
                                        <td colspan="{{ $invoiceSetting->hsn_sac_code_show ? '4' : '3' }}" class="dash-border-top bblr border-right-0">
                                            <textarea class="f-14 border p-3 rounded w-100 desktop-description form-control" name="item_summary[]" placeholder="@lang('placeholders.invoices.description')"></textarea>
                                        </td>
                                        <td class="border-left-0">
                                ...
                                <table width="100%">
                                    <tbody>
                                        ...
                                        <tr>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td width="70%" class="c-inv-sub-padding">
                                                                <input type="number" min="0" name="discount_value" class="form-control f-14 border-0 w-100 text-right discount_value" placeholder="0" value= "{{ isset($estimate) ? $estimate->discount : (isset($invoice) ? $invoice->discount : '0') }}">
                                                            </td>
                                            ...
                                            <td><span id="discount_amount">{{ isset($invoice) ? number_format((float) $invoice->discount, 2, '.', '') : '0.00' }}</span>
                                            </td>
                                        </tr>
                                        <tr>
            ...
            <div class="d-flex flex-wrap px-lg-4 px-md-4 px-3 py-3">
                <div class="col-md-6 col-sm-12 c-inv-note-terms p-0 mb-lg-0 mb-md-0 mb-3">
                    ...
                    <textarea class="form-control" name="note" id="note" rows="4" placeholder="@lang('placeholders.invoices.note')">{{ isset($estimate) ? $estimate->note : (isset($invoice) ? $invoice->note : '') }}</textarea>
                </div>
            ...
            <div class="d-flex px-lg-4 px-md-4 px-3 py-2 bg-light-grey">
                ...
                <div class="col-md-3 payment-types d-none">
                    <x-forms.select fieldId="payment_gateway_id" :fieldLabel="__('modules.payments.paymentGateway')" fieldName="gateway" search="true" fieldRequired="true">
                        <option value="">--</option>
                ...
                <div class="col-md-3 d-none" id="add_offline">
                    <x-forms.select fieldId="add_offline_methods" :fieldLabel="__('modules.payments.offlinePaymentMethod')" fieldName="offline_methods" search="true" fieldRequired="true">
                    </x-forms.select>
                </div>
            ...
            <x-form-actions class="c-inv-btns d-block d-lg-flex d-md-flex">
                <div class="d-flex mb-3 mb-lg-0 mb-md-0">
                    ...
                    <div class="inv-action dropup mr-3">
                        <button class="btn-primary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            @lang('app.save')
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuBtn" tabindex="0">
                            ...
                            <li>
                                <a class="dropdown-item f-14 text-dark save-form" href="javascript:void(0);" data-type="send">
                                    <i class="fa fa-paper-plane f-w-500  mr-2 f-12"></i> @lang('app.saveSend')
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item f-14 text-dark save-form" href="javascript:void(0);" data-type="mark_as_send" data-toggle="tooltip" data-original-title="@lang('messages.markSentInfo')">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function initRemoteBootstrapSelect(selectSelector, endpointUrl, optionBuilder, baseOptionHtml, extraParamsProvider) {
                ...
                function load(term, page, appendMode) {
                    ...
                    var extraParams = typeof extraParamsProvider === 'function' ? (extraParamsProvider() || {}) : {};
            ...
            function changeProductCategory(url) {
                ...
                window.apiHttp.get("{{ route('invoices.search_products') }}", {
                    params: {
            ...
            initRemoteBootstrapSelect('#client_list_id', "{{ route('invoices.search_clients') }}", function(item, selected) {
        ...
        function saveForm(type, exceed) {
            ...
            var storeUrl = "{{ route('invoices.store') }}" + "?type=" + type;
