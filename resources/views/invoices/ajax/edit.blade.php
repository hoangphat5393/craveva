...
@else
        ...
        <x-form class="c-inv-form" id="saveInvoiceForm">
            ...
            <div class="row px-lg-4 px-md-4 px-3 py-3">
                ...
                <div class="col-md-2">
                    <div class="form-group mb-lg-0 mb-md-0 mb-4">
                        ...
                        <x-forms.input-group>
                            <input type="text" name="invoice_number" id="invoice_number" class="form-control height-35 f-15 readonly-background" readonly value="{{ $invoice->invoice_number }}">
                        </x-forms.input-group>
                    </div>
                </div>
                <!-- INVOICE NUMBER END -->
                ...
                <div class="col-md-2">
                    <div class="form-group mb-lg-0 mb-md-0 mb-4">
                        ...
                        <div class="input-group">
                            <input type="text" id="invoice_date" name="issue_date" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="@lang('placeholders.date')" value="{{ $invoice->issue_date->translatedFormat(company()->date_format) }}">
                        </div>
                    </div>
                </div>
                <!-- INVOICE DATE END -->
                ...
                <div class="col-md-2">
                    <div class="form-group mb-lg-0 mb-md-0 mb-4">
                        ...
                        <div class="input-group ">
                            <input type="text" id="due_date" name="due_date" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="@lang('placeholders.date')" value="{{ $invoice->due_date->translatedFormat(company()->date_format) }}">
                        </div>
                    </div>
                </div>
                <!-- DUE DATE END -->
                        ...
                        <div class="select-others height-35 rounded">
                            <select class="form-control select-picker" name="currency_id" id="currency_id">
                                @foreach ($currencies as $currency)
                                    <option @if ($invoice->currency_id == $currency->id) selected @endif value="{{ $currency->id }}" data-currency-code="{{ $currency->currency_code }}">
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" id="exchange_rate" name="exchange_rate" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" value="{{ $invoice->exchange_rate }}" @if ($companyCurrency->id == $invoice->currency_id) readonly @endif>
                    <small id="currency_exchange" class="form-text text-muted">
                        @if (company()->currency->currency_code != $invoice->currency->currency_code)
                            ( {{ $invoice->currency->currency_code }} @lang('app.to') {{ company()->currency->currency_code }} )
                        @endif
                    </small>
                </div>
            </div>
                                ...
                                <select class="form-control select-picker" data-live-search="true" data-size="8" name="bank_account_id" id="bank_account_id">
                                    ...
                                    @if ($viewBankAccountPermission != 'none')
                                        @foreach ($bankDetails as $bankDetail)
                                            <option value="{{ $bankDetail->id }}" @if ($bankDetail->id == $invoice->bank_account_id) selected @endif>
                ...
                <div class="col-md-4">
                    <div class="form-group c-inv-select mb-lg-0 mb-md-0 mb-4">
                        ...
                        <p class="d-none" id="add-shipping-field">
                            <textarea class="form-control f-14 pt-2" rows="3" placeholder="@lang('placeholders.address')" name="shipping_address" id="shipping_address"></textarea>
                        </p>
                    </div>
                </div>
                            ...
                            <table width="100%">
                                <tbody>
                                    <tr class="text-dark-grey font-weight-bold f-14">
                                        <td width="{{ $invoiceSetting->hsn_sac_code_show ? '40%' : '50%' }}" class="border-0 inv-desc-mbl btlr">@lang('app.description')
                                    ...
                                    <tr>
                                        ...
                                        @if ($invoiceSetting->hsn_sac_code_show)
                                            <td class="border-bottom-0">
                                                <input type="text" class="f-14 border-0 w-100 text-right hsn_sac_code form-control" value="{{ $item->hsn_sac_code }}" name="hsn_sac_code[]">
                                            </td>
                                        @endif
                                        <td class="border-bottom-0">
                                            ...
                                            @else
                                                <select class="text-dark-grey float-right border-0 f-12" name="unit_id[]">
                                                    @foreach ($units as $unit)
                                                        <option @if ($item->unit_id == $unit->id) selected @endif value="{{ $unit->id }}">{{ $unit->unit_type }}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <td class="border-bottom-0">
                                            <input type="number" min="1" class="f-14 border-0 w-100 text-right cost_per_item form-control" placeholder="0.00" value="{{ $item->unit_price }}" name="cost_per_item[]">
                                        </td>
                                        <td class="border-bottom-0">
                                    ...
                                    <tr class="d-none d-md-block d-lg-table-row">
                                        ...
                                        <td class="border-left-0">
                                            <input type="file" class="dropify" name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg bmp" data-messages-default="test" data-height="70" data-id="{{ $item->id }}" id="{{ $item->id }}" data-default-file="{{ $item->invoiceItemImage ? $item->invoiceItemImage->file_url : '' }}"
                                                @if ($item->invoiceItemImage && $item->invoiceItemImage->external_link) data-show-remove="false" @endif />
                                ...
                                <table width="100%">
                                    <tbody>
                                        ...
                                        <tr>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td width="70%" class="c-inv-sub-padding">
                                                                <input type="number" min="0" name="discount_value" class="form-control f-14 border-0 w-100 text-right discount_value" placeholder="0" value="{{ $invoice->discount }}">
                                                            </td>
                                            ...
                                            <td><span id="discount_amount">{{ number_format((float) $invoice->discount, 2, '.', '') }}</span>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr class="bg-amt-grey f-16 f-w-500">
                                            ...
                                            <td><span class="total">{{ number_format((float) $invoice->total, 2, '.', '') }}</span>
                                            </td>
            ...
            <div class="d-flex flex-wrap px-lg-4 px-md-4 px-3 py-3">
                <div class="col-md-6 col-sm-12 c-inv-note-terms p-0 mb-lg-0 mb-md-0 mb-3">
                    ...
                    <textarea class="form-control" name="note" id="note" rows="4" placeholder="@lang('placeholders.invoices.note')">{{ $invoice->note }}</textarea>
                </div>
            ...
            <div class="d-flex px-lg-4 px-md-4 px-3 py-2 bg-light-grey">
                ...
                <div class="col-md-3 @if ($invoice->offline_methods == null || $invoice->payment_status != 1) d-none @endif " id="add_offline">
                    <x-forms.select fieldId="offline_method_id" :fieldLabel="__('modules.payments.offlinePaymentMethod')" fieldName="offline_methods" search="true" fieldRequired="true">
                ...
                <div class="col-md-3 payment-types @if ($invoice->payment_status != 1) d-none @endif">
                    <x-forms.text fieldId="transaction_id" :fieldLabel="__('modules.payments.transactionId')" fieldName="transaction_id" :fieldPlaceholder="__('placeholders.payments.transactionId')" :fieldValue="$invoice->transaction_id" />
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#saveInvoiceForm .order-line-unit-select').selectpicker();
