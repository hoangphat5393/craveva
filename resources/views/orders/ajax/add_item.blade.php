            ...
            <tbody>
                ...
                <tr>
                    <td class="border-bottom-0 btrr-mbl btlr">
                        ...
                        <input type="text" class="f-14 border-0 w-100 item_name bg-additional-grey" readonly name="item_name[]" placeholder="@lang('modules.expenses.itemName')" value="{{ $item->name }}">
                    </td>
                    @if ($invoiceSetting->hsn_sac_code_show)
                    ...
                    <td class="border-bottom-0 d-block d-lg-none d-md-none">
                        <input type="text" readonly class="f-14 border-0 w-100 mobile-description bg-additional-grey" placeholder="@lang('placeholders.invoices.description')" name="item_summary[]" value="{{ strip_tags($item->description) }}">
                    </td>
                    <td class="border-bottom-0">
                    ...
                    <td class="border-bottom-0">
                        <input type="number" min="1" class="f-14 border-0 w-100 text-right cost_per_item bg-additional-grey" placeholder="0.00" value="{{ $item->price }}" name="cost_per_item[]" {{ $user->isAdmin($user->id) ? '' : 'readonly' }}>
                    </td>
                    <td class="border-bottom-0">
                        ...
                        <div class="select-others height-35 d-none rounded border-0">
                            <select id="multiselect" name="taxes[0][]" multiple="multiple" class="select-picker type customSequence border-0 bg-additional-grey" data-size="3">
                    ...
                    <td rowspan="2" align="right" valign="top" class="bg-amt-grey btrr-bbrr">
                        <span class="amount-html">{{ number_format((float) (1 * $item->price), 2, '.', '') }}</span>
                        <input type="hidden" class="amount" name="amount[]" value="{{ number_format((float) (1 * $item->price), 2, '.', '') }}">
                    </td>
                </tr>
