...
<script>
    @if ($credentials->stripe_status == 'active')
        ...
        var makeInvoice = function(paymentIntent) {
            window.apiHttp.post("{{ route('orders.make_invoice', [$order->id]) }}", {
        ...
        var orderComplete = function(paymentIntent) {
            loading(false);
            cardButton.disabled = true;
            window.apiHttp.post("{{ route('stripe', [$order->id]) }}", {
        ...
        var paymentFailed = function(result) {
            window.apiHttp.post("{{ route('orders.payment_failed', [$order->id]) }}", {
