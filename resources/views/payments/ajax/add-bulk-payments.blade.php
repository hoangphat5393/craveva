                            ...
                            <thead lass="thead-light">
                                <tr>
                                    ...
                                    @if ($linkPaymentPermission == 'all')
                                        <td>@lang('app.menu.bankaccount')</td>
                                    @endif
                                    ...
                                    <tr>
                                        ...
                                        <td class="border-bottom-0 btrr-mbl btlr">
                                            <div class="input-group">
                                                <input type="text" data-id="{{ $key }}" id="payment_date{{ $key }}" name="payment_date[]" class="payment_date px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15 w-100" placeholder="@lang('placeholders.date')" value="{{ now(company()->timezone)->format(company()->date_format) }}">
                                            </div>
                                        </td>
                                        <td class="border-bottom-0 btrr-mbl btlr">
                                            <div class="input-group">
                                                <select name="gateway[]" data-id={{ $key }} id="payment_gateway_id{{ $key }}" class="form-control select-picker payment_gateway_id" data-live-search="true" search="true">
                                        ...
                                        <td class="border-bottom-0 btrr-mbl btlr">
                                            <div class="input-group" id="add_offline{{ $key }}">
                                                <select class="form-control select-picker add_offline_methods" id="add_offline_methods{{ $key }}" data-id="{{ $key }}" name="offline_methods[]" data-live-search="true" search="true">
                                                    <option value="">--</option>
                                                    @foreach ($offlineMethods as $offlineMethod)
                                                        <option value="{{ $offlineMethod->id }}">
                                                            {{ $offlineMethod->name }}</option>
                                            ...
                                            <input type="hidden" id="offline_method_id{{ $key }}" name="offline_method_id[]" value="">
                                        </td>
                                        @if ($linkPaymentPermission == 'all')
                                            <td class="border-bottom-0 btrr-mbl btlr">
                                                <div class="input-group" id="bank_account_id{{ $key }}">
                                                    <select class="form-control select-picker bank_account_id" id="bank_account_id{{ $key }}" data-id="{{ $key }}" name="bank_account_id[]" data-live-search="true" search="true" @if ($pendingPayment->bank_account_id) disabled @endif>
                                                        <option value="">--</option>
                                                        @if ($viewBankAccountPermission != 'none')
                                                            @foreach ($bankDetails as $bankDetail)
                                                                @if ($pendingPayment->currency->id == $bankDetail->currency_id)
                                                                    <option @selected($pendingPayment->bank_account_id == $bankDetail->id) value="{{ $bankDetail->id }}">
                                        ...
                                        <td class="border-bottom-0 btrr-mbl btlr">
                                            <div class="input-group">
                                                <input type="text" class="form-control height-35 f-14" name="transaction_id[]" id="transaction_id">
                                            </div>
                                        </td>
                                        <td class="border-bottom-0 btrr-mbl btlr">
                                            <div class="input-group">
                                                <input type="number" class="form-control height-35 f-14 amount" name="amount[]" id="amount{{ $key }}" data-id="{{ $key }}">
                                            </div>
                                        </td>
                                        <td class="border-bottom-0 btrr-mbl btlr text-right pr-0">
                                            <input type="hidden" id="due_amount{{ $key }}" value="{{ $pendingPayment->amountDue() }}">
...
<script>
    $(document).ready(function() {
        ...
        const showTable = () => {
            ...
            $.easyBlockUI('#save-bulk-payment-form');
            window.apiHttp.get(url).then(function(response) {
                ...
                $.easyUnblockUI('#save-bulk-payment-form');
            });
        };
        ...
        $(document).on('click', '#save-bulk-payment-button', function() {
            ...
            var $bulkBtn = $('#save-bulk-payment-button');
            var bulkFormEl = document.getElementById('save-bulk-payment-form');
            $bulkBtn.prop('disabled', true);
            $.easyBlockUI('#save-bulk-payment-form');
            window.apiHttp.postUrlEncoded(url, $('#save-bulk-payment-form').serialize()).then(function(response) {
                if (response.status == 'success') {
