...
<div class="bg-white rounded b-shadow-4 create-inv">
    ...
    <div class="d-block d-lg-flex d-md-flex justify-content-between action-bar">
        ...
        <div class="px-lg-4 px-md-4 px-3 py-3 cart_empty">
            <x-forms.link-primary :link="route('products.empty_cart')" class="empty-cart" icon="trash">
                @lang('app.emptyCart')
                ...
                @foreach ($products as $key => $item)
                    ...
                    <div class="d-flex px-4 py-3 c-inv-desc item-row">
                        ...
                        <div class="c-inv-desc-table w-100 d-lg-flex d-md-flex d-block">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="border-bottom-0 btrr-mbl btlr">
                                            ...
                                            <input type="text" class="f-14 border-0 w-100 item_name bg-additional-grey" readonly name="item_name[]" placeholder="@lang('modules.expenses.itemName')" value="{{ $item->item_name }}">
                                        </td>
                                        @if ($invoiceSetting->hsn_sac_code_show)
                                            <td class="border-bottom-0">
                                                <span>{{ $item->hsn_sac_code }}</span>
                                                <input type="hidden" class="form-control f-14 border-0 w-100 text-right hsn_sac_code" value="{{ $item->hsn_sac_code }}" name="hsn_sac_code[]">
                                            </td>
                                        @endif
                                        <td class="border-bottom-0 d-block d-lg-none d-md-none">
                                            <input type="text" readonly class="f-14 border-0 w-100 mobile-description bg-additional-grey" placeholder="@lang('placeholders.invoices.description')" name="item_summary[]" value="{{ strip_tags($item->item_summary) }}">
                                        </td>

                                        <td class="border-bottom-0">
                                            <input type="number" min="1" class="f-14 border-0 w-100 text-right quantity mt-3" value="{{ $item->quantity }}" id="quantity" name="quantity[]">
                                            <span class="text-dark-grey float-right border-0 f-12">{{ $item->unit->unit_type }}</span>
                                            <input type="hidden" name="product_id[]" value="{{ $item->product_id }}">
                                            <input type="hidden" name="unit_id[]" value="{{ $item->unit_id }}">
                                        </td>
                                        <td class="border-bottom-0">
                                            <input type="number" min="1" class="f-14 border-0 w-100 text-right cost_per_item bg-additional-grey" placeholder="0.00" value="{{ $item->unit_price }}" name="cost_per_item[]" readonly>
                                        </td>
                                        <td class="border-bottom-0">
                                            <div class="select-others height-35 rounded border-0">
                                                <select id="multiselect" name="taxes[{{ $key }}][]" multiple="multiple" class="select-picker type customSequence border-0 bg-additional-grey tax" data-size="3">
                                                    @foreach ($taxes as $tax)
                                                        <option data-rate="{{ $tax->rate_percent }}" data-tax-text="{{ $tax->tax_name . ':' . $tax->rate_percent }}%" @if (isset($item->product->taxes) && array_search($tax->id, json_decode($item->product->taxes)) !== false) selected @endif value="{{ $tax->id }}">
                                                            {{ $tax->tax_name }}:
                                        ...
                                        <td rowspan="2" align="right" valign="top" class="bg-amt-grey btrr-bbrr">
                                            ...
                                            <input type="hidden" class="amount" name="amount[]" value="{{ number_format((float) $item->amount, 2, '.', '') }}">
                                        </td>
                                    </tr>
                                    <tr class="d-none d-md-block d-lg-table-row">
                                        <td colspan="{{ $invoiceSetting->hsn_sac_code_show ? '4' : '3' }}" class="dash-border-top bblr">
                                            <textarea type="text" readonly class="f-14 border-0 w-100 desktop-description" name="item_summary[]" placeholder="@lang('placeholders.invoices.description')">{{ strip_tags($item->description) }}</textarea>
                                        </td>
                            ...
                            <a href="javascript:;" class="d-flex align-items-center justify-content-center ml-3 remove-item" data-item-id="{{ $item->id }}"><i class="fa fa-times-circle f-20 text-lightest"></i></a>
                        </div>
                    </div>
                                ...
                                <table width="100%">
                                    <tbody>
                                        <tr>
                                            ...
                                            <input type="hidden" id="calculate_tax" name="calculate_tax" value="{{ $invoiceSetting->calculate_tax }}">
                                        </tr>
                                        <tr>
...
<script>
    $(document).ready(function() {
        ...
        $('#saveInvoiceForm').on('keyup', '.quantity', function() {
        ...
        $('#saveInvoiceForm').on('change', '.quantity', function() {
