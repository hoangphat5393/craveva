    ...
    <x-form class="c-inv-form" id="saveInvoiceForm">
        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
            ...
            @if (isset($deal) && !is_null($deal))
                <div class="col-md-6 col-lg-3">
                    ...
                    <x-forms.input-group>
                        <input type="text" readonly class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="{{ $deal->name }}" value="{{ $deal->name }}">
                    </x-forms.input-group>
            ...
            @else
                <div class="col-lg-4 ">
                    <x-forms.select fieldId="lead_contact" :fieldLabel="__('modules.leadContact.leadContacts')" search="true" fieldName="lead_contact" fieldRequired="true">
                        <option value="">--</option>
                    ...
                    <select name="deal_id" id="deal_id" class="form-control select-picker" data-live-search="true" data-size="8">
                        ...
                        @foreach ($deals as $clientOpt)
                            <option @if (isset($proposal) && $clientOpt->id == $proposal->deal->id) selected
                                    @elseif ($proposalTemplate && $clientOpt->id == $proposalTemplate->deal_id) selected @endif value="{{ $clientOpt->id }}">
            ...
            <div class="col-md-6 col-lg-3">
                <div class="form-group mb-lg-0 mb-md-0 mb-4 mt-3">
                    ...
                    <x-forms.input-group>
                        <input type="text" id="valid_till" name="valid_till" class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" placeholder="@lang('placeholders.date')" value="{{ Carbon\Carbon::today()->addDays(30)->format(company()->date_format) }}">
                    </x-forms.input-group>
                </div>
            </div>
            <!-- INVOICE DATE END -->
            ...
            <div class="col-md-6 col-lg-3">
                <x-forms.select :fieldLabel="__('modules.invoices.currency')" fieldName="currency_id" field_id="currency_id">
                    @foreach ($currencies as $currency)
                        <option @if (isset($proposal)) @selected($currency->id == $proposal->currency_id)
                            @else @selected($currency->id == company()->currency_id)
                                @selected(isset($proposalTemplate) && $currency->id == $proposalTemplate->currency_id) @endif value="{{ $currency->id }}">
            ...
            <!-- FREQUENCY START -->
            <div class="col-md-6">
                <x-forms.checkbox :fieldLabel="__('modules.proposal.requireSignature')" fieldName="require_signature" fieldId="require_signature" fieldValue="true" checked="true" />
            </div>
            <!-- FREQUENCY END -->
                                    ...
                                    <tr>
                                        ...
                                        @if ($invoiceSetting->hsn_sac_code_show)
                                            <td class="border-bottom-0">
                                                <input type="text" min="1" class="f-14 border-0 w-100 text-right hsn_sac_code form-control" value="{{ $item->hsn_sac_code }}" name="hsn_sac_code[]">
                                            </td>
                                        @endif
                                        <td class="border-bottom-0">
                                            <input type="number" min="1" class="f-14 border-0 w-100 text-right quantity form-control mt-3" value="{{ $item->quantity }}" name="quantity[]">
                            ...
                            <table width="100%">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @endif
                                        <td class="border-bottom-0">
                                            <input type="number" min="1" class="f-14 border-0 w-100 text-right quantity form-control mt-3" name="quantity[]" value="{{ $item->quantity }}">
                                            @if (!is_null($item->product_id) && $item->product_id != 0)
                                                <span class="text-dark-grey float-right border-0 f-12">{{ $item->unit->unit_type }}</span>
                                                <input type="hidden" name="product_id[]" value="{{ $item->product_id }}">
                                                <input type="hidden" name="unit_id[]" value="{{ $item->unit_id }}">
                                        ...
                                        <td class="border-bottom-0">
                                            <input type="number" min="1" class="f-14 border-0 w-100 text-right cost_per_item form-control" placeholder="0.00" name="cost_per_item[]" value="{{ $item->unit_price }}">
                                        </td>
                                        <td class="border-bottom-0">
                                            <div class="select-others height-35 rounded border-0">
                                                <select id="multiselect" name="taxes[{{ $key }}][]" multiple="multiple" class="select-picker type customSequence border-0" data-size="3">
                                    ...
                                    <tr class="d-none d-md-table-row d-lg-table-row">
                                        ...
                                        <td class="border-left-0">
                                            <input type="hidden" id="imageId_{{ $item->id }}" class="itemOldImage" name="image_id[]" value={{ isset($item->proposalTemplateItemImage->id) ? $item->proposalTemplateItemImage->id : '' }} />
                                            <input type="file" class="dropify" name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg bmp" data-messages-default="test" data-height="70" data-id="{{ $item->id }}" id="{{ $item->id }}" data-default-file="{{ $item->proposalTemplateItemImage ? $item->proposalTemplateItemImage->file_url : '' }}"
                                                @if ($item->proposalTemplateItemImage && $item->proposalTemplateItemImage->external_link) data-show-remove="false" @endif />
                        ...
                        <table width="100%">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @if ($invoiceSetting->hsn_sac_code_show)
                                        <td class="border-bottom-0">
                                            <input type="text" class="f-14 border-0 w-100 text-right hsn_sac_code form-control" value="" name="hsn_sac_code[]">
                                        </td>
                                    @endif
                                    <td class="border-bottom-0">
                            ...
                            <table width="100%">
                                <tbody>
                                    ...
                                    <tr>
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td width="70%" class="c-inv-sub-padding">
                                                            <input type="number" min="0" name="discount_value" class="f-14 border-0 w-100 text-right discount_value" placeholder="0" value="{{ isset($proposal) ? number_format((float) $proposal->discount, 2, '.', '') : ($proposalTemplate ? number_format((float) $proposalTemplate->discount, 2, '.', '') : '0.00') }}">
                                                        </td>
                                                        <td width="30%" align="left" class="c-inv-sub-padding">
                                                            <div class="select-others select-tax height-35 rounded border-0">
                                                                <select class="form-control select-picker" id="discount_type" name="discount_type">
                                                                    <option @if (isset($proposal) && $proposal->discount_type == 'percent') selected
                                                            @elseif(isset($proposalTemplate) && $proposalTemplate->discount_type == 'percent') selected @endif value="percent">%
                                                                    </option>
                                                                    <option @if (isset($proposal) && $proposal->discount_type == 'fixed') selected
                                                            @elseif (isset($proposalTemplate) && $proposalTemplate->discount_type == 'fixed') selected @endif value="fixed">
                                        ...
                                        <td>
                                            <span id="discount_amount">{{ isset($proposal) ? number_format((float) $proposal->discount, 2, '.', '') : ($proposalTemplate ? number_format((float) $proposalTemplate->discount, 2, '.', '') : '0.00') }}
                                            </span>
                                        </td>
                                    </tr>
                                    <tr>
        ...
        <div class="d-flex flex-wrap px-lg-4 px-md-4 px-3 py-3">
            <div class="col-md-6 col-sm-12 c-inv-note-terms p-0 mb-lg-0 mb-md-0 mb-3">
                ...
                <textarea class="form-control" name="note" id="lead_note" rows="4" placeholder="@lang('placeholders.invoices.note')"></textarea>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function addProduct(id) {
            ...
            $.easyBlockUI('#saveInvoiceForm');
            window.apiHttp.get("{{ route('proposals.add_item') }}", {
                params: {
                    id: id,
        ...
        $('.save-form').click(function() {
            ...
            var storeUrl = "{{ route('proposals.store') }}" + "?type=" + type;
            $.easyBlockUI('#saveInvoiceForm');
            window.apiHttp.postUrlEncoded(storeUrl, $('#saveInvoiceForm').serialize()).then(function(response) {
        ...
        // Volume Discount System Integration
        let discountTimer;
        ...
        function applyVolumeDiscount() {
            ...
            $('.item-row').each(function() {
                ...
                let p = parseFloat(row.find('.cost_per_item').val()) || 0;
                let pid = row.find('input[name="product_id[]"]').val();
            ...
            window.apiHttp.post("{{ route('discount.calculate') }}", {
                ...
                if (response.status == 'success' && response.global_discount) {
                    if (response.global_discount.value > 0) {
                        $('#discount_type').val('fixed').selectpicker('refresh');
                        $('.discount_value').val(response.global_discount.value);
        ...
        $('#saveInvoiceForm').on('keyup change', '.quantity, .cost_per_item', function() {
            clearTimeout(discountTimer);
            discountTimer = setTimeout(applyVolumeDiscount, 800);
        });

        // Trigger when a product is added (wait for DOM update)
        $('#add-products').on('changed.bs.select', function() {
            setTimeout(applyVolumeDiscount, 2000); // Wait for the AJAX add_item to finish
        });
    });
</script>
