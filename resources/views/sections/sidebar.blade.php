...
<aside class="{{ !user()->dark_theme ? 'sidebar-' . $appTheme->sidebar_theme : '' }}">
    ...
    @php
        ...
        $showCompanySettingsIcon = $canManageCompanySetting || $isAdminRole || user()->is_superadmin;
        $companySettingsLink = $canManageCompanySetting || $isAdminRole || user()->is_superadmin ? route('company-settings.index') : route('profile-settings.index');
    @endphp
    <!-- MAIN SIDEBAR START -->
    <div class="main-sidebar" id="mobile_menu_collapse">
            ...
            <div class="dropdown-toggle sidebar-brand d-flex align-items-center justify-content-between  w-100" type="link" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                @if (companyOrGlobalSetting()->sidebar_logo_style !== 'full')
                    ...
                    <div class="sidebar-brand-name">
                        <h1 class="mb-0 f-16 f-w-500 text-white-shade mt-0" data-placement="bottom" data-toggle="tooltip" data-original-title="{{ $appName }}">{{ $appName }}
                        ...
                        <div class="mb-0 position-relative pro-name">
                            ...
                            <p class="f-13 text-lightest mb-0" data-placement="bottom" data-toggle="tooltip" data-original-title="{{ $userName }}">{{ $userName }}</p>
                        </div>
                    </div>
                ...
                @else
                    <!-- SIDEBAR BRAND NAME START -->
                    <div class="sidebar-brand-name">
                        <h1 class="mb-0 f-16 f-w-500 text-white-shade mt-0" data-placement="bottom" data-toggle="tooltip" data-original-title="{{ $appName }}">
                            <img src="{{ companyOrGlobalSetting()->logo_url }}">
                        </h1>
                    </div>
                    <!-- SIDEBAR BRAND NAME END -->
            ...
            <div class="dropdown-menu dropdown-menu-right sidebar-brand-dropdown ml-3" aria-labelledby="dropdownMenuLink" tabindex="0">
                <div class="d-flex justify-content-between align-items-center profile-box">
                    <a @if (in_array('client', user_roles())) href="{{ route('profile-settings.index') }}" @elseif (user()->is_superadmin) href="{{ route('superadmin.settings.super-admin-profile.index') }}" @else href="{{ route('employees.show', user()->id) }}" @endif>
                        <div class="profileInfo d-flex align-items-center mr-1 flex-wrap">
                            <div class="profileImg mr-2">
                                <img class="h-100" src="{{ $user->image_url }}" alt="{{ $userName }}">
                            </div>
                            <div class="ProfileData">
                                <h3 class="f-15 f-w-500 text-dark" data-placement="bottom" data-toggle="tooltip" data-original-title="{{ $userName }}">{{ $userName }}</h3>
                    ...
                    @if (user()->is_superadmin)
                        <a href="{{ route('superadmin.settings.super-admin-profile.index') }}" data-toggle="tooltip" data-original-title="{{ __('app.menu.profileSettings') }}">
                            <i class="side-icon bi bi-pencil-square"></i>
                        </a>
                    @else
                        <a href="{{ route('profile-settings.index') }}" data-toggle="tooltip" data-original-title="{{ __('app.menu.profileSettings') }}">
                            <i class="side-icon bi bi-pencil-square"></i>
                        </a>
                    @endif
                </div>
                ...
                @if ($showCompanySettingsIcon)
                    <a class="dropdown-item d-flex justify-content-between align-items-center f-15 text-dark" href="{{ $companySettingsLink }}">
                ...
                <a class="dropdown-item d-flex justify-content-between align-items-center f-15 text-dark" href="{{ route('logout') }}" onclick="event.preventDefault();
        ...
        <!-- Sidebar Toggler -->
        <div class="text-center d-flex justify-content-between align-items-center position-fixed sidebarTogglerBox {{ user()->dark_theme ? 'bg-dark' : '' }}">
            ...
            <div class="d-flex align-items-center">
                ...
                <p class="mb-0">
                    <a href="{{ $companySettingsLink }}" class="text-secondary ml-2 f-15" data-toggle="tooltip" data-original-title="{{ $showCompanySettingsIcon ? __('app.menu.accountSettings') : __('app.menu.profileSettings') }}">
                        <i class="fa fa-cog"></i>
                    </a>
                </p>
