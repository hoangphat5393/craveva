        ...
        <x-form id="update-company-package-form" method="PUT">
            ...
            <div class="add-company bg-white rounded">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-4">
                        <x-forms.select fieldId="package" :fieldLabel="__('superadmin.packages.packages')" search fieldName="package">
                            <option value=""> @lang('superadmin.packages.selectPackage')</option>
                            @foreach ($allPackages as $package)
                                <option value="{{ $package->id }}" data-type="{{ $package->type }}" data-days="{{ $package->days }}" data-default="{{ $package->default }}" {{ $company->package_id == $package->id ? 'selected' : '' }}
                                    @if ($package->default == 'trial') @if ($packageSetting->status == 'inactive')
                                            data-content="{{ $package->name }}
                                                <span class='badge badge-pill badge-light border'>{{ __('app.inactive') }}</span>" @endif @endif>
                    ...
                    <div class="col-md-4 payment-field">
                        <x-forms.datepicker fieldId="pay_date" fieldRequired="true" :fieldLabel="__('superadmin.paymentDate')" fieldName="pay_date" :fieldPlaceholder="__('placeholders.date')" />
                    </div>

                    <div class="col-md-4 payment-field nextPaydateField">
                        <x-forms.datepicker fieldId="next_pay_date" :fieldLabel="__('superadmin.nextPaymentDate')" fieldName="next_pay_date" :fieldPlaceholder="__('placeholders.date')" />
                    </div>

                    <div class="col-md-4 payment-field licenceExpire">
                        <x-forms.text fieldId="licence_expire_on" :fieldLabel="__('superadmin.packages.licenseExpiresOn')" fieldName="licence_expire_on" :fieldPlaceholder="__('placeholders.date')" />
                    </div>

                    <div class="col-md-4 trial_expire d-none">
                        <x-forms.text fieldId="trial_expire_on" :fieldLabel="__('superadmin.packages.trialExpiresOn')" fieldName="trial_expire_on" :fieldPlaceholder="__('placeholders.date')" />
                    </div>
                </div>
                <x-form-actions>
...
<script>
    $(document).ready(function() {
        ...
        function handlePackageChange() {
            ...
            if (packageInfo[id] && type) {
                $('#amount').val(packageInfo[id][type]);
            }
            ...
            if (def === 'trial') {
                ...
                if (def === 'lifetime') {
                    ...
                    $('.nextPaydateField').removeClass('d-none');
                    $('.licenceExpire').removeClass('d-none');

                    if ($('#pay_date').val() !== '' && days != null) {
                        let startDate = moment($("#pay_date").val(), '{{ $global->moment_date_format }}');
                        let endDate = startDate.add(days, 'days');
