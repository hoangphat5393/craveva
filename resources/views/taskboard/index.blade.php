...
@section('content')
    ...
    <div class="w-task-board-box px-4 py-2 bg-white">
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar my-3">
            ...
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if ($addTaskPermission == 'all' || $addTaskPermission == 'added')
                    <x-forms.link-primary :link="route('tasks.create')" class="mr-lg-3 mr-1 openRightModal float-left" icon="plus"
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                ...
                <a href="{{ route('taskboards.index') }}" class="btn btn-secondary f-14 btn-active"
                    data-toggle="tooltip" data-original-title="@lang('modules.tasks.taskBoard')"><i
                        class="side-icon bi bi-kanban"></i></a>
                ...
                @if (in_array('admin', user_roles()) || in_array('employee', user_roles()))
                    <a href="{{ route('tasks.waiting-approval') }}"
                        class="btn btn-secondary f-14 show-waiting-approval-task" data-toggle="tooltip"
                        data-original-title="@lang('app.menu.waiting-approval')">
                        <svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" fill="#000000" width="18"
                            height="18">
                        ...
                        @if ($waitingApprovalCount > 0)
                            <span
                                class="badge badge-pill badge-danger position-absolute">{{ $waitingApprovalCount }}</span>
                        @endif
                    </a>
                @endif
            </div>
        ...
        <x-alert type="warning">
            <div><b>Note:</b>@lang('modules.tasks.taskboardNote') <a href="{{ route('tasks.index') }}">@lang('app.tasks')</a>
                @lang('modules.tasks.menu') </div>
        </x-alert>
    ...
    <script>
        ...
        function loadData() {
            ...
            var url = "{{ route('taskboards.index') }}?startDate=" + encodeURIComponent(startDate) + '&endDate=' +
                ...
                '&searchText=' + searchText + '&billable=' + billable + '&milestone_id=' + milestone_id + '&priority=' +
                priority +
        ...
        //pusher
        if ((pusher_setting.status === 1 && pusher_setting.taskboard === 1) || (pusher_setting.status == "1" &&
                pusher_setting.taskboard == "1")) {
