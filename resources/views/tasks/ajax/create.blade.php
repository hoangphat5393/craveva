            ...
            <div class="add-client bg-white rounded">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-lg-6 col-md-6">
                        <x-forms.text :fieldLabel="__('app.title')" fieldName="heading" fieldRequired="true" fieldId="heading" :fieldPlaceholder="__('placeholders.task')" :fieldValue="$task ? $task->heading : ''" />
                    </div>

                    <div class="col-md-6 col-lg-6">
                        <x-forms.label class="my-3" fieldId="category_id" :fieldLabel="__('modules.tasks.taskCategory')">
                        </x-forms.label>
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="category_id" id="task_category_id" data-live-search="true" data-size="10">
                                <option value="">--</option>
                                @if ($viewTaskCategoryPermission == 'all' || $viewTaskCategoryPermission == 'added')
                                    @foreach ($categories as $category)
                                        <option @if (!is_null($task) && $task->task_category_id == $category->id) selected @endif value="{{ $category->id }}">{{ html_entity_decode($category->category_name) }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                            ...
                            @if ($addTaskCategoryPermission == 'all' || $addTaskCategoryPermission == 'added')
                                <x-slot name="append">
                                    <button id="create_task_category" type="button" class="btn btn-outline-secondary border-grey" data-toggle="tooltip" data-original-title="{{ __('modules.taskCategory.addTaskCategory') }}">@lang('app.add')</button>
                                </x-slot>
                            @endif
                        </x-forms.input-group>
                    </div>
                    ...
                    @if (in_array('projects', user_modules()))
                        <div class="col-md-12 col-lg-6">
                            ...
                            @if (isset($project) && !is_null($project))
                                ...
                                <x-forms.text :fieldLabel="__('app.project')" fieldName="projectName" fieldId="projectName" :fieldValue="$project->project_name" fieldReadOnly="true" />
                            ...
                            @else
                                <x-forms.select fieldId="project_id" fieldName="project_id" :fieldLabel="__('app.project')" :popover="__('modules.tasks.notFinishedProjects')" search="true">
                                    <option value="">--</option>
                                    @if ($viewProjectPermission != 'none' && (in_array('employee', user_roles()) || in_array('client', user_roles())))
                                        @foreach ($projects as $data)
                                            <option data-content="{!! '<strong>' . $data->project_short_code . '</strong> ' . $data->project_name !!}" @if ((isset($project) && $project->id == $data->id) || (!is_null($task) && $data->id == $task->project_id)) selected @endif value="{{ $data->id }}">
                                            </option>
                                        @endforeach
                                    @endif
                                </x-forms.select>
                    ...
                    <div class="col-md-5 col-lg-3">
                        <x-forms.datepicker fieldId="task_start_date" fieldRequired="true" :fieldLabel="__('modules.projects.startDate')" fieldName="start_date" :fieldValue="$task ? $task->start_date->format(company()->date_format) : now(company()->timezone)->translatedFormat(company()->date_format)" :fieldPlaceholder="__('placeholders.date')" />
                    </div>

                    <div class="col-md-5 col-lg-3 dueDateBox" @if ($task && is_null($task->due_date)) style="display: none" @endif>
                        <x-forms.datepicker fieldId="due_date" fieldRequired="true" :fieldLabel="__('app.dueDate')" fieldName="due_date" :fieldPlaceholder="__('placeholders.date')" :fieldValue="$task && $task->due_date ? $task->due_date->format(company()->date_format) : now(company()->timezone)->translatedFormat(company()->date_format)" />
                    </div>
                    ...
                    @if (user()->permission('change_status') == 'all')
                        <div class="col-lg-3 col-md-6">
                            <x-forms.select fieldId="board_column_id" :fieldLabel="__('app.status')" fieldName="board_column_id" search="true">
                                @foreach ($taskboardColumns as $item)
                                    ...
                                    @endphp
                                    <option @if ($columnId == $item->id || (!is_null($task) && $task->board_column_id == $item->id)) selected
                    ...
                    <div class="col-md-12 col-lg-8">
                        <div class="form-group my-3">
                            ...
                            <x-forms.input-group>
                                <select class="form-control multiple-users" multiple name="user_id[]" id="selectAssignee" data-live-search="true" data-size="10">
                                    @foreach ($employees as $item)
                                        <x-user-option :user="$item" :pill="true" :selected="(isset($defaultAssignee) && $defaultAssignee == $item->id) || (!is_null($task) && isset($projectMember) && in_array($item->id, $projectMember))" />
                                    @endforeach
                                </select>

                                <x-slot name="preappend">
                                    <button id="assign-self" type="button" class="btn btn-outline-secondary border-grey" data-toggle="tooltip" data-original-title="@lang('modules.tasks.assignMe')">
                                        <img src="{{ user()->image_url }}" width="23" class="img-fluid rounded-circle">
                                    </button>
                                </x-slot>

                                @if ($addEmployeePermission == 'all' || $addEmployeePermission == 'added')
                                    <x-slot name="append">
                                        <button id="add-employee" type="button" class="btn btn-outline-secondary border-grey" data-toggle="tooltip" data-original-title="{{ __('modules.employees.addNewEmployee') }}">@lang('app.add')</button>
                                    </x-slot>
                                @endif
                            </x-forms.input-group>
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group my-3">
                            ...
                            <div id="description" class="min-h-100" style="min-height: 300px">{!! $task ? $task->description : '' !!}</div>
                ...
                <div class="row p-20 d-none" id="other-details">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 col-lg-4">
                                <div class="form-group my-3">
                                    ...
                                    <x-forms.input-group>
                                        <select class="select-picker form-control" multiple name="task_labels[]" id="task_labels" data-live-search="true" data-size="10">
                                            @foreach ($taskLabels as $label)
                                                <option data-content="<span class='badge badge-secondary' style='background-color: {{ $label->label_color }}'>{{ $label->label_name }}</span>" value="{{ $label->id }}" @if ($task && isset($selectedLabel) && in_array($label->id, $selectedLabel))
                                                    selected
                                            @endif>{{ $label->label_name }}</option>
                                            @endforeach
                                        </select>

                                        @if (user()->permission('task_labels') == 'all')
                                            <x-slot name="append">
                                                <button id="createTaskLabel" type="button" class="btn btn-outline-secondary border-grey" data-toggle="tooltip" data-original-title="{{ __('modules.taskLabel.addLabel') }}">@lang('app.add')</button>
                                            </x-slot>
                                        @endif
                                    </x-forms.input-group>
                                </div>
                            ...
                            @if (in_array('projects', user_modules()))
                                <div class="col-md-12 col-lg-4">
                                    <x-forms.select fieldName="milestone_id" fieldId="milestone-id" :fieldLabel="__('modules.projects.milestones')">
                                        <option value="">--</option>
                                        @if ($project)
                                            @if (in_array($viewMilestonePermission, ['all', 'owned', 'added']) || user()->id == $project->client_id)
                                                @foreach ($milestones as $item)
                                                    <option value="{{ $item->id }}" @selected(!is_null($task) && $item->id == $task->milestone_id)>{{ $item->milestone_title }}</option>
                                                @endforeach
                                            @endif
                                        @endif
                                    </x-forms.select>
                            ...
                            <div class="col-lg-3 col-md-6">
                                <x-forms.select fieldId="priority" :fieldLabel="__('modules.tasks.priority')" fieldName="priority">
                                    <option @selected(!is_null($task) && $task->priority == 'high') data-content="<i class='fa fa-circle mr-2' style='color: #dd0000'></i> @lang('modules.tasks.high')" value="high">@lang('modules.tasks.high')</option>
                                    <option @selected(!is_null($task) && $task->priority == 'medium') value="medium" data-content="<i class='fa fa-circle mr-2' style='color: #ffc202'></i> @lang('modules.tasks.medium')" @selected(is_null($task))>@lang('modules.tasks.medium')</option>
                                    <option @selected(!is_null($task) && $task->priority == 'low') data-content="<i class='fa fa-circle mr-2' style='color: #0a8a1f'></i> @lang('modules.tasks.low')" value="low">@lang('modules.tasks.low')</option>
                                </x-forms.select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-3">
                        <div class="form-group">
                            <div class="d-flex mt-5">
                                <x-forms.checkbox :fieldLabel="__('modules.tasks.makePrivate')" fieldName="is_private" fieldId="is_private" :popover="__('modules.tasks.privateInfo')" :checked="$task ? $task->is_private : ''" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-3">
                        <div class="form-group">
                            <div class="d-flex mt-5">
                                <x-forms.checkbox :fieldLabel="__('modules.tasks.billable')" :checked="true" fieldName="billable" fieldId="billable" :popover="__('modules.tasks.billableInfo')" :checked="$task ? $task->billable : ''" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-3">
                        <div class="form-group">
                            <div class="d-flex mt-5">
                                <x-forms.checkbox :fieldLabel="__('modules.tasks.setTimeEstimate')" fieldName="set_time_estimate" fieldId="set_time_estimate" :checked="$task ? $task->estimate_hours > 0 || $task->estimate_minutes > 0 : ''" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-3 d-none" id="set-time-estimate-fields">
                        <div class="form-group mt-5">
                            <input type="number" min="0" class="w-25 border rounded p-2 height-35 f-14" name="estimate_hours" value="{{ $task ? $task->estimate_hours : '0' }}">
                            @lang('app.hrs')
                            &nbsp;&nbsp;
                            <input type="number" min="0" name="estimate_minutes" value="{{ $task ? $task->estimate_minutes : '0' }}" class="w-25 height-35 f-14 border rounded p-2">
                            @lang('app.mins')
                        </div>
                    </div>
                    ...
                    @if (is_null($task))
                        <div class="col-md-6">
                            <div class="form-group my-3">
                                <div class="d-flex">
                                    <x-forms.checkbox :fieldLabel="__('modules.tasks.dependent')" fieldName="dependent" fieldId="dependent-task" :popover="__('modules.tasks.dependentTaskInfo')" />
                                </div>
                            </div>

                            <div class="d-none" id="dependent-fields">
                                <x-forms.select fieldId="dependent_task_id" :fieldLabel="__('modules.tasks.dependentTask')" fieldName="dependent_task_id" search="true">
                                    <option value="">--</option>
                                    @if ($projectID)
                                        @foreach ($dependantTasks as $dependantTask)
                                            <option value="{{ $dependantTask->id }}">{{ $dependantTask->heading }} ( @lang('app.dueDate'):
                                                @if (!is_null($dependantTask->due_date))
                                                    {{ $dependantTask->due_date->translatedFormat(company()->date_format) }} )
                                                @else
                                                    -
                                                @endif
                                            </option>
...
<script>
    $(document).ready(function() {
        ...
        if (typeof quillMention === 'function') {
            ...
            console.error('quillMention function not found');
        }
        ...
        function saveTask(data, url, buttonSelector) {
            ...
            var $btn = container.find(buttonSelector);
            var prevHtml = $btn.html();
            $btn.attr('data-prev-text', prevHtml);
        ...
        $('.toggle-other-details').click(function() {
            $(this).find('svg, i').toggleClass('fa-chevron-down fa-chevron-up');
