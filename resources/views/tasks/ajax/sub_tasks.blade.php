...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    @if (
        ...
        <div class="p-20">
            ...
            <div class="row">
                <div class="col-md-12">
                    <a class="f-15 f-w-500" href="javascript:;" id="add-sub-task"><i class="icons icon-plus font-weight-bold mr-1"></i>@lang('app.menu.addSubTask')
                    </a>
                </div>
            </div>
            ...
            @else
                <x-form id="save-subtask-data-form" class="d-none">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <x-forms.datepicker fieldId="sub_task_start_date" :fieldLabel="__('app.startDate')" fieldName="start_date" :fieldPlaceholder="__('placeholders.date')" />
                        </div>

                        <div class="col-md-4">
                            <x-forms.datepicker fieldId="sub_task_due_date" :fieldLabel="__('app.dueDate')" fieldName="due_date" :fieldPlaceholder="__('placeholders.date')" />
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group my-3">
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="user_id" id="subTaskAssignee" data-live-search="true">
                                        <option value="">--</option>
                        ...
                        <div class="col-md-12">
                            <a class="f-15 f-w-500" href="javascript:;" id="add-subtask-file"><i class="fa fa-paperclip font-weight-bold mr-1"></i>@lang('modules.projects.uploadFile')
                            </a>
                        </div>

                        @if ($addSubTaskPermission == 'all' || $addSubTaskPermission == 'added')
                            <div class="col-lg-12 add-file-box d-none">
                                <x-forms.file-multiple class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('modules.projects.uploadFile')" fieldName="file" fieldId="task-file-upload-dropzone" />
            ...
            @forelse ($task->subtasks as $subtask)
                <div class="card w-100 rounded-0 border-0 subtask mb-1">
                    ...
                    <div class="card-horizontal">
                        ...
                        <div class="d-flex">
                            <x-forms.checkbox :fieldId="'checkbox' . $subtask->id" class="task-check" data-sub-task-id="{{ $subtask->id }}" :checked="$subtask->status == 'complete' ? true : false" fieldLabel="" :fieldName="'checkbox' . $subtask->id" :fieldPermission="$checkBoxDisablePermission" />
                        </div>
                                ...
                                <div class="dropdown ml-auto subtask-action">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                        @if ($viewSubTaskPermission == 'all' || ($viewSubTaskPermission == 'added' && ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds))))
                                            <a class="dropdown-item view-subtask" href="javascript:;" data-row-id="{{ $subtask->id }}">@lang('app.view')</a>
                                        @endif
                                        @if (
                                            $editSubTaskPermission == 'all' ||
                                                ($editSubTaskPermission == 'added' && ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds))) ||
                                                ($editSubTaskPermission == 'owned' && $subtask->assigned_to == user()->id) ||
                                                ($editSubTaskPermission == 'both' && ($subtask->assigned_to == user()->id || ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds)))))
                                            <a class="dropdown-item edit-subtask" href="javascript:;" data-row-id="{{ $subtask->id }}">@lang('app.edit')</a>
                                        @endif

                                        @if (
                                            $deleteSubTaskPermission == 'all' ||
                                                ($deleteSubTaskPermission == 'added' && ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds))) ||
                                                ($deleteSubTaskPermission == 'owned' && $subtask->assigned_to == user()->id) ||
                                                ($deleteSubTaskPermission == 'both' && ($subtask->assigned_to == user()->id || ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds)))))
                                            <a class="dropdown-item delete-subtask" data-row-id="{{ $subtask->id }}" href="javascript:;">@lang('app.delete')</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            @if (count($subtask->files) > 0)
                                <div class="d-flex flex-wrap mt-4">
                                    @foreach ($subtask->files as $file)
                                        <x-file-card :fileName="$file->filename" :dateAdded="$file->created_at->diffForHumans()" class="subTask{{ $file->id }}">
                                            ...
                                            <x-slot name="action">
                                                <div class="dropdown ml-auto file-action">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                                        ...
                                                        <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 " href="{{ route('sub-task-files.download', md5($file->id)) }}">@lang('app.download')</a>
                                                        ...
                                                        @if ($deleteSubTaskPermission == 'all' || ($deleteSubTaskPermission == 'added' && ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds))))
                                                            <a class="cursor-pointer d-block text-dark-grey f-13 pb-3 px-3 delete-sub-task-file" data-row-id="{{ $file->id }}" href="javascript:;">@lang('app.delete')</a>
                                                        @endif
                                                    </div>
                                                </div>
                                            </x-slot>
    ...
    $(document).ready(function() {
        ...
        $('body').on('click', '#add-sub-task', function() {
            if (send_approval == 1 && employee == 1 && admin != 1 && needApproval == 1 && status == 'waiting_approval') {
                ...
                if ($('#description .ql-editor').length == 0) {
                    quillImageLoad('#description');
                }
            }
        });
