<div class="row gap-4">
    <div class="col-md-4">
        <div class='input-group w-auto'>
            <div class="input-group-prepend">
                <button id="week-start-date" data-date="{{ $weekStartDate->copy()->subDay()->toDateString() }}" type="button" class="btn btn-outline-secondary border-grey height-35"><i class="fa fa-chevron-left"></i>
                </button>
            </div>
            ...
            <div class="input-group-append">
                <button id="week-end-date" data-date="{{ $weekEndDate->copy()->addDay()->toDateString() }}" type="button" class="btn btn-outline-secondary border-grey height-35"><i class="fa fa-chevron-right"></i>
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-8 text-right">
        <span @class([
            ...
            'badge-danger' =>
                ($weekTimesheet && $weekTimesheet->status == 'draft') ||
                !$weekTimesheet,
                            ...
                            <div class="form-group d-flex justify-content-between">
                                <select class="form-control week-task select-picker" name="task_ids[{{ $key }}]" data-container="body" data-live-search="true">
                                    @foreach ($tasksForWeek as $task)
                                        <option value="{{ $task->id }}" {{ $key == $task->id ? 'selected' : '' }} data-content="<h5 class='f-13 text-darkest-grey mb-0'>{{ $task->heading }}</h5>
                                    @if ($task->project)
                        ...
                        @foreach ($entries as $key2 => $entry)
                            ...
                            <td class="p-1 hours-td date-index-{{ $key2 }}" data-index="{{ $key2 }}">
                                ...
                                @if (!\Carbon\Carbon::parse($day)->isFuture())
                                    <input type="number" class="form-control week-hours height-35" name="hours[{{ $key }}][]" min="0" max="24" value="{{ $entry->hours }}" />
                                @else
                                    <span class="text-muted">--</span>
                                    <input type="hidden" readonly class="week-hours" name="hours[{{ $key }}][]" value="{{ $entry->hours }}" />
                                @endif
                            </td>
                        @endforeach
                    </tr>
    ...
    $(document).ready(function() {
        ...
        $('.submit-timesheet').click(function(e) {
            ...
            if ($(this).data('status') == 'draft') {
                var $subBtn = $('#submit-timesheet');
                $subBtn.prop('disabled', true);
                $.easyBlockUI('#weekly-timesheet-form');
                window.apiHttp.postUrlEncoded("{{ route('weekly-timesheets.store') }}", $('#weekly-timesheet-form').serialize()).then(function(response) {
                    ...
                    $.easyUnblockUI('#weekly-timesheet-form');
                });
            } else {
                Swal.fire({
