...
| **Đổi tên**    | `export-cloudsql-allowlist-report.ps1` → **`export_sql_allowlist.ps1`** (output `FUNC_REPORT/CLOUDSQL_ALLOWLIST_ARCHIVE.md` hoặc `CLOUDSQL_ALLOWLIST_STATUS_<timestamp>.md`).     |
...
| **Tham chiếu** | `FUNC_IMPROVE/LEGACY_ARCHIVE.md` — retired audits; allowlist scripts trong repo `scripts/`.                                                                                       |
