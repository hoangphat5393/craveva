...
#
# Mặc định (2026-05): **có** chạy git add/commit/push trên máy dev rồi mới SSH deploy — khớp mong đợi “deploy = đẩy code lên rồi kéo hub”.
# Chỉ deploy mà **không** đụng repo local (chỉ pull những gì đã có trên origin — máy khác/CI đã push):
#   .\scripts\upload_hub.ps1 -DeployOnly
#   hoặc: .\scripts\upload_hub.ps1 -SkipLocalGit:$true
#
...
#   cd /var/www/hub.craveva.com && sudo -u hoangphat5393 git pull origin main
#
...
$remotePathEscaped = Escape-BashSingleQuotedString $HubPath
$remoteTokenEscaped = ""
if ($resolvedToken) {
...
ssh $HubHost "bash -lc '$remoteBashEscaped'"
if ($LASTEXITCODE -ne 0) {
