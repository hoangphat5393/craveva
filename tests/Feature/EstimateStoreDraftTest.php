...
it('stores estimate draft when first item row is blank but another row is valid', function (): void {
    ...
    $beforeEstimateCount = DB::table('estimates')->count();
    $beforeItemCount = DB::table('estimate_items')->count();
    ...
    $response = $this->actingAs($userAuth, 'web')
        ...
        ->post(route('estimates.store', ['type' => 'draft']), [
            '_token' => csrf_token(),
            'estimate_number' => 'EST-AUTO-' . now()->format('YmdHis'),
            'client_id' => $clientId,
            ...
            'item_name' => ['', 'Biomixing Draft Line'],
            'item_summary' => ['', 'Test line'],
            'quantity' => ['', '1'],
            'cost_per_item' => ['', '100'],
            'amount' => ['', '100'],
            'unit_id' => ['', ''],
    ...
    $response->assertJsonPath('redirectUrl', route('estimates.index'));

    expect(DB::table('estimates')->count())->toBe($beforeEstimateCount + 1);
    expect(DB::table('estimate_items')->count())->toBe($beforeItemCount + 1);

    $savedEstimate = DB::table('estimates')->latest('id')->first();
    expect($savedEstimate)->not->toBeNull();
    expect($savedEstimate->status)->toBe('draft');
});
